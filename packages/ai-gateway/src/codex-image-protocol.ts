...
export function validateCodexImageRequest(value: unknown, now?: number, expectedProvider?: ImageSpoolProvider): CodexImageRequest {
  ...
  if (Object.keys(v).some(k => !['schemaVersion', 'provider', 'id', 'prompt', 'promptHash', 'createdAt', 'deadlineAt'].includes(k)) || !['codex', 'chatgpt-web'].includes(provider as string) || (expectedProvider !== undefined && provider !== expectedProvider) || (expectedProvider === 'chatgpt-web' && v.provider === undefined) || typeof v.prompt !== 'string' || !v.prompt.trim() || v.prompt.length > 1500 || sha256Text(v.prompt) !== v.promptHash || typeof v.createdAt !== 'number' || !Number.isSafeInteger(v.createdAt) || v.createdAt < 0 || typeof v.deadlineAt !== 'number' || !Number.isSafeInteger(v.deadlineAt) || v.deadlineAt <= v.createdAt || v.deadlineAt - v.createdAt > CODEX_IMAGE_MAX_DEADLINE_MS) return invalid();
  if (now !== undefined && (v.deadlineAt <= now || v.createdAt > now)) throw new Error('EXPIRED');
  return { schemaVersion: 1, ...(v.provider !== undefined ? { provider: provider as ImageSpoolProvider } : {}), id: v.id as string, prompt: v.prompt, promptHash: v.promptHash as string, createdAt: v.createdAt, deadlineAt: v.deadlineAt };
}
