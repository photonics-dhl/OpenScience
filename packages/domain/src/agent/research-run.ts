...
export async function getHermesResearchRun(
  ...
  if (planningRecovery) {
    ...
    view.chargeableAttempts = planningRecovery.chargeableAttempts;
  ...
  const artResume = !artCorrection && WRITE_ROLES.has(authority.membership.role)
    ? await inspectStoryboardArtCorrection(deps.prisma, run, true).catch(() => null) : null;
  if (artCorrection || artResume) {
    view.canRetryGeneration = true;
    view.chargeableAttempts = artResume ? 0 : 2;
    view.generationRecovery = 'storyboard-art';
  }
  if (!artCorrection && !artResume && WRITE_ROLES.has(authority.membership.role) && run.profile === VISUAL_NARRATIVE_PROFILE && run.status === 'stopped') {
  ...
  if (WRITE_ROLES.has(authority.membership.role) && await inspectNarrativeImageRenderRecovery(deps.prisma, run).catch(() => null)) {
    view.canRetryGeneration = true;
    view.canAuthorizeNarrativeCorrection = false;
    view.generationRecovery = 'image-render';
    view.chargeableAttempts = 1;
  }
  const pixelReplan = WRITE_ROLES.has(authority.membership.role)
    ? await inspectNarrativePixelReplan(deps.prisma, run, deps.inspectImageRecoveryState).catch(() => null) : null;
  if (pixelReplan) {
    view.canRetryGeneration = true; view.canAuthorizeNarrativeCorrection = false;
    view.generationRecovery = 'narrative-scientific-replan';
    view.chargeableAttempts = 1 + pixelReplan.source.sceneReviews.length;
  }
  const pixelPlanRevision = WRITE_ROLES.has(authority.membership.role)
    ? await inspectNarrativePixelPlanRevision(deps.prisma, run).catch(() => null) : null;
  if (pixelPlanRevision) {
    view.canRetryGeneration = true; view.canAuthorizeNarrativeCorrection = false;
    view.generationRecovery = 'narrative-scientific-replan'; view.chargeableAttempts = 1;
  }
  const completedReviews = !pixelReplan && !pixelPlanRevision && WRITE_ROLES.has(authority.membership.role)
    ? await inspectCompletedImageReviewRecovery(deps.prisma, run, deps.canResumeImageReviewFromCompletedResult).catch(() => null) : null;
  if (completedReviews) {
    view.canRetryGeneration = true; view.canAuthorizeNarrativeCorrection = false;
    view.generationRecovery = 'image-render'; view.chargeableAttempts = 0;
  }
  const technical = !pixelReplan && !pixelPlanRevision && !completedReviews && WRITE_ROLES.has(authority.membership.role)
...
async function inspectNarrativePixelReplan(tx: Prisma.TransactionClient, run: RunRow,
  inspectImageRecoveryState?: HermesResearchRunDeps['inspectImageRecoveryState']) {
  if (run.profile !== VISUAL_NARRATIVE_PROFILE || (run.status !== 'failed' && (run.status !== 'stopped' || run.error !== NARRATIVE_CORRECTION_STOP))
    || !run.versionId || await validateReviewedSources(tx, run) !== 'ready') return null;
  ...
  if (!ro || ro.deletedAt || ro.status !== 'draft'
    ...
    || images.some((image, index) => image.ordinal !== index || !['failed', 'stopped', 'awaiting_approval', 'succeeded'].includes(image.status)
      || !image.agentTaskId || (image.presentationAssetId !== null && image.presentationAssetId !== image.agentTaskId))
    || run.steps.some(item => !['source_ingestion', 'source_composition', 'source_review', 'storyboard', 'scene_image'].includes(item.stage))
    || await tx.agentTask.count({ where: { OR: [{ id: { in: run.steps.flatMap(item => item.agentTaskId ? [item.agentTaskId] : []) } },
      { payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } }], status: { in: ['pending', 'running'] } } }) !== 0
    || await tx.auditLog.findFirst({ where: { action: 'hermes.research_run.generation_retry', targetType: 'hermes_research_run', targetId: run.id,
      AND: [{ metadata: { path: ['correction'], equals: NARRATIVE_PIXEL_REPLAN } },
        { metadata: { path: ['parentStoryboardAssetId'], equals: step.agentTaskId } }] } })) return null;
  const source = await readNarrativePixelReplanSource(tx, { actorId: run.actorId, runId: run.id,
    ...
    parentAssetId: step.agentTaskId, imageAssetIds: images.map(image => image.agentTaskId!), terminalSceneSet: { inspectImageRecoveryState } }).catch(() => null);
  if (!source) return null;
  const checkpoint = jsonRecord(jsonRecord(source.task.result).storyboardCheckpoint);
  if (!isDeepStrictEqual(checkpoint.payload, source.task.payload)
    || checkpoint.sourceEvidenceIdentity !== source.sourceEvidenceIdentity
  ...
  const existingTaskCount = await tx.agentTask.count({ where: { kind: 'presentation.generate',
    ...
    + run.steps.filter(item => ['source_composition', 'source_review'].includes(item.stage)).length;
  if (!Number.isSafeInteger(run.maxAgentTasks) || existingTaskCount > run.maxAgentTasks!) return null;
  ...
  return { [PIXEL_REPLAN_ELIGIBILITY]: true as const, runId: run.id, source, checkpoint,
    replacesTaskId: source.task.id,
    sceneSteps: images.map(({ id, ordinal, status, agentTaskId, presentationAssetId, error }) => ({ id, ordinal, status, agentTaskId, presentationAssetId, error })),
    previousMaxAgentTasks: run.maxAgentTasks!, existingTaskCount,
    maxAgentTasks: Math.max(run.maxAgentTasks!, existingTaskCount + 1 + source.sceneReviews.length),
    payload: { ...source.payload, storyboard: { ...settings, revisionImageAssetId: source.image.id,
      narrativeSceneLimit: source.sceneReviews.length } } };
}

function pixelSceneBindingMatches(step: RunRow['steps'][number], index: number,
  pixel: NonNullable<Awaited<ReturnType<typeof readNarrativePixelReplanAuthority>>>) {
  const recorded = pixel.metadata.sceneSet === 'terminal' ? jsonRecord((pixel.metadata.sceneSteps as unknown[])[index]) : null;
  return step.ordinal === index && step.status === 'stopped' && step.agentTaskId === pixel.sceneReviews[index]!.taskId
    && step.presentationAssetId === (recorded ? recorded.presentationAssetId : pixel.sceneReviews[index]!.imageId)
    && (!recorded || step.id === recorded.id);
}

/** A current policy failure in an accepted plan is source evidence, not a fabricated pixel-review rejection. */
async function inspectNarrativeSourceSupportReplan(tx: Prisma.TransactionClient, run: RunRow, storage: HermesResearchRunDeps['storage']) {
  if (!storage || run.profile !== VISUAL_NARRATIVE_PROFILE || !['failed', 'stopped'].includes(run.status) || !run.versionId
    || await validateReviewedSources(tx, run) !== 'ready') return null;
  const ro = await tx.researchObject.findUnique({ where: { id: run.researchObjectId } });
  const version = await tx.version.findUnique({ where: { id: run.versionId } });
  const plans = run.steps.filter(step => step.stage === 'storyboard'); const plan = plans[0];
  ...
  if (!ro || ro.deletedAt || ro.status !== 'draft' || !version || version.status !== 'draft' || version.publicVersionId || version.publicationNo
    || plans.length !== 1 || !plan?.agentTaskId || plan.status !== 'succeeded' || plan.presentationAssetId !== plan.agentTaskId
    || images.length < 1 || images.length > 6 || images.some((step, index) => step.ordinal !== index || !step.agentTaskId
      || !['failed', 'stopped', 'awaiting_approval', 'succeeded'].includes(step.status)
    ...
    || await tx.agentTask.count({ where: { status: { in: ['pending', 'running'] }, OR: [
      { id: { in: run.steps.flatMap(step => step.agentTaskId ? [step.agentTaskId] : []) } },
      { payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } }] } }) !== 0
    || await tx.auditLog.findFirst({ where: { action: 'hermes.research_run.generation_retry', targetType: 'hermes_research_run', targetId: run.id,
      AND: [{ metadata: { path: ['cause'], equals: STORYBOARD_SOURCE_SUPPORT_INVALID } },
        { metadata: { path: ['parentStoryboardAssetId'], equals: plan.agentTaskId } }] }, select: { id: true } })) return null;
  const parent = await readNarrativeSourceSupportParent(tx, { actorId: run.actorId, runId: run.id,
    researchObjectId: run.researchObjectId, versionId: run.versionId, sourceClaimIds: run.sourceClaimIds, parentAssetId: plan.agentTaskId });
  const prior = await readNarrativePixelReplanAuthority(tx, { runId: run.id, actorId: run.actorId });
  if (prior?.metadata.cause === STORYBOARD_SOURCE_SUPPORT_INVALID || parent.checkpoint.baseIdentity !== (prior?.baseIdentity ?? null)
    || parent.view.document.scenes.length !== images.length) return null;
  const evidence = await readNarrativeCheckpointEvidence(tx, run);
  if (!evidence || parent.checkpoint.claimContent !== evidence.claimContent || parent.checkpoint.sourceEvidenceIdentity !== evidence.sourceEvidenceIdentity
    || parent.checkpoint.narrativeSourceIdentity !== evidence.narrativeSourceIdentity) return null;
  const projected = await projectIllustrationEvidence(storage, evidence.evidence);
  const claims = evidence.claims.map(claim => ({ id: claim.id, sourcePassages: evidence.evidence.filter(row => row.claimId === claim.id)
    .map(row => ({ evidenceId: row.id, text: row.exactQuote!, relation: row.relation })) }));
  const invalidBases = [];
  for (const [sceneIndex, scene] of parent.view.document.scenes.entries()) {
    if (!scene.illustration) return null;
    requireIllustrationSourceSupport(scene.illustration, claims);
    for (const [subjectIndex, subject] of scene.illustration.subjects.entries()) {
      const row = projected.find(candidate => candidate.id === subject.basis.evidenceId && candidate.claimId === subject.basis.claimId);
      if (!row || !row.exactQuote?.includes(subject.basis.quote)) return null;
      if (row.relation === 'supports') continue;
      if (row.relation !== 'context' || !row.headingProof || !row.headingProof.text.includes(row.exactQuote)
        || !row.headingProof.text.includes(subject.basis.quote)) return null;
      invalidBases.push({ sceneIndex, subjectIndex, basis: subject.basis, heading: row.headingProof });
    }
  }
  if (!invalidBases.length) return null;
  const oldTasks = await tx.agentTask.findMany({ where: { id: { in: images.map(step => step.agentTaskId!) } }, include: { session: true } });
  if (oldTasks.length !== images.length || oldTasks.some(task => {
    const input = parsePresentationGenerationPayload(task.payload); const step = images.find(item => item.agentTaskId === task.id)!;
    return task.deletedAt || !['failed', 'succeeded'].includes(task.status) || task.kind !== 'presentation.generate'
      ...
      || input.researchObjectId !== run.researchObjectId || input.versionId !== run.versionId || input.kind !== 'image'
      || !isDeepStrictEqual(input.sourceClaimIds, parent.payload.sourceClaimIds)
      || input.sceneImage?.storyboardAssetId !== parent.parent.id || input.sceneImage?.sceneIndex !== step.ordinal
      || !isDeepStrictEqual(input.hermesRunAuthority, { runId: run.id, stage: 'scene_image', ordinal: step.ordinal, profile: run.profile });
  })) return null;
  const existingTaskCount = await tx.agentTask.count({ where: { kind: 'presentation.generate', payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } } })
    + run.steps.filter(step => ['source_composition', 'source_review'].includes(step.stage)).length;
  if (!Number.isSafeInteger(run.maxAgentTasks) || existingTaskCount > run.maxAgentTasks!) return null;
  const sourceProof = { parentTaskIdentity: parent.taskIdentity, invalidBases };
  ...
  return { [PIXEL_REPLAN_ELIGIBILITY]: true as const, runId: run.id, parent, sourceProof,
    sourceIdentity: JSON.stringify({ parentIdentity: parent.parentIdentity, sourceProof }),
    replacesTaskId: parent.task.id, previousMaxAgentTasks: run.maxAgentTasks!, existingTaskCount,
    maxAgentTasks: Math.max(run.maxAgentTasks!, existingTaskCount + 1 + images.length),
    sceneSteps: images.map(({ id, ordinal, status, agentTaskId, presentationAssetId, error }) => ({ id, ordinal, status, agentTaskId, presentationAssetId, error })),
    payload: { ...parent.payload, storyboard: { ...settings, narrativeSceneLimit: images.length } } };
}

async function inspectNarrativeTechnicalRecovery(tx: Prisma.TransactionClient, run: RunRow, deps: HermesResearchRunDeps) {
  if (run.profile !== VISUAL_NARRATIVE_PROFILE || run.status !== 'failed' || !run.versionId
    || !deps.inspectImageRecoveryState || await validateReviewedSources(tx, run) !== 'ready') return null;
  ...
  if (!pixel || pixel.technicalRecovery || !ro || ro.deletedAt || ro.status !== 'draft'
    ...
    || scenes.some((step, index) => step.ordinal !== index || !step.agentTaskId
      || (step.presentationAssetId !== null && step.presentationAssetId !== step.agentTaskId))
    || run.steps.some(step => !['source_ingestion', 'source_composition', 'source_review', 'storyboard', 'scene_image'].includes(step.stage))
...
async function readCompletedImageReviewSource(tx: Prisma.TransactionClient, run: RunRow, taskId: string) {
  ...
  const pixel = await readNarrativePixelReplanAuthority(tx, { runId: run.id, actorId: run.actorId });
  const step = run.steps.find(item => item.stage === 'scene_image' && item.agentTaskId === taskId);
  const task = await tx.agentTask.findUnique({ where: { id: taskId }, include: { session: true } });
  if (!pixel || !step || !task || task.deletedAt || task.kind !== 'presentation.generate' || task.session.deletedAt
    || task.session.status !== 'active' || task.session.userId !== run.actorId || task.session.researchObjectId !== run.researchObjectId
    || step.ordinal < 0 || step.ordinal >= pixel.sceneLimit
    || (step.presentationAssetId !== null && step.presentationAssetId !== task.id)
    || pixel.task.status !== 'succeeded' || pixel.step.status !== 'succeeded' || pixel.step.presentationAssetId !== pixel.task.id) return null;
  const payload = parsePresentationGenerationPayload(task.payload);
  if (payload.researchObjectId !== run.researchObjectId || payload.versionId !== run.versionId || payload.kind !== 'image'
    || !isDeepStrictEqual(payload.sourceClaimIds, run.sourceClaimIds)
    || !isDeepStrictEqual(payload.sceneImage, { storyboardAssetId: pixel.task.id, sceneIndex: step.ordinal })
    || !isDeepStrictEqual(payload.hermesRunAuthority, { runId: run.id, stage: 'scene_image', ordinal: step.ordinal, profile: run.profile })) return null;
  ...
  const provenance = jsonRecord(asset?.provenance); const { imageReview: _review, ...imageProvenance } = provenance;
  const checkpoint = jsonRecord(jsonRecord(pixel.task.result).storyboardCheckpoint);
  if (!parent || parent.view.output !== 'image' || !parent.view.narrative || !asset || asset.deletedAt || asset.kind !== 'image' || asset.status !== 'draft'
    || asset.researchObjectId !== run.researchObjectId || asset.versionId !== run.versionId || !asset.objectKey
    || !/^[a-f0-9]{64}$/.test(asset.contentHash) || provenance.contentType !== 'image/png'
    || provenance.taskId !== task.id || provenance.source !== 'approved_storyboard_scene'
    || provenance.parentIdentity !== parent.identity || provenance.storyboardContentHash !== parent.contentHash
    || provenance.sourceEvidenceIdentity !== parent.sourceEvidenceIdentity || parent.sourceEvidenceIdentity !== pixel.metadata.sourceEvidenceIdentity
    || !isDeepStrictEqual(presentationSceneImageView(asset), payload.sceneImage)
    || !isDeepStrictEqual(asset.sourceClaims.map(link => link.claimId).sort(), run.sourceClaimIds)
    || !isDeepStrictEqual(checkpoint.payload, pixel.task.payload) || checkpoint.sourceEvidenceIdentity !== parent.sourceEvidenceIdentity
    || !await readUnchangedNarrativeCheckpointEvidence(tx, run, checkpoint)) return null;
  return { step, task, payload, asset, identity: JSON.stringify({ taskId, taskPayload: task.payload, stepId: step.id, ordinal: step.ordinal,
    pixelReceiptId: pixel.receipt.id, rootReceiptId: pixel.rootReceipt.id, parentIdentity: parent.identity,
    sourceEvidenceIdentity: checkpoint.sourceEvidenceIdentity, narrativeSourceIdentity: checkpoint.narrativeSourceIdentity,
    claimContent: checkpoint.claimContent, assetId: asset.id, contentHash: asset.contentHash, objectKey: asset.objectKey, imageProvenance }) };
}

async function inspectCompletedImageReviewRecovery(tx: Prisma.TransactionClient, run: RunRow,
  canResume?: HermesResearchRunDeps['canResumeImageReviewFromCompletedResult']) {
  if (!canResume || run.profile !== VISUAL_NARRATIVE_PROFILE || run.status !== 'failed' || !run.versionId) return null;
  const ro = await tx.researchObject.findUnique({ where: { id: run.researchObjectId } });
  const version = await tx.version.findUnique({ where: { id: run.versionId } });
  if (!ro || ro.deletedAt || ro.status !== 'draft' || !version || version.status !== 'draft' || version.researchObjectId !== ro.id
    || version.publicVersionId || version.publicationNo
    || await tx.agentTask.count({ where: { OR: [{ id: { in: run.steps.flatMap(step => step.agentTaskId ? [step.agentTaskId] : []) } },
      { payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } }], status: { in: ['pending', 'running'] } } }) !== 0) return null;
  const recoverable = [];
...
export async function requireHermesCompletedImageReviewRecovery(tx: Prisma.TransactionClient, input: { taskId: string; actorId: string; runId: string }) {
...
async function inspectNarrativePixelPlanRevision(tx: Prisma.TransactionClient, run: RunRow) {
  if (run.profile !== VISUAL_NARRATIVE_PROFILE || run.status !== 'stopped' || !run.versionId
    || !run.error?.startsWith('[blocked] Illustration needs upstream scientific revision: ')
    || await validateReviewedSources(tx, run) !== 'ready') return null;
  const pixel = await readNarrativePixelReplanAuthority(tx, { runId: run.id, actorId: run.actorId });
  if (!pixel || pixel.depth >= 2 || pixel.task.error !== run.error || pixel.step.status !== 'stopped' || pixel.step.presentationAssetId) return null;
  const ro = await tx.researchObject.findUnique({ where: { id: run.researchObjectId } });
  const version = await tx.version.findUnique({ where: { id: run.versionId } });
  const images = run.steps.filter(step => step.stage === 'scene_image').sort((a, b) => a.ordinal - b.ordinal);
  ...
  for (const [index, raw] of (failed.review.issues as unknown[]).entries()) {
    const issue = jsonRecord(raw); const scene = typeof issue.sceneIndex === 'number' ? failed.document.scenes[issue.sceneIndex] : undefined;
    if (issue.id !== `issue-${index + 1}` || !Number.isInteger(issue.sceneIndex) || !scene
      || !['requires_replan', 'label_clarification'].includes(String(issue.kind))
      ...
      || issue.sources.some(rawSource => { const item = jsonRecord(rawSource); return !evidence.some(row => row.id === item.evidenceId && row.claimId === item.claimId); })
      || (issue.kind === 'requires_replan' ? issue.labelIndex !== null : !Number.isInteger(issue.labelIndex)
        || typeof scene.illustration?.labels[Number(issue.labelIndex)] !== 'string' || !issue.sources.length)) return null;
  }
  const existingTaskCount = await tx.agentTask.count({ where: { kind: 'presentation.generate',
    payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } } })
    + run.steps.filter(step => ['source_composition', 'source_review'].includes(step.stage)).length;
  if (existingTaskCount !== Number(pixel.metadata.existingTaskCount) + pixel.depth + 1) return null;
  const { revisionTaskId: _task, revisionImageAssetId: _image, baseAssetId: _base, revisionMode: _mode, ...settings } = pixel.payload.storyboard!;
  return { [PIXEL_REPLAN_ELIGIBILITY]: true as const, runId: run.id, pixel, failed, existingTaskCount,
    replacesTaskId: pixel.task.id, previousMaxAgentTasks: run.maxAgentTasks!, maxAgentTasks: run.maxAgentTasks! + 1,
    payload: { ...pixel.payload, storyboard: { ...settings, revisionTaskId: pixel.task.id } } };
}

/** One explicit extra render of a rejected original; revisions never receive another generation. */
async function inspectNarrativeImageRenderRecovery(tx: Prisma.TransactionClient, run: RunRow) {
  if (run.profile !== VISUAL_NARRATIVE_PROFILE || typeof run.maxAgentTasks !== 'number'
    || ![9, 11, 13].includes(run.maxAgentTasks) || run.status !== 'stopped'
  ...
  const step = imageSteps[0]; const storyboard = storyboards[0];
  if (imageSteps.length !== 1 || storyboards.length !== 1 || !step || !storyboard || step.ordinal !== 0
    || step.status !== 'stopped' || !step.agentTaskId || step.presentationAssetId !== step.agentTaskId
    || storyboard.status !== 'succeeded' || storyboard.ordinal !== 0 || storyboard.presentationAssetId !== storyboard.agentTaskId
    || await tx.auditLog.findFirst({ where: { action: HERMES_IMAGE_RENDER_RECOVERY_ACTION,
      targetType: 'hermes_research_run', targetId: run.id }, select: { id: true } })) return null;
  const source = await readNarrativeImageRenderSource(tx, { actorId: run.actorId, runId: run.id,
    researchObjectId: run.researchObjectId, versionId: run.versionId, sourceClaimIds: run.sourceClaimIds, imageAssetId: step.agentTaskId }).catch(() => null);
  if (!source || source.parentTask.id !== storyboard.agentTaskId || source.parentAsset.id !== storyboard.presentationAssetId) return null;
  try { await requireSceneImageSpendIsNew(tx, source.parent, source.payload); } catch { return null; }
  const presentations = await tx.agentTask.findMany({ where: { kind: 'presentation.generate',
    payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } }, select: { id: true, status: true, payload: true } });
  const taskCount = presentations.length + run.steps.filter(item => ['source_composition', 'source_review'].includes(item.stage)).length;
  if (taskCount !== run.maxAgentTasks || presentations.some(task => ['pending', 'running'].includes(task.status)
    || jsonRecord(jsonRecord(task.payload).sceneImage).revisionAssetId === source.image.id)) return null;
  return { [IMAGE_RENDER_ELIGIBILITY]: true as const, runId: run.id, previousMaxAgentTasks: run.maxAgentTasks,
    existingTaskCount: taskCount, step, source };
}

type InitialScienceFailureClass = 'initial_science_thinking_exhausted' | 'initial_science_schema_exhausted';
const INITIAL_SCIENCE_ERRORS: Record<InitialScienceFailureClass, string> = {
  initial_science_thinking_exhausted: 'Provider exhausted output allowance before producing text',
  initial_science_schema_exhausted: '结构化输出超过重试上限',
};

function initialScienceCallsMatch(calls: Array<{ actorId: string | null; targetType: string | null; metadata: Prisma.JsonValue }>, failureClass: InitialScienceFailureClass) {
  const thinking = failureClass === 'initial_science_thinking_exhausted';
  const first = jsonRecord(calls[0]?.metadata);
  return calls.length === (thinking ? 2 : 3) && calls.every((call, index) => {
    const meta = jsonRecord(call.metadata); const blocks = jsonRecord(meta.responseBlockCounts);
    const tokens = thinking ? (index === 0 ? 16384 : 32768) : 65536;
    return call.actorId === null && call.targetType === 'ai_gateway' && meta.operation === 'text'
      ...
      && meta.maxOutputTokens === tokens && typeof meta.provider === 'string' && Boolean(meta.provider)
      && typeof meta.model === 'string' && Boolean(meta.model) && meta.provider === first.provider && meta.model === first.model
      && typeof meta.promptHash === 'string' && /^[a-f0-9]{64}$/.test(meta.promptHash)
      && (thinking ? meta.outcome === 'failed' && meta.error === 'provider_empty' && meta.finishReason === 'length'
        && meta.outputTokens === tokens && meta.promptHash === first.promptHash && blocks.text === 0 && blocks.other === 0
        && typeof blocks.thinking === 'number' && Number.isSafeInteger(blocks.thinking) && blocks.thinking > 0
        : meta.outcome === 'succeeded' && meta.error === null && meta.finishReason === 'stop');
  });
}

/** Shared by the retry transaction and worker: only a continuous same-source initial-planning chain is authority. */
export async function readInitialSciencePlanningRetryChain(prisma: Pick<Prisma.TransactionClient, 'auditLog'>, input: {
  runId: string; actorId: string; taskId: string; retryCount: number; taskPayload: unknown;
  sourceEvidenceIdentity: string; claimContent: string; narrativeSourceIdentity: string; remainingImageTasks: number;
}) {
  const receipts = await prisma.auditLog.findMany({ where: { action: 'hermes.research_run.generation_retry',
    targetType: 'hermes_research_run', targetId: input.runId, actorId: input.actorId,
    metadata: { path: ['taskId'], equals: input.taskId } }, orderBy: [{ createdAt: 'asc' }, { id: 'asc' }], take: 3 });
  if (!Number.isSafeInteger(input.retryCount) || input.retryCount < 0 || input.retryCount > 2 || receipts.length !== input.retryCount)
    throw new Error('[blocked] Initial science recovery receipt count changed');
  const calls = await prisma.auditLog.findMany({ where: { requestId: input.taskId, action: 'ai.gateway.call' },
    orderBy: [{ createdAt: 'asc' }, { id: 'asc' }] });
  const consumedAuditIds = new Set<string>();
  const chain = [];
  for (const [index, receipt] of receipts.entries()) {
    const meta = jsonRecord(receipt.metadata);
    const failureClass = meta.planningFailureClass as InitialScienceFailureClass;
    const auditIds = meta.planningAuditIds;
    if (receipt.actorId !== input.actorId || meta.correction !== STORYBOARD_PLANNING_RETRY || meta.taskId !== input.taskId
      || !['initial_science_thinking_exhausted', 'initial_science_schema_exhausted'].includes(failureClass)
      || (index > 0 && failureClass !== 'initial_science_schema_exhausted')
      || meta.previousExecutionAttempt !== index + 1 || meta.previousRetryCount !== index
      || (index === 0 ? meta.previousReceiptId != null : meta.previousReceiptId !== receipts[index - 1]!.id)
      || meta.previousError !== INITIAL_SCIENCE_ERRORS[failureClass] || meta.baseIdentity !== null
      || !isDeepStrictEqual(meta.taskPayload, input.taskPayload) || meta.sourceEvidenceIdentity !== input.sourceEvidenceIdentity
      || meta.claimContent !== input.claimContent || meta.narrativeSourceIdentity !== input.narrativeSourceIdentity
      || meta.remainingImageTasks !== input.remainingImageTasks || meta.chargeableAttempts !== 1 || meta.newTaskCount !== 0
      || meta.noProviderSwitch !== true || !Array.isArray(auditIds)
      || auditIds.some(id => typeof id !== 'string' || consumedAuditIds.has(id)) || new Set(auditIds).size !== auditIds.length)
      throw new Error('[blocked] Initial science recovery receipt identity changed');
    const bound = calls.filter(call => auditIds.includes(call.id));
    if (bound.length !== auditIds.length || !initialScienceCallsMatch(bound, failureClass)
      || bound.some(call => call.createdAt > receipt.createdAt || (index > 0 && call.createdAt <= receipts[index - 1]!.createdAt))
      || (calls[0] && bound.some(call => jsonRecord(call.metadata).provider !== jsonRecord(calls[0]!.metadata).provider
        || jsonRecord(call.metadata).model !== jsonRecord(calls[0]!.metadata).model)))
      throw new Error('[blocked] Initial science recovery provider evidence changed');
    auditIds.forEach(id => consumedAuditIds.add(id as string));
    chain.push({ id: receipt.id, createdAt: receipt.createdAt, metadata: meta, failureClass });
  }
  return { receipts: chain, consumedAuditIds: [...consumedAuditIds] };
}
...
async function inspectPixelPlanningRetry(tx: Prisma.TransactionClient, run: RunRow, failureClass: PixelPlanningFailureClass) {
  ...
  if (source.identity !== pixel.metadata.sourceIdentity || source.parentIdentity !== pixel.metadata.parentIdentity
    || !isDeepStrictEqual(source.sceneReviews, pixel.sceneReviews) || !isDeepStrictEqual(checkpoint.payload, source.task.payload)
    ...
    || !await readUnchangedNarrativeCheckpointEvidence(tx, run, checkpoint)) return null;
  const taskCount = await tx.agentTask.count({ where: { kind: 'presentation.generate',
    payload: { path: ['hermesRunAuthority', 'runId'], equals: run.id } } })
    + run.steps.filter(step => ['source_composition', 'source_review'].includes(step.stage)).length;
...
async function inspectFailedStoryboardPlanning(tx: Prisma.TransactionClient, run: RunRow) {
  ...
  const initialSchemaFailure = run.status === 'failed' && run.maxAgentTasks === 9
    && run.error === INITIAL_SCIENCE_ERRORS.initial_science_schema_exhausted;
  const initialFailure = initialThinkingFailure || initialSchemaFailure;
  if (run.profile !== VISUAL_NARRATIVE_PROFILE || !validGrant(run) || !run.versionId
    || (!initialFailure && (run.status !== 'stopped' || run.error !== '结构化输出超过重试上限'))
    || await validateReviewedSources(tx, run) !== 'ready') return null;
  const ro = await tx.researchObject.findUnique({ where: { id: run.researchObjectId } });
  const storyboards = run.steps.filter(step => step.stage === 'storyboard');
  ...
  if (payload.researchObjectId !== run.researchObjectId || payload.versionId !== run.versionId
    ...
    || (!initialFailure && (!payload.storyboard.revisionImageAssetId || payload.storyboard.narrativeSceneLimit !== 1))
  ...
  if (presentations.some(item => ['pending', 'running'].includes(item.status))
    || (!initialFailure && sourceCount + presentations.length + 1 !== run.maxAgentTasks)
    || await tx.presentationAsset.findUnique({ where: { id: task.id }, select: { id: true } })) return null;
  const calls = await tx.auditLog.findMany({ where: { requestId: task.id, action: 'ai.gateway.call' }, orderBy: [{ createdAt: 'asc' }, { id: 'asc' }] });
  ...
  if (initialFailure) {
    if (step.status !== 'failed' || step.error !== run.error || task.idempotencyKey !== `hermes-run:${run.id}:storyboard:0`
      ...
      || await tx.agentTask.count({ where: { id: { in: run.steps.flatMap(item => item.agentTaskId ? [item.agentTaskId] : []) },
        status: { in: ['pending', 'running'] } } }) !== 0) return null;
    const source = await readNarrativeCheckpointEvidence(tx, run);
    const review = jsonRecord(source?.sourceResult.scientificReview);
    if (!source || source.version.publicVersionId || source.version.publicationNo
      || review.status !== 'review_received' || review.contractVersion !== '5'
      ...
      || source.claims.some(claim => claim.updatedAt > task.createdAt) || source.evidence.some(row => row.updatedAt > task.createdAt)) return null;
    const chain = await readInitialSciencePlanningRetryChain(tx, { runId: run.id, actorId: run.actorId, taskId: task.id,
      retryCount: task.retryCount, taskPayload: task.payload, sourceEvidenceIdentity: source.sourceEvidenceIdentity,
      claimContent: source.claimContent, narrativeSourceIdentity: source.narrativeSourceIdentity,
      remainingImageTasks: payload.storyboard.narrativeSceneLimit! }).catch(() => null);
    if (!chain) return null;
    const previousReceipt = chain.receipts.at(-1);
    const currentCalls = calls.filter(call => !chain.consumedAuditIds.includes(call.id));
    const planningFailureClass = initialThinkingFailure ? 'initial_science_thinking_exhausted' as const : 'initial_science_schema_exhausted' as const;
    if (!initialScienceCallsMatch(currentCalls, planningFailureClass)
      || currentCalls.some(call => call.createdAt < task.createdAt || call.createdAt > task.updatedAt
        || (previousReceipt && call.createdAt <= previousReceipt.createdAt)
        || jsonRecord(call.metadata).model !== jsonRecord(calls[0]!.metadata).model)) return null;
    return { step, task, planningAuditIds: currentCalls.map(call => call.id), baseIdentity: null,
      ...
      planningFailureClass, previousReceiptId: previousReceipt?.id, chargeableAttempts: 1 as const };
  }
  ...
  return { step, task, planningAuditIds: calls.map(call => call.id), baseIdentity: source.identity,
    ...
    planningFailureClass: undefined, previousReceiptId: undefined, claimContent: undefined, narrativeSourceIdentity: undefined, chargeableAttempts: 1 as const };
}
...
const STORYBOARD_ART_RESUME_ACTION = 'hermes.research_run.storyboard_art_presubmission_resume';

/** Authorize art and review once, or resume that authorization once before either was submitted. */
async function inspectStoryboardArtCorrection(tx: Prisma.TransactionClient, run: RunRow, resumeBeforeSubmission = false) {
  if (run.profile !== VISUAL_NARRATIVE_PROFILE || run.status !== 'stopped' || !run.versionId
    || !validGrant(run) || await validateReviewedSources(tx, run) !== 'ready') return null;
  const version = await tx.version.findUnique({ where: { id: run.versionId } });
  const ro = await tx.researchObject.findUnique({ where: { id: run.researchObjectId } });
  if (!ro || ro.deletedAt || ro.status !== 'draft'
    || !version || version.status !== 'draft' || version.researchObjectId !== run.researchObjectId) return null;
  ...
  if (payload.researchObjectId !== run.researchObjectId || payload.versionId !== run.versionId
    ...
    || !isDeepStrictEqual(payload.hermesRunAuthority, { runId: run.id, stage: 'storyboard', ordinal: 0, profile: run.profile })) return null;
  const result = jsonRecord(task.result);
  if (Object.keys(result).sort().join(',') !== (resumeBeforeSubmission
    ? 'storyboardArtCorrection,storyboardCheckpoint,storyboardReview' : 'storyboardCheckpoint,storyboardReview')) return null;
  const checkpoint = jsonRecord(result.storyboardCheckpoint);
  const planned = jsonRecord(checkpoint.planned);
  const review = jsonRecord(result.storyboardReview);
  const document = parseStoryboardDocument(planned.document, payload.sourceClaimIds, 'image');
  ...
  if (presentations.some(item => ['pending', 'running'].includes(item.status))
    ...
    || await tx.presentationAsset.findUnique({ where: { id: task.id }, select: { id: true } })) return null;
  let authorization: Awaited<ReturnType<typeof readStoryboardArtCorrectionAuthorization>> | undefined;
  if (resumeBeforeSubmission) {
    // No submitted marker is stronger than an absent gateway audit: code always saves it before invoking the provider.
    if (Object.keys(jsonRecord(result.storyboardArtCorrection)).sort().join(',')
      !== 'authorizationRequestDigest,executionAttempt,originalCandidateHash,originalReviewResponseHash,schemaVersion') return null;
    authorization = await readStoryboardArtCorrectionAuthorization(tx, { taskId: task.id, actorId: run.actorId,
      workspaceId: ro.workspaceId, payload, executionAttempt: task.executionAttempt, retryCount: task.retryCount,
      result: task.result }, 'stopped');
    if (await tx.auditLog.findFirst({ where: { requestId: task.id, action: 'ai.gateway.call',
      createdAt: { gte: authorization.createdAt } }, select: { id: true } })
      || await tx.auditLog.findFirst({ where: { action: STORYBOARD_ART_RESUME_ACTION,
        targetType: 'hermes_research_run', targetId: run.id,
        metadata: { path: ['taskId'], equals: task.id } }, select: { id: true } })) return null;
  } else if (await tx.auditLog.findFirst({ where: { action: STORYBOARD_ART_CORRECTION_ACTION,
    targetType: 'hermes_research_run', targetId: run.id, metadata: { path: ['taskId'], equals: task.id } } })) return null;
  return { task, step, checkpoint, review, authorization };
}
...
export async function requireStoryboardArtCorrectionAuthorization(tx: Pick<Prisma.TransactionClient, 'auditLog' | 'hermesResearchRun'>, input: {
  ...
  return readStoryboardArtCorrectionAuthorization(tx, input, 'generating_storyboard');
}

async function readStoryboardArtCorrectionAuthorization(tx: Pick<Prisma.TransactionClient, 'auditLog' | 'hermesResearchRun'>,
  input: { taskId: string; actorId: string; workspaceId: string; payload: PresentationGenerationPayload;
    executionAttempt: number; retryCount: number; result: unknown }, expectedStatus: 'generating_storyboard' | 'stopped') {
  ...
  const meta = jsonRecord(receipt?.metadata);
  const resumes = runId ? await tx.auditLog.findMany({ where: { action: STORYBOARD_ART_RESUME_ACTION,
    targetType: 'hermes_research_run', targetId: runId,
    metadata: { path: ['taskId'], equals: input.taskId } }, take: 2 }) : [];
  const resume = resumes[0];
  const resumed = jsonRecord(resume?.metadata);
  const originalAttempt = meta.authorizedExecutionAttempt === input.executionAttempt && meta.authorizedRetryCount === input.retryCount;
  const resumedAttempt = resumes.length === 1 && resume?.actorId === input.actorId && resume.workspaceId === input.workspaceId
    && resumed.authorizationAuditId === receipt?.id && resumed.authorizationRequestDigest === meta.requestDigest
    && resumed.stepId === meta.stepId && resumed.resumedExecutionAttempt === input.executionAttempt
    && resumed.previousExecutionAttempt === meta.authorizedExecutionAttempt && resumed.previousExecutionAttempt === input.executionAttempt - 1
    && resumed.previousRetryCount === input.retryCount - 1 && resumed.resumedRetryCount === input.retryCount
    && typeof meta.authorizedExecutionAttempt === 'number' && input.executionAttempt > meta.authorizedExecutionAttempt
    && typeof meta.authorizedRetryCount === 'number'
    && input.retryCount - meta.authorizedRetryCount === input.executionAttempt - meta.authorizedExecutionAttempt
    && resumed.additionalAuthorizedOperations === 0 && resumed.newTaskCount === 0 && resumed.chargeableAttempts === 0
    && resumed.priorSubmission === 'none' && resumed.noScientificReplanning === true && resumed.noProviderSwitch === true
    && resumed.originalCandidateHash === meta.originalCandidateHash && resumed.originalReviewResponseHash === meta.originalReviewResponseHash
    && resumed.sourceEvidenceIdentity === meta.sourceEvidenceIdentity && resumed.narrativeSourceIdentity === meta.narrativeSourceIdentity
    && resumed.baseIdentity === meta.baseIdentity && isDeepStrictEqual(resumed.taskPayload, meta.taskPayload);
  ...
  if (!run || run.actorId !== input.actorId || run.researchObjectId !== input.payload.researchObjectId
    || run.versionId !== input.payload.versionId || run.profile !== VISUAL_NARRATIVE_PROFILE || run.status !== expectedStatus
    || !run.steps.some(step => step.id === meta.stepId && step.stage === 'storyboard' && step.ordinal === 0
      && step.status === (expectedStatus === 'stopped' ? 'stopped' : 'running') && step.agentTaskId === input.taskId && step.presentationAssetId === null)
    ...
    || (originalAttempt ? resumes.length !== 0 : !resumedAttempt)
    || typeof meta.authorizedExecutionAttempt !== 'number' || meta.previousExecutionAttempt !== meta.authorizedExecutionAttempt - 1
    || typeof meta.authorizedRetryCount !== 'number' || meta.previousRetryCount !== meta.authorizedRetryCount - 1
  ...
  return { id: receipt.id, createdAt: receipt.createdAt, requestDigest: meta.requestDigest, originalCandidateHash: review.candidateHash as string,
...
async function inspectGenerationRecovery(
  ...
  for (const step of sceneSteps) {
    ...
    } else if ((recoveryState === 'failed' || recoveryState === 'not_submitted' || recoveryState === 'usage_limited' || !inspectImageRecoveryState)
    ...
    try {
      const outcome = await deps.prisma.$transaction(async tx => {
        ...
        if (run.profile === VISUAL_NARRATIVE_PROFILE) {
          ...
          const sourceSupport = await inspectNarrativeSourceSupportReplan(tx, run, deps.storage).catch(() => null);
          const pixel = sourceSupport ? null : await inspectNarrativePixelReplan(tx, run, deps.inspectImageRecoveryState);
          const pixelRevision = sourceSupport || pixel ? null : await inspectNarrativePixelPlanRevision(tx, run);
          const pixelRecovery = sourceSupport ?? pixel ?? pixelRevision;
          if (pixelRecovery) {
            if (run.version !== input.expectedVersion) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Narrative pixel reviews changed; reload before replanning');
            if (planningReceipt) throw new HermesResearchRunError('IDEMPOTENCY_CONFLICT', 'Narrative retry key was already used');
            if (!deps.audit?.record) throw new HermesResearchRunError('SOURCE_NOT_READY', 'Narrative replanning audit is unavailable');
            const changed = await tx.hermesResearchRun.updateMany({ where: { id: run.id, actorId: input.actorId,
              status: run.status, error: run.error, version: input.expectedVersion, versionId: run.versionId,
              maxAgentTasks: pixelRecovery.previousMaxAgentTasks },
            data: { status: 'generating_storyboard', error: null, lastReconciledAt: null,
              maxAgentTasks: pixelRecovery.maxAgentTasks, version: { increment: 1 } } });
            if (changed.count !== 1) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Narrative pixel-feedback recovery changed');
            if (sourceSupport || pixel) for (const step of (sourceSupport ?? pixel)!.sceneSteps) {
              const stopped = await tx.hermesResearchStep.updateMany({ where: { id: step.id, runId: run.id, stage: 'scene_image',
                ordinal: step.ordinal, status: step.status, agentTaskId: step.agentTaskId, presentationAssetId: step.presentationAssetId, error: step.error },
              data: { status: 'stopped' } });
              if (stopped.count !== 1) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Terminal scene binding changed before replanning');
            }
            const next = { ...run, status: 'generating_storyboard', maxAgentTasks: pixelRecovery.maxAgentTasks };
            await createPresentationSteps(deps, tx, next, 'storyboard', [{ ordinal: 0, replacesTaskId: pixelRecovery.replacesTaskId,
              payload: pixelRecovery.payload }], undefined, pixelRecovery);
            const replacement = await tx.hermesResearchStep.findUniqueOrThrow({ where: { runId_stage_ordinal: { runId: run.id, stage: 'storyboard', ordinal: 0 } },
              include: { agentTask: true } });
            await recordAudit(deps, tx, { actorId: input.actorId, workspaceId: ro.workspaceId,
              action: 'hermes.research_run.generation_retry', targetType: 'hermes_research_run', targetId: run.id,
              metadata: { correction: sourceSupport || pixel ? NARRATIVE_PIXEL_REPLAN : NARRATIVE_PIXEL_PLAN_REVISION, requestDigest, clientIdempotencyKey: input.idempotencyKey,
                expectedVersion: input.expectedVersion, previousMaxAgentTasks: pixelRecovery.previousMaxAgentTasks,
                maxAgentTasks: pixelRecovery.maxAgentTasks, existingTaskCount: pixelRecovery.existingTaskCount,
                ...(sourceSupport ? { cause: STORYBOARD_SOURCE_SUPPORT_INVALID, sourceProof: sourceSupport.sourceProof,
                sceneSet: 'terminal', sceneSteps: sourceSupport.sceneSteps, previousStatus: run.status, previousError: run.error,
                parentStoryboardAssetId: sourceSupport.parent.parent.id, parentIdentity: sourceSupport.parent.parentIdentity,
                sourceIdentity: sourceSupport.sourceIdentity, sourceEvidenceIdentity: sourceSupport.parent.checkpoint.sourceEvidenceIdentity,
                claimContent: sourceSupport.parent.checkpoint.claimContent, narrativeSourceIdentity: sourceSupport.parent.checkpoint.narrativeSourceIdentity,
                maxNewImages: sourceSupport.sceneSteps.length, chargeableAttempts: 1 + sourceSupport.sceneSteps.length }
                  : pixel ? { sceneSet: 'terminal', sceneSteps: pixel.sceneSteps, previousStatus: run.status, previousError: run.error,
                parentStoryboardAssetId: pixel.source.parent.id, parentIdentity: pixel.source.parentIdentity,
                sourceIdentity: pixel.source.identity, sourceEvidenceIdentity: pixel.source.sourceEvidenceIdentity,
                claimContent: pixel.checkpoint.claimContent, narrativeSourceIdentity: pixel.checkpoint.narrativeSourceIdentity,
                sceneReviews: pixel.source.sceneReviews, anchorImageId: pixel.source.image.id,
                maxNewImages: pixel.source.sceneReviews.length, chargeableAttempts: 1 + pixel.source.sceneReviews.length }
                  : { previousReceiptId: pixelRevision!.pixel.receipt.id, rootReceiptId: pixelRevision!.pixel.rootReceipt.id,
                    failedStoryboardTaskId: pixelRevision!.pixel.task.id, failedTaskIdentity: pixelRevision!.failed.identity,
                    previousError: pixelRevision!.pixel.task.error, candidateHash: pixelRevision!.failed.review.candidateHash,
                    reviewResponseHash: pixelRevision!.failed.review.responseHash,
                    sourceEvidenceIdentity: pixelRevision!.pixel.metadata.sourceEvidenceIdentity,
                    claimContent: pixelRevision!.pixel.metadata.claimContent, narrativeSourceIdentity: pixelRevision!.pixel.metadata.narrativeSourceIdentity,
                    maxNewImages: 0, inheritedRemainingImages: pixelRevision!.pixel.sceneLimit, chargeableAttempts: 1 }),
                newTaskId: replacement.agentTaskId, taskPayload: replacement.agentTask!.payload,
                maxNewStoryboards: 1, newRunCount: 0,
                noProviderSwitch: true, publicationAuthorized: false } }, ctx);
            return { run: await tx.hermesResearchRun.findUniqueOrThrow({ where: { id: run.id }, include: RUN_INCLUDE }),
              dispatchIds: [replacement.agentTaskId!] };
          }
          const completedReviews = await inspectCompletedImageReviewRecovery(tx, run, deps.canResumeImageReviewFromCompletedResult);
          if (completedReviews) {
            if (run.version !== input.expectedVersion) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Image review recovery changed; reload before continuing');
            if (planningReceipt) throw new HermesResearchRunError('IDEMPOTENCY_CONFLICT', 'Image review retry key was already used');
            if (!deps.audit?.record) throw new HermesResearchRunError('SOURCE_NOT_READY', 'Image review recovery audit is unavailable');
            const changed = await tx.hermesResearchRun.updateMany({ where: { id: run.id, actorId: input.actorId,
              status: 'failed', error: run.error, version: input.expectedVersion, versionId: run.versionId, maxAgentTasks: run.maxAgentTasks },
            ...
            for (const { task, step, asset } of completedReviews) {
              ...
              if (taskChanged.count !== 1 || stepChanged.count !== 1)
                throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Completed image review task changed');
            }
            await recordAudit(deps, tx, { actorId: input.actorId, workspaceId: ro.workspaceId,
              action: 'hermes.research_run.generation_retry', targetType: 'hermes_research_run', targetId: run.id,
              metadata: { correction: COMPLETED_IMAGE_REVIEW_RECOVERY, requestDigest, clientIdempotencyKey: input.idempotencyKey,
            ...
            return { run: await tx.hermesResearchRun.findUniqueOrThrow({ where: { id: run.id }, include: RUN_INCLUDE }),
              dispatchIds: completedReviews.map(item => item.task.id) };
          }
          ...
          if (technical) {
            ...
            const changed = await tx.hermesResearchRun.updateMany({ where: { id: run.id, actorId: input.actorId,
              status: 'failed', error: run.error, version: input.expectedVersion, versionId: run.versionId, maxAgentTasks: run.maxAgentTasks },
            data: { status: 'generating_scene_images', maxAgentTasks: technical.maxAgentTasks,
            ...
            for (const scene of technical.recoverableFailures) {
              ...
              const bound = await tx.hermesResearchStep.updateMany({ where: { id: step.id, runId: run.id, stage: 'scene_image',
                ordinal: step.ordinal, agentTaskId: previous.id, status: step.status, error: step.error, presentationAssetId: step.presentationAssetId },
              data: { agentTaskId: task.id, presentationAssetId: reviewOnly ? task.id : null, status: 'running', error: null } });
              if (bound.count !== 1) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Technical replacement step changed');
              replacements.push({ sceneIndex: step.ordinal, previousTaskId: previous.id, newTaskId: task.id, mode: reviewOnly ? 'review_only' : 'render' });
            }
            await recordAudit(deps, tx, { actorId: input.actorId, workspaceId: ro.workspaceId,
              action: 'hermes.research_run.generation_retry', targetType: 'hermes_research_run', targetId: run.id,
            ...
            // Validate the complete receipt and all new bindings before committing any allowance or reservation.
            await readNarrativePixelReplanAuthority(tx, { runId: run.id, actorId: run.actorId });
            return { run: await tx.hermesResearchRun.findUniqueOrThrow({ where: { id: run.id }, include: RUN_INCLUDE }),
              dispatchIds: replacements.map(item => item.newTaskId) };
          }
          const savedReceipt = await tx.auditLog.findFirst({ where: {
            action: { in: ['hermes.research_run.storyboard_checkpoint_resume', 'hermes.research_run.storyboard_output_resume',
              STORYBOARD_ART_CORRECTION_ACTION, STORYBOARD_ART_RESUME_ACTION, HERMES_IMAGE_RENDER_RECOVERY_ACTION] },
            targetType: 'hermes_research_run', targetId: run.id, actorId: input.actorId,
            metadata: { path: ['clientIdempotencyKey'], equals: input.idempotencyKey },
          } });
          if (savedReceipt) {
          ...
          const render = await inspectNarrativeImageRenderRecovery(tx, run);
          if (render) {
            if (run.version !== input.expectedVersion) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Hermes run changed; reload before rendering correction');
            if (planningReceipt) throw new HermesResearchRunError('IDEMPOTENCY_CONFLICT', 'Rendering correction key was already used');
            if (!deps.audit?.record) throw new HermesResearchRunError('SOURCE_NOT_READY', 'Rendering correction requires durable audit storage');
            const fenced = await tx.hermesResearchRun.updateMany({ where: { id: run.id, actorId: input.actorId,
              version: input.expectedVersion, versionId: run.versionId, status: 'stopped', maxAgentTasks: render.previousMaxAgentTasks },
            data: { maxAgentTasks: { increment: 1 }, status: 'generating_scene_images', version: { increment: 1 }, error: null, lastReconciledAt: null } });
            if (fenced.count !== 1) throw new HermesResearchRunError('CONCURRENT_UPDATE', 'Rendering correction allowance changed');
            const extended = await tx.hermesResearchRun.findUniqueOrThrow({ where: { id: run.id }, include: RUN_INCLUDE });
            await createPresentationSteps(deps, tx, extended, 'scene_image', [{ ordinal: render.step.ordinal,
              replacesTaskId: render.source.task.id, payload: render.source.payload as unknown as Record<string, unknown> }], render);
            const step = await tx.hermesResearchStep.findUniqueOrThrow({ where: { id: render.step.id } });
            await recordAudit(deps, tx, { actorId: input.actorId, workspaceId: ro.workspaceId,
              action: HERMES_IMAGE_RENDER_RECOVERY_ACTION, targetType: 'hermes_research_run', targetId: run.id,
              metadata: { requestDigest, clientIdempotencyKey: input.idempotencyKey, expectedVersion: input.expectedVersion,
                previousMaxAgentTasks: render.previousMaxAgentTasks, maxAgentTasks: extended.maxAgentTasks,
                existingTaskCount: render.existingTaskCount, newTaskCount: 1, newRunCount: 0, chargeableAttempts: 1,
                scientificReplanning: false, stepId: step.id, sceneIndex: step.ordinal,
                previousTaskId: render.source.task.id, newTaskId: step.agentTaskId, taskPayload: render.source.payload,
                rejectedImageAssetId: render.source.image.id, rejectedImageContentHash: render.source.image.contentHash,
                reviewResponseHash: render.source.reviewHash, storyboardAssetId: render.source.parentAsset.id,
                storyboardContentHash: render.source.parent.contentHash, parentIdentity: render.source.parent.identity,
                sourceEvidenceIdentity: render.source.sourceEvidenceIdentity } }, ctx);
            return { run: await tx.hermesResearchRun.findUniqueOrThrow({ where: { id: run.id }, include: RUN_INCLUDE }), dispatchIds: [step.agentTaskId!] };
          }
          const art = await inspectStoryboardArtCorrection(tx, run) ?? await inspectStoryboardArtCorrection(tx, run, true);
          if (art) {
            ...
            const correction = { schemaVersion: 1, authorizationRequestDigest: art.authorization?.requestDigest ?? requestDigest,
            ...
            await recordAudit(deps, tx, { actorId: input.actorId, workspaceId: ro.workspaceId,
              action: art.authorization ? STORYBOARD_ART_RESUME_ACTION : STORYBOARD_ART_CORRECTION_ACTION,
              targetType: 'hermes_research_run', targetId: run.id,
              metadata: { requestDigest, clientIdempotencyKey: input.idempotencyKey,
                ...
                newTaskCount: 0, remainingImageTasks: 1, noScientificReplanning: true, noProviderSwitch: true,
                ...(art.authorization ? { authorizationAuditId: art.authorization.id,
                  authorizationRequestDigest: art.authorization.requestDigest,
                  resumedExecutionAttempt: art.task.executionAttempt + 1, resumedRetryCount: art.task.retryCount + 1,
                  additionalAuthorizedOperations: 0, chargeableAttempts: 0, priorSubmission: 'none' }
                  : { correction: STORYBOARD_ART_CORRECTION,
                    authorizedExecutionAttempt: art.task.executionAttempt + 1, authorizedRetryCount: art.task.retryCount + 1,
                    allowedOperations: ['illustration-art', 'illustration-plan-review'], chargeableAttempts: 2 }) } }, ctx);
            ...
            await recordAudit(deps, tx, { actorId: input.actorId, workspaceId: ro.workspaceId,
              ...
              metadata: { correction: STORYBOARD_PLANNING_RETRY, requestDigest, clientIdempotencyKey: input.idempotencyKey,
                ...
                ...(planning.planningFailureClass ? { planningFailureClass: planning.planningFailureClass,
                  ...
                  ...(planning.previousReceiptId ? { previousReceiptId: planning.previousReceiptId } : {}) }
                ...
                creditPolicy: planning.chargeableAttempts === 0 ? 'reuse-original-reservation-and-authorization;provider-usage-still-applies'
                  : 'reuse-original-reservation;explicit-planning-retry-incurs-provider-usage' } }, ctx);
            return { run: await tx.hermesResearchRun.findUniqueOrThrow({ where: { id: run.id }, include: RUN_INCLUDE }),
...
export async function requireHermesPresentationTaskAuthority(
  ...
  const pixelAuthority = run ? await readNarrativePixelReplanAuthority(prisma, { runId: run.id, actorId: input.actorId }) : null;
  const renderAuthority = run && !validGrant(run) && input.authority.stage === 'scene_image'
    ? await requireHermesImageRenderRecoveryAuthority(prisma, { runId: run.id, actorId: input.actorId,
      taskId: input.taskId, phase: 'generation' }).catch(() => null) : null;
  ...
  if (pixelAuthority && (input.authority.stage === 'storyboard'
    ? input.taskId !== pixelAuthority.task.id || !isDeepStrictEqual(input.payload, pixelAuthority.payload)
    : input.authority.stage !== 'scene_image' || input.authority.ordinal >= pixelAuthority.sceneLimit
      || input.payload.sceneImage?.storyboardAssetId !== pixelAuthority.task.id || input.payload.sceneImage?.revisionAssetId)) {
    throw new Error('[blocked] Narrative replacement is outside its receipt');
  }
  if (pixelAuthority?.technicalRecovery) {
    const checkpoint = jsonRecord(jsonRecord(pixelAuthority.task.result).storyboardCheckpoint);
    if (input.authority.stage !== 'scene_image' || await validateReviewedSources(prisma, pixelAuthority.run) !== 'ready'
      || !isDeepStrictEqual(checkpoint.payload, pixelAuthority.task.payload)
      || checkpoint.sourceEvidenceIdentity !== pixelAuthority.metadata.sourceEvidenceIdentity
      || !await readUnchangedNarrativeCheckpointEvidence(prisma, pixelAuthority.run, checkpoint))
      throw new Error('[blocked] Narrative technical recovery source changed');
  }
...
async function createPresentationSteps(
  ...
  renderRecovery?: NonNullable<Awaited<ReturnType<typeof inspectNarrativeImageRenderRecovery>>>,
  ...
  const renderAuthorized = renderRecovery?.[IMAGE_RENDER_ELIGIBILITY] === true && run.id === renderRecovery.runId
    ...
    && inputs[0]!.replacesTaskId === renderRecovery.source.task.id && isDeepStrictEqual(inputs[0]!.payload, renderRecovery.source.payload);
  const pixelAuthorized = pixelRecovery?.[PIXEL_REPLAN_ELIGIBILITY] === true && pixelRecovery.runId === run.id
    && run.maxAgentTasks === pixelRecovery.maxAgentTasks && run.status === 'generating_storyboard'
    && stage === 'storyboard' && inputs.length === 1 && inputs[0]!.replacesTaskId === pixelRecovery.replacesTaskId
    && inputs[0]!.ordinal === 0 && isDeepStrictEqual(inputs[0]!.payload, pixelRecovery.payload);
  const pixelAuthority = pixelAuthorized ? null : await readNarrativePixelReplanAuthority(tx, { runId: run.id, actorId: run.actorId });
  if (pixelAuthority?.technicalRecovery) throw new Error('Technical recovery does not authorize automatic tasks');
  if (!run.versionId || (!validGrant(run) && !renderAuthorized && !pixelAuthorized && !pixelAuthority)) {
    throw new Error('Hermes generation grant is invalid');
  }
  if (pixelAuthority && (stage !== 'scene_image' || inputs.length < 1 || inputs.length > pixelAuthority.sceneLimit
    || inputs.some((item, index) => item.ordinal !== index || item.replacesTaskId !== pixelAuthority.sceneReviews[index]?.taskId
      || !isDeepStrictEqual(item.payload, { schemaVersion: 1, researchObjectId: run.researchObjectId, versionId: run.versionId,
        kind: 'image', sourceClaimIds: run.sourceClaimIds, sceneImage: { storyboardAssetId: pixelAuthority.task.id, sceneIndex: index } })))) {
    throw new Error('Narrative replacement tasks exceed their explicit receipt');
  }
...
async function validateReviewedSources(tx: Prisma.TransactionClient, run: RunRow): Promise<'pending' | 'ready' | 'invalid'> {
  ...
  const imageTaskId = run.steps.find(step => step.stage === 'scene_image')?.agentTaskId;
  const renderAuthority = !validGrant(run) && imageTaskId
    ? await requireHermesImageRenderRecoveryAuthority(tx, { runId: run.id, actorId: run.actorId,
      taskId: imageTaskId, phase: 'current' }).catch(() => null) : null;
  if (!run.versionId || (!validGrant(run) && !renderAuthority && !pixelAuthority) || run.sourceClaimIds.length === 0) return 'invalid';
  const version = await tx.version.findUnique({ where: { id: run.versionId } });
...
async function repairRejectedNarrativeImages(deps: HermesResearchRunDeps, tx: Prisma.TransactionClient, run: RunRow): Promise<boolean> {
  if (run.profile !== VISUAL_NARRATIVE_PROFILE) return false;
  // This explicit receipt covers one new image per new scene; another failure needs its own user action.
  if (await readNarrativePixelReplanAuthority(tx, { runId: run.id, actorId: run.actorId })) return false;
      ...
      try {
        ...
        result = await deps.prisma.$transaction(async (tx) => {
          ...
          if (run.status === 'awaiting_storyboard_review') {
            ...
            if (pixelAuthority) await tx.hermesResearchStep.deleteMany({ where: { runId: run.id, stage: 'scene_image', ordinal: { gte: sceneCount } } });
            return moveRun(deps, tx, run, 'generating_scene_images', ro.workspaceId);
          }
