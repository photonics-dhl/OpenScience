const currentRenderer = 'docs/specs/2026-08-19-hermes-wanko-live2d-design.md';
const companionFoundation = 'docs/specs/2026-08-17-hermes-workspace-companion-motion-design.md';
...
export function validateHermesRendererIndex(indexText) {
  ...
  const companionFoundationRow = indexText.split(/\r?\n/).find((line) => line.includes(companionFoundation)) ?? '';
  ...
  if (!currentRow.includes('CURRENT Hermes visual/guide design')) issues.push(`D: Wanko 必须是唯一 CURRENT Hermes design -> ${currentRenderer}`);
  if (!companionFoundationRow.includes('IMPLEMENTED FOUNDATION / VISUAL SUPERSEDED')) issues.push(`D: workspace-companion foundation 必须保持 IMPLEMENTED FOUNDATION / VISUAL SUPERSEDED -> ${companionFoundation}`);
