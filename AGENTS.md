...
- `infra/`：`compose` 已含 dev/monitor/prod 三套栈；生产栈按 data_net/app_net 分段，2026-08-09 增加仅 data_net 可达的 SeaweedFS 4.41 S3 对象存储（ADR-007）；`migrations` 已含迁移 1–25（Prisma 格式，均含 rollback.sql）；沙箱表原始 SQL 留存于 `packages/database/migrations/`（13/14，已标 DEPRECATED，勿再手工执行）；`nginx` 已含 `portainer.conf` + `openscience.conf`（均部署云上，/admin basic_auth）；`scripts/deploy.sh` + `backup.sh`（P1A-9）；`sandbox/` 已含沙箱基础镜像 Dockerfile + 构建/测试脚本 + README（P1E-3，Python 3.11-slim + 科学计算库 + 非 root）。
