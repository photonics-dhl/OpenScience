...
exec /opt/scansci-browsers/chromium-1234/chrome-linux64/chrome --user-data-dir=/profile/chromium --no-first-run --no-default-browser-check \
  --remote-debugging-address=127.0.0.1 --remote-debugging-port=9233 --proxy-server=http://127.0.0.1:7891 \
  --disable-quic --disable-background-networking --disable-component-update --disable-default-apps --disable-sync \
  --disable-features=AutofillServerCommunication,OptimizationHints,MediaRouter --window-size=1440,960 https://chatgpt.com/
