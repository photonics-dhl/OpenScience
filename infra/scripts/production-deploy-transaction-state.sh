...
transaction_verify_already_active_release() {
  ...
  if [ "$EMBEDDING_DEPLOY" -eq 0 ]; then
    ...
    run_remote "set -euo pipefail; cd '$RELEASE_ROOT'; services=\$(XGS_RELEASE_ROOT='$RELEASE_ROOT' XGS_RELEASE_IMAGE_TAG='$RELEASE_SHA' SCANSCI_BROWSER_REQUIREMENTS_SHA256='$SCANSCI_BROWSER_REQUIREMENTS_SHA256_VALUE' docker compose --project-directory '$RELEASE_ROOT' --profile embedding --env-file '$PROD_ENV' -f '$COMPOSE_FILE' ps --status running --services); if printf '%s\n' \"\$services\" | grep -qx embedding-worker; then exit 1; fi"
  fi
}
