...
- [x] **Step 8: Create component families**
  ...
  Completion evidence (2026-08-08): Version Diff (`86:47`, 3 kinds × 2 contexts) and Hermes Rail (`86:106`, 4 modes × 2 contexts) are present under wrapper `86:7`. Full `02 Components` audit reports 12 expected sets, 96 variants, zero missing/duplicate set names, zero image fills and zero placeholders. Legacy Button specimen node `29:8` was corrected from an orange `ro-node-1` to a blue `evidence-node`; all remaining accent-diff owners are diff/change nodes. Screenshot: `docs/design-assets/figma/2026-08-08-step8-change-hermes-v1.png`. The user explicitly confirmed the final batch; Step 8 is complete.
- [ ] **Step 9: Prototype the six screens**
  ...
  Migration gate passed (2026-08-08): the user copied the latest design into long-account file `gjhowMG7cG4clKwvhvF08E`. `figma-primary` confirms the long account now has a Full seat and net-zero write access. Source/copy parity passed for 6 pages, 4 collections / 85 variables, 9 text styles, 2 effect styles, 12 component sets / 96 variants, master nodes `60:2` / `60:7` / `60:8`, and prototype 0 reactions / 0 flow starts. Start Step 9 only in this canonical file; the transitional source and empty probe are read-only retained evidence. Code Connect parity is a separate Step 11 gate because the current student tier is rejected by the official API.
  First checkpoint (2026-08-08, user-approved): canonical `04 Screens` contains batch wrapper `2009:3`, Landing `2010:2`, Auth/Create `2012:4`, and six-state annotation rail `2009:5`. Landing is an exact clone of approved master `60:7`; Auth/Create implements invite identity, material-first RO creation, PAPER/DATA/CODE provenance, provisional stable ID, Hermes/blank-six-field choice, processing-scope confirmation and waitlist fallback. Audit: 1440×900 each, zero image fills/gradients/Auth orange/semantic overflow, approved fonts only, Button/Input instances reused, and all actions at least 44px. Local evidence: `docs/design-assets/figma/2026-08-08-step9-entry-{landing,auth-create,states}-v1.png`. The user explicitly confirmed this checkpoint; Dashboard/RO Workspace is unlocked. Public RO/Collection, clickable destinations and full Step 13 review remain pending; do not mark Step 9 complete yet.
  Second checkpoint (2026-08-09, user-approved): canonical `04 Screens` contains top-level Section `2057:99`, Dashboard `2031:10`, RO Workspace `2031:11`, and cockpit state/interaction rail `2031:12`. Dashboard is action-first and excludes social noise; Workspace preserves one RO ID/version/Hermes session across SDF outline, evidence editor, version diff, review and permission/provenance rails. Final audit: both screens are 1440×900, zero image fills/gradients/semantic overflow/structural overlaps, approved fonts only, all named actions at least 44px, and the sole orange semantic owner is Version Diff `2045:85`. Local evidence: `docs/design-assets/figma/2026-08-09-step9-{dashboard,ro-workspace,cockpit-states}-v1.png`. Two real routes are verified: Auth/Create `2020:10` → Dashboard and Dashboard `2036:17` → Workspace. Landing `2010:24` → Auth/Create remains open because the approved entry screens are nested under a Frame wrapper and Figma rejected that NAVIGATE destination (`e2c24bcb-14d1-42a0-abaf-eae1e7caeab8`); do not retry blindly or restructure the approved batch without review. Public RO/Collection are delivered by the third checkpoint below; Landing route repair and final flow audit still keep Step 9 unchecked.
  Third checkpoint (2026-08-09, user-approved): canonical `04 Screens` contains Section `2069:99`, Public RO `2069:100`, Ultrafast Science Collection `2069:101`, and public-scholarship state/interaction rail `2069:102`. Public RO combines the approved paper master with stable identity, version citation, license and media provenance; Collection preserves the same RO while adding journal hierarchy, editorial note, SDF/license/media readiness and scoped Editorial Curator ownership. Final audit: 1440×900 / 86 nodes / 6 instances and 1440×900 / 71 nodes / 4 instances, zero image fills/gradients, approved fonts only and all named actions at least 44px. Three public routes are real: Dashboard → Public RO, Public RO → Collection and Collection → Public RO. Local evidence: `docs/design-assets/figma/2026-08-09-step9-{public-ro,ultrafast-collection,public-states}-v1.png`. Landing → Auth/Create remains a later Figma structure repair; it does not block the web implementation slice.
...
- [ ] **Step 11: Configure Code Connect**
  ...
  Plan gate (2026-08-08): both source and canonical `get_code_connect_map` calls are rejected by Figma with an Organization/Enterprise Dev-or-Full-seat requirement. The long account is Full on student tier, so this step remains blocked until the plan requirement is satisfied or ADR-004 approves a non-Code-Connect handoff.
...
> **Current implementation slice (2026-08-09):** user-approved Figma baselines are now being mapped into the existing Next.js surfaces. First delivery is Landing evidence field + Dashboard cockpit + RO Workspace chrome, with TDD, reduced-motion and docs-sync gates.
...
- [x] **Step 1: Write failing Dashboard tests**
  2026-08-09 首个网页切片：`apps/web/test/dashboard-cockpit.test.tsx` 先验证 action-first 结构、稳定 RO ID 与装饰粒子门禁。
