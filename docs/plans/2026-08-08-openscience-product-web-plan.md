...
- [ ] **Step 9: Prototype the six screens**
  ...
  Migration gate (2026-08-08): do not start Step 9 in the transitional file. A target file was successfully created through `figma-primary`, proving long-account write access, but the long account still lacks edit access to the transitional source and the official MCP exposes no share/duplicate/move/ownership-transfer operation. Resume only after the source is shared to the long account as Editor and duplicated or moved into its target Team, then pass the ADR-004 parity gate.
