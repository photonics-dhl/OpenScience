...
1. 多风格配图沿用上游文献科学分析、经审核关系及现有Gateway/Chat链路；已安装的设计Skill负责适合图像用途的构图、字体和材质，不能将单篇图的对象或布局固化为通用模板。
2. 本批是学术机制图、编辑封面插画、淡彩手绘；水墨作为已有艺术指导能力保留。按CURRENT中的用户认可及拒绝结果完成缺项，保留已认可图，不以重复水彩微调代替其他风格交付，也不只换颜色。
3. 能力复用与故障处理服务上述交付，已处理问题不得默认重新启动；未解决断点见 [能力台账](../runbooks/hermes-capability-registry.md#当前技术债与处理)。科学方案通过、成图生成、用户认可分别判断。本批质量未确认前，Figma、视频、第三篇和批量不启动。
