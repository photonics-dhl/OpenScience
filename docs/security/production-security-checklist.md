...
- [x] 共享 100 MiB 不再只是 post-check：每个 POSIX acquisition subprocess 在 import/call upstream 前设置并复验 soft+hard `RLIMIT_FSIZE=104857600`，SIGXFSZ/EFBIG 稳定失败并清 partial，external child 继承。成功只接受 exact regular source temp，并以同目录 rename 完成，无第二份 100 MiB copy。
...
- [x] 本地门禁：build/typecheck/integration compilation/lint/test/docs lint/docs-sync/diff；完整 test `2112 pass / 21 platform skips / 0 fail`。Task 9 latest fix `36f985c`；Linux CI 执行实际 RLIMIT/child 继承用例。
