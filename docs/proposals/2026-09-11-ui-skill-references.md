...
## 2026-09-11 项目安装与本次实际采用
- 用户已明确授权，四项设计Skill已安装到本工作树 `.agents/skills/` 并纳入版本管理：`apple-design`、`emil-design-eng`、`design-artifact`、`html-prototype`。每项保留上游文件、引用、`SOURCE.md`和MIT许可；来源分别为emilkowalski/skills@d23d7f88a2e21c9e4b1418c7abe420f5c1052ba7、plannotator/effective-html@d95debbaef15af1d201fc6c10c77cf92b524a0d6。
- 实施统一从项目frontend-design入口读取相关规则，不叠加多个审美总规范。采用稳定媒体比例、单一内容网格、正文与工具层次、克制即时反馈；保留用户批准色板，不新增动画或UI运行库。下个回合可自动发现新增skills，本轮已显式读取并使用。
- 图与视频上移到六字段之前；单图优先总结性概念图、多图用共享HTML幻灯片，不把附件/内部记录放在媒体前。SVG沿用原安全下载路径，不伪装可内嵌图片；私有状态不能进入公开读者页。
- 新版代码、部署与实际科学产物的状态只看CURRENT handoff，不以下方或本文件历史release作为生产事实。
