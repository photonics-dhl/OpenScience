...
## 2026-08-11（Landing 原型形变纠偏）— ✅ 已实现，待生产发布

- **根因**：原实现把粒子核心作为跟随指针的椭圆光团，且静止光轴在视口 50% 中线；用户原型实际是固定的垂直半色调狭缝，指针只对狭缝周围点阵产生局部径向位移，光轴落在 `Science` 与 `evolves.` 的交界附近。
- **实现**：桌面恢复单轴横排标题；Canvas 核心点阵固定于狭缝并按纵向基线渐隐，指针只改变局部偏移；文字底层在焦点区让位给 SVG 局部位移层，避免整句重影；移动端仍保持两行可读布局。
- **验收**：focused 20/20、Web 159/159、typecheck、production build 通过；隔离端口 3040 的 Playwright 1440/1920/390 normal/reduced/Open RO 截图门禁通过；额外浏览器截图确认鼠标位于词间交界时呈现局部狭缝形变。
- **下一步**：提交后执行 ECS 备份、部署和线上三视口截图复验；更新最终 release ref。
