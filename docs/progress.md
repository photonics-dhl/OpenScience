...
- 首篇v3已公开。第二篇最新4图1通过/2科学拒收/1渲染拒收，几何与条件分组提示已部署并由原页面提交新科学方案。第三篇技术子集恢复及队列/执行时限分离已High静态PASS并正常服务器构建上线，原页面202仅提交3生图+1已有PNG审阅，保留已审图和原科学方案。第三篇子集已全部终态，2图通过/2科学拒收/1审阅未提交失败，原入口已支持后续科学返工；第二篇科学规划成功后art超时，精确同task恢复与正文fill仅留前缀的同draft修复均High PASS并部署；原页面两次202已续作第二篇同task规划与第三篇新科学方案。第二篇已存完整规划但末审超时，第三篇原图下载失败并出现Chat对话访问限流，暂停追加Chat请求，保留原会话/检查点；已知科学绑定错误不发布。后两篇reader/新公开版本尚未完成；精确状态和证据见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。未测试/预检/CI/本机构建。
