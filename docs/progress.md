...
- 首篇v3已公开。后两篇新稿M3 accepted、六张PNG已保存；六次6Pro审阅均提交前技术失败，另两张出图结果unknown。真实target记录确证image/review跨broker共用三个页面。自有页面隔离和正式issues到art的接线候选High通过；混合unknown场景中只恢复已有PNG审阅的Domain候选也已High GO，均未交付。原PDF/六维/旧图保留，不重发unknown。用户允许换账号，但尚未找到第二份可直接切换登录，当前登录保留。后两篇reader/新公开版本及最终用户认可未完成，已存图仍有科学/叙事问题，不以accepted代替质量。精确状态和证据见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。未测试/预检/CI/本机构建。
