...
- Task 6 Task 2 search migration 2 已完成本地 expand-only TDD：五表 tenant scope、locator/claim/lexical metadata、database-owned `tsvector`+GIN、1024-dimension/4096-byte/vector hash+norm 约束、复合 workspace/chunk FK、content-free query telemetry 与逆序 rollback；未引入 pgvector/extension，core `DATABASE_URL` 不进入 schema。Search `5/5`、Prisma generate/build/typecheck GREEN；尚待 exact SHA 在 ECS disposable search DB 做 1→2、rollback→1、redeploy→2，生产仍为 search `1/1`。
