...
- 第一、二篇均出现标签过密，且第二篇初稿把数字放在机制解释之前；原配图 skill v14 只补“机制配文先讲关系、显式标签预算须核对、必要的独立工况拆幕”到 science/review。定向加载测试 6/6、Worker typecheck、独立 High GO、[CI 36175932790](https://github.com/photonics-dhl/OpenScience/actions/runs/36175932790) success 后曾部署 d296。两幕 auto 真任务 `3909a349` 结构化失败、无方案/PNG；原规划器多幕 auto 输出提醒经 automatic-style 12/12、Worker typecheck、独立 High GO、[CI 36178400376](https://github.com/photonics-dhl/OpenScience/actions/runs/36178400376) success，生产 `909cc3e3` 部署、回退 d296。独立 `4660140f` 成功保存两幕 auto draft，但固定算例/参数扫描混用、跨幕比宽 NO-GO。`117fe82c` 与 `1cd14e06` 科学阶段仅 thinking、16k/32k 耗尽，无资产；聚焦单电子机制＋独立电子团结果的 `0b5e5db7` 完成两幕 draft，原末审 accepted 却漏检：19 as、99 as、270 都未绑各幕结果原文，且要求生图模型画钟形曲线。独立 High 科学审查 NO-GO，未批准/生图。原规划器的新方案数值＋单位/变量必须出现在同一 subject 及其原文的校验已覆盖场景/总标题和中文邻接数值，automatic-style 36/36、Worker typecheck、独立 High GO、CI 36186477404 success，生产 72d 部署。真实 c071/8a36 两次 science 三次拒收同一 19 as 绑定，均无资产；中文单位/图号词法与缺失层级诊断 39/39、独立 High GO、CI 36189783888 success，生产 08d 部署；真实 0386efbe 拒收 19_as_source，单幕 a8c04e9d 拒收 1_bare_description，均无新 PNG。私有拒收摘要沿 Gateway 回调写 AuditLog，只存数字/字段/合法来源编号及哈希，42/42、typecheck、独立 High、CI 36193303110 成功，生产 a737 部署。真任务 1d1ec917 收据证实 19 as 已绑结果原文，而配文/encoding/标签的裸 0/1 被误作结果，仍无新图；新窄候选只识别显式画面编号，裸 0/1 仍校验，43/43、typecheck、独立 High GO，CI/部署/真任务待做。普通修订 science 无检查点的缺口记[能力台账](runbooks/hermes-capability-registry.md)。
