...
- 2026-09-23 用户明确必要测试/CI须运行，避免无关的过度测试；交付树与根入口 AGENTS.md 已纠偏。两个手绘上游技能筛选为项目内[匹配](../.agents/skills/openscience-handdraw-router/SKILL.md)与[表现](../.agents/skills/openscience-handdraw-style/SKILL.md)指导，接原艺术规划与图像提示，不重做Hermes全文分析或让用户选中间风格。定向新技能阶段与非目标风格测试5/5、依赖重建及worker类型检查通过；既有分镜测试有与本改动无关的旧fixture/流程断言失败，独立High指出的非目标风格串扰已修；定向CI 35832552646 已通过，旧分镜测试债务与本次改动分开登记。新技能尚未部署或获真实图像效果证据，精确状态见 [CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。
