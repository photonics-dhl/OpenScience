...
- 结果：磁盘 112G/80% → **56G/40%**（可用 30G → 86G）；11 个生产容器 healthy、公网 `/__release` 200。未决：netdata 监控卷 2.37G、Serena build 1.1G、chatgpt-browser jobs 166M（受授权流程管辖）、`/var/backups` 333M（KEEP）。详见 [deployment runbook](runbooks/deployment.md)。
