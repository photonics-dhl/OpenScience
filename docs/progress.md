...
- 自有页面隔离、正式 issues 到 art、仅恢复已有 PNG 审阅及30秒事务修复已交付。显式选Pro、额度拒绝分类、P2034有界重试及最多第二段review-only续审获独立High静态GO；保留期刊线上代码的集成候选a6a27ef5已推送为draft PR #110，Gateway已在服务器构建、独立浏览器provider已安装，应用尚未部署或实际观察；准确状态见 [Hermes CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。
- 用户允许更换账号；9月11日所谓第二账号已是当前登录，未发现另一份可直接切换凭据，当前Pro额度受限。后两篇 reader、新公开版本及最终用户认可仍未完成。
