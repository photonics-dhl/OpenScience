...
- 首篇v3已公开。后两篇新稿M3 accepted、六张PNG已保存；六次6Pro审阅均提交前技术失败，另两张出图结果unknown。真实target记录确证image/review跨broker共用三个页面。页面隔离、正式issues到art及混合unknown中只恢复PNG审阅均High通过并部署。原GET已提供各3项续审；第三篇按钮500/P2028，默认5秒事务在5.574秒过期、完全回滚，未新建task；第二篇未提交。候选仅将原Serializable事务期限设为既有30秒边界，全部资格复验和P2034-only重试保持，High GO，待正常交付。原PDF/六维/旧图保留，不重发unknown。用户允许换账号，但尚未找到第二份可直接切换登录，当前登录保留。后两篇reader/新公开版本及最终用户认可未完成，已存图仍有科学/叙事问题，不以accepted代替质量。精确状态和证据见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。未测试/预检/CI/本机构建。
