...
## 2026-08-09（生产故障修复）— 验证码申请持续报错

- **✅ 根因**：生产活动 Nginx 配置的 API `proxy_pass` 缺少尾斜杠，未剥离浏览器 `/api/` 前缀；Fastify 在匹配 `/auth/*` 豁免前执行 CSRF，最终返回 `CSRF_INVALID`。仓库配置正确，但 `deploy.sh` 仅在目标不存在时复制，造成配置漂移。
- **✅ 修复**：生产部署改为每次 `install` 对账 Nginx 配置；服务器已通过 `nginx -t` 并 reload。旧 `invited` 账户现在也能申请新 signup challenge，并在确认时原位升级为 verified 用户，不创建重复账户。
- **✅ 真实验证**：公网 `request-signup-code` 使用测试邮箱返回 HTTP 202，API 请求耗时约 832ms 且无邮件投递错误；活动 `nginx -T` 已显示带尾斜杠代理。Auth 21/21、routing contract 7/7、Web 86/86、全仓 build 通过。
- **⏳ 下一步**：用户使用最新邮件中的六位验证码完成确认注册；验证码本身不得写入日志、文档或交接。
...
## 2026-08-09（产品闭环 Task 5 生产部署）— 验证码注册已上线

- **✅ 部署**：代码同步至 ECS；因 workspace 并行构建存在 Prisma generate 竞态，改为 database → auth → api → web 串行构建后成功；生产栈已 `--force-recreate`。
- **✅ 迁移**：API 容器内应用 `20260809020000_signup_challenges`，随后 status 报告 23 migrations、schema up to date。
- **✅ 公网验收**：`/register`、`/verify-email`、`/dashboard` 均 HTTP 200；无 CSRF 的 signup 请求被 403 拦截，说明写接口安全门仍生效。
- **⏳ 下一步**：用户在邮箱收到验证码后完成真实两步注册；再根据截图做视觉微调，继续保留旧账号验证入口作为迁移兼容路径。
...
## 2026-08-09（公网验证恢复）— 验证码已重发

- **✅ 服务器动作**：针对已建立但仍处于 `invited` 的测试账户，公网 `/auth/resend-code` 返回 202；验证码已重新发送，未输出验证码内容。
- **⏳ 待用户动作**：在 `/verify-email` 输入收件箱中的最新六位验证码；验证后继续服务器侧登录、创建 RO 和 Workspace 验收。
