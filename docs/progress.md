...
## 2026-09-21 — 真实路径验证并修复流程找回/重复收费入口，审校及新图组仍阻塞
- 本次真实验证：旧公开三图/PDF可读，入口曾丢run并可能重复收费；同来源找回、重复create与公共refresh保护经High及正常服务器构建部署，普通入口回到原run、材料页回进度、零POST；只读DB确认原run/source/单一流程不变、无新增模型。自动链路与grant约束补迁移已部署，真实站内原请求恢复HTTP202、刷新同run零POST；六维composition成功，随后来源审校主请求300秒超时、备用凭据401，未得到有效审校/新图。最终审校65k/600s、primary-only和服务错误分类已部署，经High静态增量复核、服务器构建/启动及release读回；页面刷新原run无POST，部署未自动重发，历史超时是否计费未知，不新增复杂恢复API或伪造通过。未测试/预检/CI/本机构建；用户随后已授权一次补审；严格同run恢复经High收口、正常服务器构建部署，原页面点击一次HTTP202，新审校41c4c57c仅主请求300638ms/provider_error后结束，未切备用；只读定位Node底层headers/body默认300秒与上层600秒断接，原cause丢失；请求级时限及白名单错误码经High审查和服务器构建部署，06:00Z刷新同run/POST0，未再付费验证长响应，保留六维初稿/原失败及9项总额度，尚无新图。随后原地升级科学Skill v3，区分综合与核源、修复自动主张要求和纯润色修订冲突，经独立High及服务器构建部署；原任务保持失败，无新增模型，质量/延迟未实测；版本、证据及质量差额见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。
