...
- 自动链路与grant约束补迁移已部署，真实站内原请求恢复HTTP202、刷新同run零POST；六维composition成功，随后来源审校主请求300秒超时、备用凭据401，未得到有效审校/新图。现补最终审校65k/600s、primary-only和服务错误分类，经High静态增量复核通过；部署修正不自动重发，历史超时是否计费未知，不新增复杂恢复API或伪造通过。未测试/预检/CI/本机构建；版本、证据、待付费决策及质量差额见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。
