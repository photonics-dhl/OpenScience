...
- **书面设计已更新**：原地重写 `docs/design-assets/prompts/2026-08-08-living-research-observatory-v1.md` 为 v2 待复核稿，包含完整正向/负向 prompt、四区布局、原生层清单、静态验收门、视频边界和新输出路径。本步骤未调用 API。
- **⏳ 下一步**：用户复核 v2 书面设计；批准后再更新执行计划并进行一次新的中国区 `image-01` 生成。
