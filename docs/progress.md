...
- 首篇v3已公开。新版恢复与Skill v9已部署并两篇各一次原页面202续作；第二篇已真实消费v9、保存science及完整方案，但末审300秒timeout后缺恢复入口；第三篇science600秒timeout，无新science。旧超时请求已终态；最新续作在途，无新图/发布。通用已存方案末审续接及叙事要点数对齐原有四项能力均已独立High静态通过、正常部署，后两篇原入口各202续作一次，末审/规划在途；恢复保留原方案与未知调用，结构拒收使用实际反馈；已有草稿仍有科学问题，不把Skill消费/阶段保存称为质量提升。后两篇reader/新公开版本未完成；精确状态和证据见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。未测试/预检/CI/本机构建。
