...
- 用户最新授权一次额外方案执行及原余图，要求全链路畅通正确。已审全文与私有版本保留；长度/传输修复已部署。当前补接原task的一次显式收费planning重试，逻辑额度不变、不新增迁移，不重分析论文；High设计及增量实现审查PASS，部署/实际链路待续。真实图片与阅读质量未完成，状态证据只见[CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。未测试/预检/CI/本机构建。
