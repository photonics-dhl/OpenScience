...
- 自有页面隔离、正式 issues 到 art、仅恢复已有 PNG 审阅及30秒事务修复已交付。显式选Pro、额度拒绝分类、P2034有界重试及最多第二段review-only续审已随合并期刊线上代码的158ee13f正常服务器构建/启动；Chat provider另为a6a27ef5，生产回退e05。功能测试按用户要求跳过，两篇实际恢复与图片质量尚未观察。用户已改选5.6Sol审图，六张PNG的本机咨询均指出不能直接公开；产品内5.6Sol审图尚未接线，准确状态见 [Hermes CURRENT](handoff/2026-09-10-hermes-web-image-handoff.md)。
