...
状态：用户已认可一站式AI助手方案，尚未实施；最新要求先准备服务器论文分析、科学写作/撰稿、精美输出、文档/公式能力，按项推进。具体顺序与新鲜服务器事实见 `docs/runbooks/hermes-capability-registry.md` 和 `server-capabilities.md`。最初方案基于源码 HEAD `4b2dda83` 定向阅读；后续只读盘点已确认生产 `363257aa`，沿用rollback `c0fdc389`。本阶段只整理文档，无产品代码改动、部署、媒体生成或测试。以下目标覆盖旧创建页强制“先建立结构”的设计，不改变已认可的成稿阅读顺序。
