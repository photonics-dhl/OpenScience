...
2026-09-12第2项落实约定：分段阅读按每条观察保存原始来源、限定来源、算例及reported/synthesis/uncertain身份，由程序分配观察编号。全文整合只引用已存在观察，程序回填支持与限定原段，同时保留未被候选选中的假设、定义、限制和不确定材料；不通过静默删ID掩盖错引。科学自省只按理论/实验/仿真实际适用项工作，不再每阶段重复注入整份流程指令；沿用已有独立网页复核与发布权限，不新增整篇人工公式审批要求。候选/部署事实见CURRENT handoff。
