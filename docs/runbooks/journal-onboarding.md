...
实现依据：[期刊入驻 PRD](../specs/2026-09-15-journal-onboarding-design.md)。既有期刊入驻已部署；本手册同时描述来源矩阵、加工顺序和服务额度增强功能。增强功能的准确部署状态、上线版本、观察边界及真实试点后续统一见 [CURRENT handoff](../handoff/2026-09-15-journal-onboarding-handoff.md)。
...
2. 使用既有迁移 CLI 应用核心迁移 `20260915000000_journals`。生产基线已经包含其他后续迁移，按实际迁移账本核对，不用旧分支的迁移数量判断线上状态。本次三项增强不新增迁移；独立 Search 数据库无期刊新增迁移。
