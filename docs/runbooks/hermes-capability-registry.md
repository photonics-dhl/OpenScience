...
拒收证据保存缺口：planner仅在完整计划通过后存checkpoint，5226拒收正文未持久化；Gateway已有onRejectedCandidate同步回调，未来沿原task私有证据路径收口，不为此失败另设恢复槽或用手工截字放行。用户已明确授权一次额外方案执行与原余图。当前补接原retry-generation：对仅text/无候选的结构化失败，复用原task的一次显式执行及原图片额度；源/权限/CAS/审计与worker收据严格复验，不新增14额度、迁移或模型环节。独立High审查/正常部署完成，原页面一次继续202，同任务进入第二次执行；实际science/art两次成功并保存checkpoint，新故障为方案文本审阅误用Chat60KiB上限。候选沿用既有100000文本输入边界，仅该text分支变化；Chat/image仍60KiB，来源投影/原文/候选不裁。同task原checkpoint已真实接到末审、不重新规划；新末审仅因美术布局新增labels外文字blocked，未生图。根因是art把title/encoding背景与标题参考当作额外画面文字；候选仅在原art prompt明确labels为完整可见文字表、title改名readerTitle，保持科学字段/原review/预算，不增加状态分支或恢复槽。增量High PASS并已部署，旧blocked保持。既有generateIllustrationStoryboard art分支可逐字复用科学base，但requireStoryboardRevisionTask读取的失败checkpoint尚未接到该分支，当前exec3/余1图的原恢复资格也不放行新请求；用户2026-09-22已授权不限额度完成任务；同任务art-only接线已部署并由原页202启动，不消耗图片槽、不新造分析模块、不跳过完整复审；首个私有marker保存发生Serializable冲突，新增模型调用0。正补仅DB事务P2034退避与唯一原授权预提交恢复，已有marker的未知结果不自动重发。后续授权/质量见CURRENT。
