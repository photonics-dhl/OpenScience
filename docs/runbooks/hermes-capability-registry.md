...
来源终审必需Claims（2026-09-21）：d35真实消费科学Skill v3、六字段accepted且无补证，但缺已审Claims；旧guard仅校验六字段、后置物化可丢弃建议且无原响应。相同materializer已接入必需Claims guard及原一次结构修正，accepted-only恢复已部署。后继a6两次主M3成功仍schema拒收（首claims_source_unmaterializable），无科学否定；原稿保留。只读SourceMap证明selectedPassageRanges跨P合并造成来源关系无法单独绑定，候选改为v5按P边界分段、guard/budget/final共用，保留64段/24000字符和精确覆盖/关系冲突拒绝。Gateway opt-in拒收回调仅为有身份、结构合格而物化失败的JSON保留私有收据，绑定来源/候选/模型/响应与序列化候选指纹；不上日志/公开内容、不自动批准。source-review-recovery新增唯一accepted锚点紧接一次schema耗尽的明确分类，独立记录成功审计并复验原稿/连续谱系/CAS/原9项预算；不把成功调用记成服务故障。已High增量静态复核与正常服务器构建/启动，原页面续接后f2f成功产出完整六维及6条已审Claims，已自动物化新私有版本。后续分镜已生成并保留checkpoint，但review输入94011>61440：背景正文18000因重复locator/hash元数据膨胀到55982；全部Claim原文13246无重复。模型输入定位投影已High审查/部署，保留完整科学文字与server原始身份；真实输入60406<61440，原run单次202恢复同checkpoint终审；该首次终审73.827秒耗尽16384输出tokens，text0/thinking1，无review保存。已定位Gateway无正文length异常绕过既有escalateMaxTokens=32768：已统一截断分支并真实沿原task完成升级及终审，不重规划/增任务额度/切provider。真实分镜revised后已生图，Chat 6Pro schema3实际像素审阅blocked/repairInstruction:null：曲线科学对象与构图矛盾、分界位置违背前提及关键标签/转换缺失。旧revised直接终态只校验可改字段/长度，未证明冻结科学字段与新构图一致。已在原终审接入修订后完整候选的一次验收，先持久化提交/结果，未知响应不重发；科学字段漂移拒绝。已部署原run一次9→11科学返工，基于真实拒图及同版本来源重新规划，并在每次付费前复核身份；系统批准要求新分镜accepted。原页grant/继续成功，候选新任务正在运行，尚无合格成图，不能仅重画或写字段字符串特判。真实质量/任务身份见CURRENT；完整图文链路尚未交付。旧Worker信号仅停止journal计时器，主poller永不退出造成正式发布等待600秒；已将主循环停止领取/在途排空和资源关闭收口到原入口，含初始化失败与API关闭，High静态PASS并正常构建部署；新实现的关闭行为未实际观察，不通过停机演练验证。后继科学返工另发现旧previousArt仍按索引传入art、拒收反馈仅进入science，导致新符号编码重新混入旧曲线/穹顶；候选现仅科学场景和顺序冻结的art-only修订继承旧构图，科学返工依据新intent和拒收摘要重设美术，不复制旧索引。该小修High静态PASS并部署；00fa最后分镜已从原页面显式续作，完整科学/图像效果仍待实际结果，不修改历史候选。
