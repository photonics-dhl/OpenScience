...
2026-09-24 逐图质量反馈：用户明确称赞第三篇 scene0 淡彩概念图，同时指出其他图仍有问题。`installed-media-skills.ts` 按选定 style 只加载相应风格段落，不等于整个 Baoyu 库逐图执行；已有真实公开结果覆盖学术、编辑、淡彩，这三类不是风格白名单。当前薄弱处是把来源支持的科学关系编码成未读者能一眼理解且有美感的画面：两张新私有 PNG 被正式像素审图 blocked；新科学方案 1d154c10 虽被末审 accepted，却把点积误写为向量同向、将空间波峰间距对应频率，人工拦下且未生图。真实 d22567ab 的标签与科学关系获正式像素 accepted，但人工实看多箭头/重复公式仍像板书，私有 NO-GO；简化方案 92548f87 的淡彩 PNG 59b861b5 仍因逻辑关联被画成发射箭头而正式 blocked。其网页审图同次提交由 grace 窗口无重发恢复并在原产品任务记为 blocked；桥接固定 30 秒 URL 等待是此次稳定性缺口，修复候选待发布实测。当前[研发观察台](../proposals/2026-09-23-hermes-development-live.html)展示逐图能力/效果；技能、自动审阅 accepted、实际画面与用户认可须分别判断。
