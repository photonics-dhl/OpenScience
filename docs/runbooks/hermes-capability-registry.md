...
| ScanSci PDF | 合法全文发现/下载 | `PRODUCTION OA / APPROVED_PILOT CARSI` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；当前只持久 publisher Cookie，不保存账号密码 | production `2019f8a…`；Task 4 local READY：独立 CPU browser、proxy-only `browser_net`、boot fail-closed、fsync exact Squid preimage、schema 3/4 immutable recovery；灰源/Tor 禁用 | OA 24,671,920-byte PDF 全绿；Task 4 local `69/5/0` + release `111/7/0`，机构 PDF/recreate/four-entry/72h/600s 仍待 ECS |
