...
## 当前能力索引：目的、调用、效果
2026-09-23正式像素审图：e7e独立CLI候选已部署但Worker开关false；设备登录虽成功、隔离服务现已停用，从未消费正式图片。用户明确不增加Codex CLI登录前置，改由Hermes现有 `generated-image-review.ts` 注入科学证据、原图字节及review阶段skills，借已登录Chat网页的5.6Sol检查真实像素。当前未部署候选只给schema3图片审阅显式model，旧无model收据继续识别6Pro；原Gateway/Domain收据门禁不变，网页版runner按菜单Sol选中与输入框`5.6 High`双重校验。新空白页无发送选择已实证，正式产品任务/质量仍未观察；准确版本见CURRENT。不得把本机Sol咨询、网页登录或服务安装当正式审图。
