...
2026-09-24 用户纠正后的风格边界：学术/编辑/淡彩仅为旧图样本，唯一目标是让未读者看懂论文。固定上游 001–277 手绘风格中 21 条缺文字特征不能自动选择；Baoyu article 23、infographic 22 可作为构图/外观资源。590902be 已将 `auto` 接入原 science→art→render/正式审图：science 不消费目录，art 选择，后续只加载选定资源并记 provenance；CI 35952973017 success、已部署，但私有真实任务 4d96bbb8 的科学阶段后在 art 以 `auto_style_invalid_scene_0` 三次拒绝，尚无新 PNG。故障候选用独立 `styleId` 消除模型手写内部标记，目录只列 256 条可选手绘，定向 12 测试/Worker typecheck/独立 High 通过；仍待 CI、发布和真实图像效果。没有增加第二论文分析器、风格配额或自动 provider 切换。
