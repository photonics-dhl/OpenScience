...
## 当前能力索引：目的、调用、效果
2026-09-23正式像素审图：e7e独立CLI候选Worker开关false、隔离服务停用，未消费正式图片。用户明确不增加Codex CLI登录前置；24b3已部署Hermes现有 `generated-image-review.ts` 注入科学证据、原图字节及review阶段skills，再交已登录Chat网页5.6Sol检查像素。仅schema3图片审阅显式model，旧无model收据继续识别6Pro；原Gateway/Domain收据门禁保持，网页runner按菜单Sol选中与输入框`5.6 High`双重校验。用户授权后同审计补内部credit，第二篇手工review-only副本已创建但Worker误套托管技术收据、provider调用0；候选在现有Domain/Worker补手工收据和当前管理员角色复核、原任务零调用安全重试，定向测试/独立High通过，待CI/部署/真实审图。长期管理员免内部额度仍需与逐任务审计一起实现；Chat供应商额度独立。不得把本机Sol咨询、网页登录或服务安装当正式审图。
