...
## 当前能力索引：目的、调用、效果
2026-09-26 机制配文与标签预算复发：第一篇曾从十标签缩至四标签；第二篇私有 v13 的 Hermes auto 初稿 `52ae14c2` 仍在明确“最多四标签”请求下给八标签/整条公式，读者 `narration` 先列结果数字而非“空间局域→短时间脉冲”机制。普通科学修订 `7a8fc8df` 收成四标签/机制骨架，但原末审正确 blocked：单幕 120 字配文容不下独立的单电子虚拟限制和电子团结果/产额权衡。原 `openscience-research-illustration` skill v14 候选在 science/review 增加机制配文先讲关系、明确标签预算核对、必要时拆成聚焦场景；既有 1–6 幕合同、SourceMap/Claim/Evidence 与 provider/状态机不变。定向加载测试 6/6、Worker typecheck、独立 High GO；CI、部署和真实 v14 任务待完成，不能把指令注入冒称成图合格。
