...
2026-09-24 用户纠正后的风格边界：上述三类是当前实图样本，不是产品目标或验收配额。[handraw-style 原库](https://github.com/yang0/handraw-style)在项目适配所固定的上游提交已有 001–277 编号风格；其中 21 条缺文字特征，需参考图，当前不能自动选择。Baoyu 本仓风格文件为 article 23、infographic 22。当前候选把 256 条可文本描述的手绘风格和 Baoyu 两组风格索引只交给现有 art 阶段，science 仍只用既有已审来源；渲染依据结构化 treatment 只读已选条目，资产设计技能记录其资源。无偏好图像入口改为 `auto`，显式用户风格仍保留；不加第二个论文分析器或模型阶段，也不把风格数量说成真实使用/质量成果。候选已通过定向单测/类型检查，仍待 CI、部署及真实 PNG 的阅读与像素效果验证。
