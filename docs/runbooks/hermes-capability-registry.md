...
拒收证据保存缺口：planner仅在完整计划通过后存checkpoint，5226拒收正文未持久化；Gateway已有onRejectedCandidate同步回调，未来沿原task私有证据路径收口，不为此失败另设恢复槽或用手工截字放行。原run已用12/max13、剩余1图，额外一次收费方案授权待明确，当前“继续”执行的是限制修复。
