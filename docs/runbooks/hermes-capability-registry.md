...
| `ink-notes` | 部署后 planner / review 反复 `out=8192` 触顶，未产出成图 | `bd1082b5` + `111a8558` + `590d1b76`（attempted） | review 路径 escalateMaxTokens 已升到 24576 且重试预算已修（1 normal + 2 escalated = 3 calls）— 真实路径里有 1 次升到 16K 出图。但 planner 自身在 8192 上反复 `encoding:length_350_max_200` / `subjects_1_description:length_141_max_100` —— 模型在 200/100 字符硬预算下不会自动压缩；retry 反馈让模型越改越短 |
| `knolling` | 部署后 review 路径成功升到 16K，planner 在 retry 阶段反复 schema 错（`encoding:length_350_max_200`、`subjects_1_description:length_141_max_100`、`label_count`），未产出成图 | `bd1082b5` + `111a8558` + `590d1b76`（attempted） | planner schema 硬预算（composition 200 / treatment 220 / subject description 100 / encoding 200 / label 80） + model 不自动缩 — 是**真**模型能力问题，不是代码缺陷。要解决：要么 prompt 显式教 model 写之前先估长度、要么 schema 放松（破坏上游校验） |

未跑过的 42 套风格：现仓库内每套 `.md` 都可被新逻辑读出。已知约束：planner 的 composition (200) / treatment (220) 字符预算对多元素、多 label 的指令是天花板——属于**模型能力**，不是代码缺陷。短路 instructions（一次性描述一个对象 + 1–2 个 labels）可绕过。`watercolor` 是这条路径已被验证的最长指令上限（~50 字符 + 6 label）；`editorial` / `subway-map` 等单对象风格应该都能过。下一步：(1) 跑 5–10 套短路 instructions 验证风格目录宽度；(2) 在 illustration-planner 的 prompt 里显式教 model 在写每个字段前**估字符数**（不要超过硬预算），让 retry 的反馈不再让模型越改越糟。
