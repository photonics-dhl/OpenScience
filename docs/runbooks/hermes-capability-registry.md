...
| ScanSci PDF | 合法全文发现/下载 | `APPROVED_PILOT / BLOCKED` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；必要时账号凭据进入 root-only server Secret | `2560bd8` 本地 gated：descriptor Secret；HTTPS/public DNS + embedded-IPv4/NAT64；compatibility-checked serial sources；256 MiB tmpfs；灰源/Tor 禁用 | 本地 open P0/P1 = 0；OA/机构 PDF、serial runtime、recreate session、四入口、72h/600s、migration 33、灰源调用 0 未完成前不得 production |
...
| ScanSci PDF | Source acquisition owner | School/source access terms and redistribution rights checked per source | upstream `1.11.0` / commit `7017814…b8e` / archive `db537914…9208b9`；legal/auth image IDs `ECS_PENDING` | 1 CPU/1 GiB/64 PID/256 MiB tmpfs；2 slots × serial 100 MiB temp；ECS measurement pending | Download success/P95 and publisher throttling measured on ECS | No per-user charge assumed until terms review；egress/storage metered | Authorized source → temp storage → parser；rights retained；Secret/session never enter task/DTO | Local signature/AST serial, NAT64/SSRF, Secret, rollback contracts green；real OA/CARSI/72h/600s pending | `SCANSCI_ENABLED=false` and credential revocation；previous release keeps own verifier/tmpfs identity | Disable adapter；expire links/cache；retain hashes/rights audit only |
