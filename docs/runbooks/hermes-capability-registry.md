...
| ScanSci PDF | 合法全文发现/下载 | `APPROVED_PILOT / BLOCKED` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；必要时账号凭据进入 root-only server Secret | `755b7b5` 本地 gated：descriptor Secret；NAT64；serial + pinned negative cache；soft+hard 100 MiB RLIMIT；fresh-`/tmp` shared-entry probe；exact-temp rename；256 MiB tmpfs；灰源/Tor 禁用 | 本地 P0/P1=0；Linux RLIMIT、OA/机构 PDF、recreate session、四入口、72h/600s、migration 33、灰源 0 未完成前不得 production |
