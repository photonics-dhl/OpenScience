...
## 当前能力索引：目的、调用、效果
2026-09-22后续真实断点：同一target被image/review不同任务登记三次，`runner.cjs`认领任意空白home造成跨broker误占；候选改为新请求只创建自有页，旧会话恢复保持。六PNG在review提交前技术失败，原技术子集因unknown sibling及没有accepted anchor被整体阻断，候选仅恢复可证明未提交的审阅/生成并保留unknown。`illustration-planner.ts`科学返工只向science传正式issues、art丢失具体旧缺陷，实际新稿再次加入禁用标题；候选复用原不可信反馈投影补接art。没有新分析器/模型阶段/技能安装，不把M3 accepted当来源与叙事质量通过；候选、独立审查、部署及真实消费只见CURRENT。
