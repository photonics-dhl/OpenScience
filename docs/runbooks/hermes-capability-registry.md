...
## 当前能力索引：目的、调用、效果
2026-09-26 v11 像素漏判：`21fefb6a` 真图 hash `4b989596…` 正式 5.6 Sol accepted、人工打开后独立 High NO-GO。上方“近场·倏逝”纹理远离圆孔且向外扩展，像第二束传播波；原 reader narration 说图“不暗示验证”，却未直述论文无近场独立实验/数值交叉核对。旧图/accepted 收据保留私有；原科学分镜普通修订 `6c384954` 把局域纹理移到孔缘、远场放远处，并要求直接说证据限制。现有 `generated-image-review.ts` 原提示已要求核形状/关系，却未明确标签正确不能抵消距离与波纹的错误空间编码；候选最小补充这一检查，原配图 skill v13 补读者限制直述，未建新门/模型阶段。17 项定向测试、Worker typecheck 本地通过；CI、部署和下一真图效果尚待验证。
