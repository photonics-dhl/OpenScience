...
| ChatGPT web image/science review | 内容驱动生图与原PDF独立科学复核 | `PRODUCTION` | 使用已授权服务器浏览器会话；不计入Codex调用；精确canonical续取、禁止重发，最终确认仍由用户完成 | 当前release/provider `36e6a8c4…`；图片与科学审阅独立锁；附件仅经受限OpenAI域名；科学审阅1800秒、持锁期间15秒heartbeat | 已回传真实PNG；Deep-sub-cycle初审与补证均回传。task `103de6d5…`零新增网页调用复用结果，保留四字段并只阻断method/reproducibility；invalid补证保留审计，不冒充通过 |
