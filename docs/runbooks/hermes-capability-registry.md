...
| 单一 image provider、无自动回退：`gateway.generateImage` 原先只用 `imageProviders[0]` | chatgpt-web 桥或账号额度故障时无备援，任务直接失败 | **已实现有界回退（提交 `3e6ee3bb`，待独立 High 审查后部署）**：按序尝试 `imageProviders`，**仅在"确定未提交"时前进**——provider 被禁用/未配置、不支持参考图、provider 明确报 `USAGE_LIMIT` 额度用尽；执行失败、超时、未知错误一律立即抛出，避免第二个账号为可能已提交的请求重复付费。回退 provider 由**显式**环境变量 `HERMES_SCENE_IMAGE_FALLBACK_PROVIDER` 开启（缺省不改变单 provider 行为，符合"不自动切换消耗额度"）。3 个回归用例覆盖：额度用尽前进、不确定性失败不前进、单 provider 保持 `IMAGE_USAGE_LIMIT` 语义。残余风险：参考图请求在 primary 不支持时会前进到支持参考图的 provider，只能由运维配置正确组合来保障 |
