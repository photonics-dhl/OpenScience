...
| 科学视觉清晰度指导（候选） | `.agents/skills/openscience-scientific-visual-clarity/SKILL.md` → `apps/agent-worker/src/skills/installed-media-skills.ts` 的 science/plan 固定章节与既有 designSkills provenance；基于 [K-Dense scientific-visualization](https://github.com/K-Dense-AI/scientific-agent-skills/tree/49c6e97775eaa18ba791bebe23162a70ae601c18/skills/scientific-visualization) MIT 方法适配 | 本机 `find-skills` 与项目 Skills CLI 早已存在，搜索本身不等于产品加载。新规则针对定量/概念符号区分、颜色冗余与未读者可理解性；独立High指出拆场景必须由science决定且末审原100k来源预算不可挤占，故正式review保持不变。不接上游脚本/OpenRouter、新模型阶段或数据绘图器。源码已接线、尚未部署或发生真实任务消费，质量收益未知；固定输入来源窗口、正式5.6Sol审图和跨任务经验缺口仍在，详见CURRENT |
