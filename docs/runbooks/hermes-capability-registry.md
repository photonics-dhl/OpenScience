...
| Docling | layout/table/OCR parser 候选 | `APPROVED_PILOT` | MIT；官方 wheel `2.123.0`/SHA-256 `95c0a4d…fde9c` 已锁定 | 独立 parser candidate image，不污染现有生产镜像；OCR/remote/plugin disabled | 双栏/表格/公式、P95、内存；与 LiteParse 二选一主 parser |
...
|---|---|---|---|---|---|---|
| 2026-08-27 | Docling evaluation candidate | untracked → source-locked pilot | official wheel `2.123.0`/MIT；SHA-256 `95c0a4d…fde9c`；image digest pending | local no-Docker runner `3/3`、worker evaluation `14/14`、Bash syntax GREEN；build-time package freeze/model hash manifest + pre-corpus strict lock contract；ECS quality/RSS not yet measured | keep `APPROVED_PILOT`; no production route/Compose change | Codex |
