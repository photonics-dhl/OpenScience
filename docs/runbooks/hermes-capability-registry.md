...
| ScanSci PDF | 全文发现/下载 | `PRODUCTION official MCP` | 浙江大学认证作为平台持久 session；管理员在普通浏览器认证后以官方 `cookie_import` 导入，不部署第二 browser/auth 服务。官方来源策略默认不覆盖，17 tools 均保留 | release `761b93d…`；`scansci-pdf==1.13.1`，CPU-only、固定 Squid、持久 `scansci-data`、瞬态 `scansci-papers`；旧 runtime 已清零 | 17/17、Worker-origin MCP 与真实 OA 已生产；ZJU Cookie/机构 PDF/four-entry/72h/600s pending |
...
| ScanSci official authenticated acquisition | Source acquisition owner | Upstream package/repository and publisher login contract pinned | `scansci-pdf==1.13.1`；archive `c5bdec13…b2507e`；candidate schema-6 MCP identity | MCP 2 CPU/2 GiB/512 PID；only retrieval network/no host port/read-only root；persistent data and transient papers separated；shared GID 11000 only for exact staging cleanup | OA Worker journey succeeded；institutional success/P95 pending one official cookie import | No platform API fee；egress/storage metered | MCP download → bounded PDF/hash/source → SeaweedFS → exact staging unlink；session data stays server-side and never enters DTO | 17 tools、Worker-origin MCP probe、OA PDF、container recreation/session and four-entry/72h journey are acceptance gates | schema-6 MCP identity；schema-5 one-time MCP-only restore | Restore immutable previous MCP release；retain `scansci-data`, hashes, provenance and accepted objects |
...
| Production `agent-worker` | 已注入 | 已注入；供应商额度耗尽 | 已注入；真实任务成功，连续请求可被 429 节流 | `761b93d…` 运行 official MCP client；migration 34、17 tools/真实 Worker OA 已生产，官方 cookie import/机构 PDF pending |
...
2026-08-26 Foundation 当时仅建立 `BASELINE_ONLY`。截至 2026-09-02，CPU parser cascade/Tesseract、BGE-M3、PostgreSQL lexical search 与 official ScanSci MCP 已通过 exact-SHA ECS 评测、隔离部署、降级/恢复或真实下载门禁升为 `PRODUCTION`；ScanSci 机构路径仍须完成一次官方 cookie import 和真实订阅 PDF。Docling、LiteParse、GROBID、PaddleOCR 仍为 `APPROVED_PILOT`，MiniMax OCR 继续 `BLOCKED`。现行 16-case parser 为 14 succeeded / 2 intentional needs_review / 0 failed / 0 false-ready，因此不再为“软件齐全”重试 Docling；只有真实用户文档暴露可归因的双栏、公式或表格缺口时，才以同一 ECS CPU/model/RSS/quality gate 重开评估。
...
|---|---|---|---|---|---|---|
| 2026-09-02 | ScanSci official-only production cleanup | private bridge retained → upstream-only runtime | release `761b93d…`；upstream `1.13.1`；PR #47 | CI `33637694396`；core/search 34/2；Parser/BGE；17 tools、Worker MCP、real OA、public CAS；old runtime counters 0；disk 66G→37G | rollback `e9e2058…`；保留 `scansci-data`/`scansci-papers` 和产品数据 | Codex |
