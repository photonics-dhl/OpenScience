...
## 当前能力索引：目的、调用、效果
2026-09-26 数值来源绑定漏检：第二篇聚焦两幕 `0b5e5db7` 被原 M3 科学末审 accepted、保存 draft；人工与独立 High 回读发现幕1 `19 as`、幕2 `99 as`/`270` 仅在 title/message/labels/narration，实际 subject 都只绑装置/参数原文，未绑结果段；美术又要求无数据渲染的钟形曲线，整案 NO-GO、未批准/生图。原 `requireIllustrationSourceSupport` 只核 subject 原文身份，review 只核所选 supporting passage 是否存在，两者均不能证明画内数值受该 subject 支持。原规划器新 science 物化时增加逐个数值及单位/变量与 subject 描述＋其确切原文的窄校验，覆盖场景/总标题、主旨及中英文邻接数字；不改历史资产读取、Provider 或发布。独立 High 前两轮 NO-GO 推动修正同值异单位、中文前缀、英文句末数值及光子产额写法；自动风格定向 36/36（含第二篇两工况及真实 Gateway 拒收→反馈→修正）、Worker typecheck，最终独立 High GO；CI、部署和真任务仍待核。该词法守卫只能拦明显未绑数值，不能替代条件/几何/虚构曲线的科学复核；旧坏图/收据保留。
