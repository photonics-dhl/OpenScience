...
制作说明容量纠正（用户明确继续）：历史1500来自MiniMax image-01（官方控制台 https://solutions.minimaxi.com/debug/image 仍为该值），不能作为Chat限制。当前候选将1500留供应商实现，Chat/Codex按原16KiB完整JSON字节资源保护，inner job沿用32KiB；compiler保留科学、已审art和repair，只按剩余字节裁可选风格指导。内部message/subjects/encoding/constraints/composition/treatment归一为原4000整份brief预算，图内labels/reader标题说明另按可读性约束；没有新身份、schema、模型或恢复槽。候选主要差异High静态PASS；独立receiver引用收口纯协议模块，避免总入口拉入新Undici而在仅含dist的bundle缺依赖，增量High静态PASS。先receiver后应用部署待完成，未新增付费请求。
