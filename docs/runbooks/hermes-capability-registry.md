...
| BGE-M3 | Search platform owner | Exact model/revision license and redistribution terms required before lock | `UNLOCKED`; model revision and SHA-256 required | CPU-only versioned volume；pilot records peak RSS/CPU and hard cap | Query P95 ≤2.5s target；index throughput and cold start measured | No API fee；CPU/RAM/disk and rebuild time budgeted | Authorized chunks → embedding worker → separate search database | Multilingual top-k relevance、explanations、P95/RSS、rebuild | Embedding route weight zero and worker scale zero | Route to PostgreSQL lexical search；restore prior model/index version |
| PostgreSQL lexical search | Search platform owner | PostgreSQL and any extension license recorded at version lock | `UNLOCKED` search schema；server major inherited only after migration plan | Separate connection pool/database；load test sets CPU/RSS cap | Deterministic baseline plus query P50/P95 and concurrency | Database compute/storage only；capacity recorded | Authorized normalized text → `SEARCH_DATABASE_URL`；no provider | Relevance、migration/failover、concurrency and rebuild | Search backend selector | Switch to prior search backend/schema and replay rebuild manifest |
| Docling | Document intelligence owner | Exact upstream release license and bundled model terms required | `UNLOCKED`; candidate image digest required | Isolated CPU image；pilot hard-capped and peak RSS recorded | Compare per-page P50/P95 and queue throughput | No API fee；CPU/RAM/image/model storage measured | Temporary document → isolated candidate → unified source map | Dual-column/table/formula/scan fidelity against corpus | Parser route weight zero and candidate container stop | Return route to current parser image；remove candidate volume after retention window |
| LiteParse | Document intelligence owner | Apache-2.0 upstream npm package；transitive lock retained for review | npm `2.14.0`，integrity `sha512-lIFB…ThWA==`；Linux x64 optional package lock present；image digest pending ECS build | Candidate run contract：2 CPU、2 GiB/64 PID、non-root/read-only/network none；peak RSS reported | Selected PDF case P50/P95；120s hard timeout，retention target ≤30s/page | No API fee；CPU/RAM/image storage measured | Read-only self-authored corpus → isolated candidate → content-free outcome report；OCR disabled | Page-text locator fidelity、false-ready、speed、memory versus Docling/current parser | No production route；candidate container is ephemeral | Candidate remains `APPROVED_PILOT`; production parser unchanged |
...
| 2026-08-27 | LiteParse evaluation harness | untracked → source-locked pilot | npm `2.14.0` / Apache-2.0；package integrity and Linux x64 optional package locked；image digest pending | schema-v2 16-case corpus / same 7-PDF layout subset；local no-Docker runner `3/3`、worker evaluation/export `18/18`、build/typecheck/bash syntax/lock integrity GREEN；ECS image/quality/RSS not yet measured | keep `APPROVED_PILOT`; run immutable ECS corpus bake-off before any retention or production manifest change | Codex |
