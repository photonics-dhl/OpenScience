...
| ScanSci PDF | 合法全文发现/下载 | `APPROVED_PILOT / BLOCKED` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；必要时账号凭据进入 root-only server Secret | `4f6361e` 本地 Task 9 gated：source-locked legal/auth/session、file-only descriptor Secret、pre-request HTTPS/public-DNS、stable error；灰源/Tor 硬禁用 | 本地 open P0/P1 = 0；OA/机构真实 PDF、recreate session、四入口、72h/600s、migration 33、灰源调用 0 未完成前不得标 production |
...
| ScanSci PDF | Source acquisition owner | School/source access terms and redistribution rights checked per source | upstream `1.11.0` / commit `7017814…b8e` / archive `db537914…9208b9`；legal/auth image IDs `ECS_PENDING` | Local Compose contract 1 CPU/1 GiB/64 PID/64 MiB tmpfs；ECS measurement pending | Download success/P95 and publisher throttling measured on ECS | No per-user charge assumed until terms review；egress/storage metered | Authorized source → temp object storage → parser；rights decision retained；Secret/session never enter task/DTO | Local policy/SSRF/Secret/rollback contracts green；real OA/CARSI/72h/600s pending | `SCANSCI_ENABLED=false` rollback and credential/session revocation | Disable adapter；expire links and cache；retain hashes/rights audit only |
