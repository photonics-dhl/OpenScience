...
| PaddleOCR | 复杂中英扫描 OCR 候选 | `APPROVED_PILOT` | Apache-2.0 package `3.7.0`/SHA-256 `c0f0a81a…d338` 已锁定；模型条款/hash 未取得 | 独立 CPU candidate image/model volume；exact ECS build 因缺少 `libgomp.so.1` fail-closed，未产出 image | 无 OCR 质量结果；修复 image runtime 后才可与 measured Tesseract 1/2 locator 比较 |
...
| PaddleOCR | Document intelligence owner | Apache-2.0 wheel `3.7.0` locked；OCR model terms/hash manifest still required | wheel SHA-256 `c0f0a81a…d338`；CPU PaddlePaddle `3.3.1` installed in build；no image/model digest because `libgomp.so.1` import dependency was absent | Intended 2 CPU/2 GiB/64 PID、non-root/read-only/network none；execution not reached | No Paddle latency result；measured Tesseract baseline is 480 ms on selected scan | No API fee；failed build resources removed | Selected scan PDF intended → isolated OCR → locator revalidation；model acquisition not reached | Build fail-closed before model lock/preflight；no quality inference or Tesseract comparison | No production route；candidate image absent | Keep `APPROVED_PILOT`；current Tesseract fallback remains production |
...
|---|---|---|---|---|---|---|
| 2026-08-28 | Task 1 parser bake-off | pilots unchanged | harness `04178b4…`；production parser `e2c0eaf…`/image `sha256:88da362b…5606`；LiteParse image `sha256:352cf5d9…5601` | current 7-PDF 7/16 locators、P50/P95 96/275 ms、peak RSS 108,040,192 B；LiteParse 13/16、8/163 ms、61,300,736 B；Tesseract selected scan 1/2、480 ms、110,149,632 B；Docling model download `Errno 99`、Paddle missing `libgomp.so.1`，均无质量推断；exact cleanup counts 0 | no production route/Compose mutation；active release remains `e2c0eaf…` | Codex |
