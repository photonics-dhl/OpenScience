...
| ScanSci PDF | 全文发现/下载 | `PRODUCTION legacy OA / RELEASE CANDIDATE official MCP` | 浙江大学认证作为平台持久 session；凭据只在 loopback noVNC 输入。官方来源策略默认不覆盖，17 tools 均保留 | production 仍 `405b85a…`；candidate `9730529…` 为 `scansci-pdf==1.13.1` 官方 MCP，CPU-only、2 CPU/2 GiB/512 PID、只读 root、持久 `scansci-data`、瞬态 `scansci-papers`，auth 独占 `auth_net`，无应用/数据库 Secret | 早期 ECS official 17/17 + Worker OA `%PDF-` 24,671,920 bytes / `d57dc94c…f484a`，`source=arXiv`；review 已关闭 durable ack/policy discard、provider version、real health、child supervision、schema-5 next deploy 与 auth peer isolation，new image/生产切换/官方登录/机构 PDF/four-entry/72h/600s pending |
