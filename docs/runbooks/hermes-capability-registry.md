...
## 当前能力索引：目的、调用、效果
2026-09-23正式像素审图候选：只在 `illustration-image` 调用中把既有 `gateway.reviewScientific` 路由至独立 `gpt-5.6-sol` CLI runner；Hermes全文、science/art/final和Chat生图不改。`packages/ai-gateway/src/codex-sol-review.ts`保存来源/图片哈希、供应商及结果收据，并在旧Chat spool同名reservation放置精确供应商绑定，回滚版遇到它会拒绝复投；`infra/codex-sol-review`隔离运行、允许官方CLI在独立0700账号目录刷新凭据。Domain新增管理员原PNG原字节review-only入口，现有审批门禁在无正式收据时保持关闭。定向测试与独立High增量复核通过；候选未提交/部署，服务器设备登录未完成，真实产品效果未知。不得把本机Sol咨询或服务安装当正式审图。
