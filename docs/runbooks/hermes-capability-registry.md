...
| ScanSci PDF | 全文发现/下载 | `PRODUCTION legacy OA / RELEASE CANDIDATE official MCP` | 浙江大学认证作为平台持久 session；凭据只在 loopback noVNC 输入。官方来源策略默认不覆盖，17 tools 均保留 | production 仍 `405b85a…`；candidate `7ca7976…` 为 `scansci-pdf==1.13.1` 官方 MCP，CPU-only、2 CPU/2 GiB/512 PID、只读 root、持久 `scansci-data`、瞬态 `scansci-papers`，无应用/数据库 Secret | 官方 17/17 tools；ECS Worker OA `%PDF-` 24,671,920 bytes / `d57dc94c…f484a`，`source=arXiv`，post-ack staging PDF `0`；生产切换、官方登录、机构 PDF/recreate/four-entry/72h/600s pending |
...
| ScanSci official authenticated acquisition | Source acquisition owner | Upstream package/repository and publisher login contract pinned | `scansci-pdf==1.13.1`；archive `c5bdec13…b2507e`；candidate MCP image `sha256:551684a7…e4570e8` | MCP 2 CPU/2 GiB/512 PID + auth profile loopback noVNC；persistent data and transient papers separated；shared GID 11000 only for exact staging cleanup | OA Worker journey succeeded；institutional success/P95 pending one official login | No platform API fee；egress/storage metered | MCP download → bounded PDF/hash/source → SeaweedFS → exact staging unlink；session data stays server-side and never enters DTO | 17 tools、OA PDF、container recreation/session and four-entry/72h journey are acceptance gates | schema-5 exact image identity、previous schema-5 restore、disable adapter | Restore immutable previous release；retain `scansci-data`, hashes, provenance and accepted objects |
...
| Production `agent-worker` | 已注入 | 已注入；供应商额度耗尽 | 已注入；真实任务成功，连续请求可被 429 节流 | `405b85a…` 仍运行旧 adapter；官方 MCP candidate 已在 ECS 通过真实 Worker OA，但未完成 production CAS/migration 34/官方登录 |
...
2026-08-26 Foundation 当时仅建立 `BASELINE_ONLY`。截至 2026-09-02，CPU parser cascade/Tesseract、BGE-M3、PostgreSQL lexical search 与旧 ScanSci OA 已通过 exact-SHA ECS 评测、隔离部署、降级/恢复或真实下载门禁升为 `PRODUCTION`；官方 ScanSci MCP 已完成 ECS candidate OA/Worker/rollback gate，待 merged production 和一次官方机构登录后替换旧 adapter。Docling、LiteParse、GROBID、PaddleOCR 仍为 `APPROVED_PILOT`，MiniMax OCR 继续 `BLOCKED`。现行 16-case parser 为 14 succeeded / 2 intentional needs_review / 0 failed / 0 false-ready，因此不再为“软件齐全”重试 Docling；只有真实用户文档暴露可归因的双栏、公式或表格缺口时，才以同一 ECS CPU/model/RSS/quality gate 重开评估。
...
|---|---|---|---|---|---|---|
| 2026-09-02 | ScanSci official MCP candidate | official pilot → release candidate | code `7ca7976…`；MCP image `sha256:551684a7…e4570e8`；upstream `1.13.1` | 17 tools；real ECS Worker OA `source_retrieval/arXiv`、24,671,920 bytes、`d57dc94c…f484a`；staging PDF `0`；schema-5 restore green | production unchanged `405b85a…` / `09093e7…`；old implementation retained until acceptance | Codex |
