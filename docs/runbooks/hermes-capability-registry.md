...
| ScanSci PDF | 合法全文发现/下载 | `TASK 10 REOPENED / APPROVED DESIGN` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；必要时账号凭据进入 root-only server Secret | 独立 `scansci-legal` + loopback auth helper + persistent session volume；Sci-Hub/LibGen/SciBban/Tor 硬禁用 | OA/机构真实 PDF、容器重建后 session、四入口、72h/600s、灰色源调用 0 均未完成前不得标 production |
