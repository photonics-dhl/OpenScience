...
| `knolling` | 16K/32K 预算（`a6dfd8ef`）下：planner + review 跑通（review 10K+ 真正跑出 3-scene verdict），但 art stage `art_scene_count` 反复失败 — planner 说 N scenes，art stage 输出非 N | `a6dfd8ef`（attempted） | 下一关是 model 一致性 + art stage `scenes.length === intent.scenes.length` 严格检查 |
