...
| ScanSci PDF | 合法全文发现/下载 | `PRODUCTION OA / BLOCKED CARSI` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；只持久 publisher Cookie，不保存账号密码；当前凭据被 ZJU CAS 拒绝 | production `405b85a…`；独立 CPU browser、proxy-only `browser_net`、boot fail-closed、IdP main-frame gate、schema 3/4 immutable recovery；灰源/Tor 禁用 | OA 24,671,920-byte PDF 全绿；ScanSci `174/11/0`、PR #38 CI/ECS green；机构 PDF/recreate/four-entry/72h/600s 待正确凭据 |
