...
## 当前能力索引：目的、调用、效果
2026-09-22叙事链路：Hermes全文/六维/Claims及science/art沿原能力，首篇已真实公开。后两篇6份真实6Pro结果已沿原产品入口写回；并发P2034恢复与失败step收尾已独立High通过并交付，两项精确exec3成功。review-broker复用完整job结果，正常composer/标签清单/队列顺序已交付。两篇已进入下一次科学修订，仍待reader/新发布。同图连续改稿未传原task至spool，新turn图片锚点也未接，不能冒称原对话改图；后续沿现有revisionAssetId/protocol/runner补接，不另建会话平台。精确状态及证据见[CURRENT](../handoff/2026-09-10-hermes-web-image-handoff.md)。
