...
| `presentationContext.figureAuditPlan` 注入到 workspace.guide | 已部署：`workspace-guide` 在 `apps/agent-worker/src/workspace-guide.ts` 的 `workspaceGuideHandler` 加 `readTrustedFigureAuditPlan` helper，**三层越权防御**（`session.userId === userId` + `session.researchObjectId === presentationVersion.researchObjectId` + `payload.researchObjectId/versionId === presentationVersion.*`），只取最新 succeeded 的 `presentation.figure-audit` task，把 `{ figures, style, auditedAt }` 注入 `presentationContext.figureAuditPlan`。**闭合 figure-audit → 出图回路**：用户在 Hermes 说"为论文 Fig. N 出图" / "按图清单生成" 时，LLM 看到 plan 并整体复制到 `presentationDraft.figurePlan`（沿用既有 `StoryboardRequest.figurePlan` 字段，无新增 schema）。Plan 与已生成 `planState` 在 prompt 中明确区分，避免 LLM 把审计结果当成已生成资产。**生产 fix**：`task.result` 实际是 `{result: FigureAuditResult}` 套层结构，helper 在 `07574e4e` 改成先尝试 `result.result.figurePlan` 回退 `result.figurePlan`，对应生产数据库 `agent_tasks 4cd4a5f9-...` 的真实形状。**prompt fix**：`b26cf5e9` 端到端触发验证发现 LLM 把 figureAuditPlan.caption 复制进 `presentationDraft.instruction` 触发 `presentation_instruction_length_1178_max_1000` 拒绝；新增 prompt 段明确"instruction ≤ 1000 字符、只描述视觉/构图/色调/材质、不要重复 caption 或 rationale" | `0e1e1b49` + `07574e4e` + 待部署 |
