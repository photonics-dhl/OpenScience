...
| 共享科学推理规则 | `skills/scientific-critical-thinking.ts` → `extractor.ts` 的reduce/bridge及科学审校；`installed-media-skills.ts` science/review复用综合指令并记录id/version | 项目runtime v2有真实41c及配图1da6/a38消费。v3候选复用同一证据规则，原综合指令保持，另提供来源审校指令给既有终审；方法适配不等于安装整个K-Dense包。当前部署与效果见CURRENT |
| 原文科学审阅 | `extractor.ts` 的 `modelScientificReviewCanonicalProposal` 显式注入科学Skill，经 `gateway.completeStructuredWithMetadata` 沿MiniMax主路由；仅显式 `context.mode=web` 改走 `webScientificReviewCanonicalProposal` → `gateway.reviewScientific` | 41c使用v2且服务失败，不能推断科学理解耗时。v3候选用原文逐主张核对、只因科学/来源问题修订，保留跨字段及相反证据；Worker从绑定的visual-narrative来源步骤派生主张要求，普通人工v5仍可选，原守卫/建议解析/自动消费校验保持。无新分析器或额外模型阶段；新指令的科学质量/耗时未观察，见CURRENT |
