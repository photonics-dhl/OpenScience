...
| ScanSci PDF | 合法全文发现/下载 | `APPROVED_PILOT / BLOCKED` | 一次浙江大学 CARSI 认证作为 Hermes 持久默认能力；必要时账号凭据进入 root-only server Secret | `cfc0ddc` 本地 Task 9 gated：source-locked legal/auth/session、file-only descriptor Secret、pre-request HTTPS/public/non-multicast DNS、stable error；灰源/Tor 硬禁用 | 本地 open P0/P1 = 0；OA/机构真实 PDF、recreate session、四入口、72h/600s、migration 33、灰源调用 0 未完成前不得标 production |
