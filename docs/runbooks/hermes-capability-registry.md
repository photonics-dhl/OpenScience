...
|---|---|---|---|---|---|---|---|---|---|---|
| MiniMax text/vision | Hermes AI Gateway owner | Provider terms and data policy must be rechecked before enable | `UNLOCKED`; exposed credential rotation blocks pinning | External compute；adapter stays inside existing worker budget | Minimal-page route；record provider latency and error rate | Admin-configured API budget；no unbounded retry | Selected pages → AI Gateway → provider → locator revalidation | OCR/table/formula fidelity、page cost、audit completeness | AI Gateway provider flag and zero route weight | Disable provider route；fall back to local parser and explicit review |
...
| LiteParse | Document intelligence owner | Exact upstream release and dependency licenses required | `UNLOCKED`; candidate image digest required | Isolated CPU image；pilot hard-capped and peak RSS recorded | Compare per-page P50/P95 and queue throughput | No API fee；CPU/RAM/image/model storage measured | Temporary document → isolated candidate → unified source map | Page/bbox fidelity、speed、memory versus Docling/current parser | Parser route weight zero and candidate container stop | Return route to current parser image；remove candidate volume after retention window |
