...
| 手绘风格匹配与表现 | `.agents/skills/openscience-handdraw-{router,style}/SKILL.md` → `installed-media-skills.ts` 的原 art plan / image render；适配 [handraw-style-router](https://github.com/lacemou/handraw-style-router) 与 [handraw-style](https://github.com/yang0/handraw-style) 的 MIT 方法，保留固定 provenance | 仅学术、编辑、淡彩；无上游图库/脚本、新 provider 或中途选择。v1定向5项测试、worker类型检查及CI 35832552646通过，49b5已部署。第二篇三种方案与真实PNG均有消费收据；结构化科学字段相同，实际像素只有编辑图获5.6Sol内部GO，学术图20 nm标尺端点错、淡彩图77→777 nm/多余波瓣均NO-GO。v2针对这三类通用失败补精确绘制指导，本地定向测试/类型检查通过，CI与部署待完成；这不是像素正确性保证。正式产品审阅未接线，旧scene-image/presentation-generation fixture/断言及主线CI lint为单独存量债务，详见CURRENT。 |
