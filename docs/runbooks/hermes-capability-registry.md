...
拒收证据保存缺口：planner仅在完整计划通过后存checkpoint，5226拒收正文未持久化；Gateway已有onRejectedCandidate同步回调，未来沿原task私有证据路径收口，不为此失败另设恢复槽或用手工截字放行。用户已明确授权一次额外方案执行与原余图。当前补接原retry-generation：对仅text/无候选的结构化失败，复用原task的一次显式执行及原图片额度；源/权限/CAS/审计与worker收据严格复验，不新增14额度、迁移或模型环节。候选与真实效果见CURRENT。
