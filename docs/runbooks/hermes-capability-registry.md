...
## 当前能力索引：目的、调用、效果
2026-09-23正式像素审图：e7e独立CLI候选虽曾部署但Worker开关false，隔离服务现已停用，从未消费正式图片。用户明确不增加Codex CLI登录前置；24b3已部署Hermes现有 `generated-image-review.ts` 注入科学证据、原图字节及review阶段skills，再交已登录Chat网页5.6Sol检查像素。仅schema3图片审阅显式model，旧无model收据继续识别6Pro；原Gateway/Domain收据门禁保持，网页runner按菜单Sol选中与输入框`5.6 High`双重校验。空白新页无发送选择与定向CI已实证，正式产品任务/质量仍未观察；当前1次内部额度被管理接口额外Basic Auth拦住，见CURRENT。不得把本机Sol咨询、网页登录或服务安装当正式审图。
