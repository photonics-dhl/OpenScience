...
|---|---|---|---|---|---|
| Existing `document-parser` sidecar | PDF/DOCX/OCR isolation | `PRODUCTION` release `c581712…` / image `sha256:0ac86bfc…d902` | 无外部 API | 固定容器；无网络/Secret、只读、非 root、512 MiB/64 PID、bounded IPC | 16-case 10 ready/6 review/0 failed/0 false-ready；startup text/locator/runtime 五项通过 |
| Tesseract `eng+chi_sim` | 扫描页 OCR fallback | `PRODUCTION` release `c581712…` | 免费、本地 CPU | 仅 parser 镜像；禁止宿主全局安装 | canonical scan 的 text、跨 block locator、Tesseract 5.3.0、confidence、bbox 全通过 |
