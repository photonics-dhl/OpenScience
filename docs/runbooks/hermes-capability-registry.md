...
| ScanSci PDF | 全文发现/下载 | `PRODUCTION official MCP` | 浙江大学认证作为平台持久 session；管理员在普通浏览器认证后以官方 `cookie_import` 导入，不部署第二 browser/auth 服务。官方来源策略默认不覆盖，17 tools 均保留 | production base `e9e2058…`；`scansci-pdf==1.13.1`，CPU-only、固定 Squid、持久 `scansci-data`、瞬态 `scansci-papers`；official-only candidate pending | 17/17 与 Worker OA 已生产；single-service topology/Worker-origin tools-list/独占 OA canary 本地 green。ZJU Cookie/机构 PDF/four-entry/72h/600s pending |
