...
## 当前能力索引：目的、调用、效果
2026-09-26 数值来源绑定漏检：第二篇聚焦两幕 `0b5e5db7` 被原 M3 科学末审 accepted、保存 draft；人工与独立 High 回读发现幕1 `19 as`、幕2 `99 as`/`270` 仅在 title/message/labels/narration，实际 subject 都只绑装置/参数原文，未绑结果段；美术又要求无数据渲染的钟形曲线，整案 NO-GO、未批准/生图。原 `requireIllustrationSourceSupport` 只核 subject 原文身份，review 只核所选 supporting passage 是否存在，两者均不能证明画内数值受该 subject 支持。原规划器新 science 物化时增加逐个数值及单位/变量与 subject 描述＋其确切原文的窄校验，覆盖场景/总标题、主旨及中英文邻接数字；不改历史资产读取、Provider 或发布。独立 High 前两轮 NO-GO 推动修正同值异单位、中文前缀、英文句末数值及光子产额写法；自动风格定向 36/36（含第二篇两工况及真实 Gateway 拒收→反馈→修正）、Worker typecheck，最终独立 High GO、CI 36186477404 success、生产 72d 部署。c071/8a36 两次真任务仍连续拒收 19 as，无方案/PNG，说明防漏生效但模型自修未通。被拒候选正文未持久化，不能断言具体写法；新候选对 Fig.2c 图号和阿秒/as 等中英单位做窄归一，并把诊断分为 subject 描述缺失/绑定原文缺失，39/39、Worker typecheck、独立 High GO、CI 36189783888 success，生产 08d 部署。真实 0386efbe 因 19_as_source、单幕 a8c04e9d 因 1_bare_description 三次拒收，均无新 PNG；Gateway 拒收回调的私有 AuditLog 只存有界数值字段/合法来源摘要、哈希，不存候选正文/原文且不改 checkpoint；42/42、typecheck、独立 High、CI 36193303110 成功，生产 a737 部署。真实 1d1ec917 三次拒收、无资产，收据证明 19 as 已绑 s6 原文，未绑定的是配文/encoding/标签中的裸 0/1，尤其编码含对象编号。显式结构编号/整对比例守卫保留无量纲 0/1、1 MeV、270 校验，43/43、typecheck、独立 High、CI 36195532262 success，生产 ac79 部署；真实 5089edf7 又因 19_as_description→1_bare_description→19_as_source 三次拒收，无资产。原文 s6 确含 `(FWHM_T, i.e., τ₁) of 19 as`，旧词法在缩写句点丢变量；新候选仅跨 i.e./e.g. 缩写点、不跨真实句界，允许 subject 省变量但 bound quote 须精确同值/单位/变量。FWHM_S 替换与跨句反例仍拒收；私有摘要只增白名单变量标记及首尾数值，44/44、typecheck、独立 High GO，CI/部署/真任务待做。该词法守卫只能拦明显未绑数值，不能替代条件/几何/虚构曲线的科学复核；旧坏图/收据保留。
2026-09-26 普通双幕科学修订的输出耗尽与检查点缺口：第二篇 `4660140f` draft 混用固定结果与 Fig.3e/f 参数扫描、跨幕比宽。原 science 修订 `117fe82c` 与不携旧长稿的 `1cd14e06` 均未产方案：Gateway 审计见 M3 先后在 16k/32k `finishReason=length`、仅 thinking 块，`AgentTask.result=null`、无资产；不是网页生图桥失败。原 `handler.ts` 只对带 managed narrative authority 的整篇叙事任务启用阶段检查点；这些独立普通修订不符合，不可冒称可同任务续接。进一步缩至单电子机制＋电子团固定结果的 `0b5e5db7` 在同次 Gateway 16k耗尽后32k成功，art/末审完成但来源质量 NO-GO（见上），说明范围收敛可恢复模型产出却不能替代科学核验。长期检查点补点应在既有任务/审计与复用权限上扩展，不另建分析器或绕过供应商额度。旧失败任务保留。
2026-09-26 多幕 auto 的结构提醒缺口：第二篇新两幕任务 `3909a349` 在生产 d296/v14 上失败，未保存方案或 PNG。时间有界 Worker 诊断显示科学方案初次 `label_reference_out_of_range_encoding_4_count_4` 后自动重试，下一结构化调用三次 `object_required` 耗尽；从调用次序推断落在艺术阶段，但没有原始拒收对象，不能断言模型具体返回了数组或其他类型。现有 `illustration-planner.ts` 只给单幕显式风格末尾重复 `scenes` 结构；对多幕/auto 添加逐幕数量、`layout`/`treatment`/`styleId` 键及顶层 wrapper 的同一请求提醒，不改 schema、模型、供应商或额度路由。automatic-style 12/12、Worker typecheck、独立 High GO、CI 36178400376 success，生产 909cc3e3 部署；新真任务 `4660140f` 成功保存两幕 auto draft，说明这次结构卡点跨过，但科学字段仍混合固定结果与参数扫描，且跨工况画面比例误导，原科学修订 `117fe82c` 在途。该样本不能证明所有多幕稳定或图文质量合格；失败收据保留。
2026-09-26 机制配文与标签预算复发：第一篇曾从十标签缩至四标签；第二篇私有 v13 的 Hermes auto 初稿 `52ae14c2` 仍在明确“最多四标签”请求下给八标签/整条公式，读者 `narration` 先列结果数字而非“空间局域→短时间脉冲”机制。普通科学修订 `7a8fc8df` 收成四标签/机制骨架，但原末审正确 blocked：单幕 120 字配文容不下独立的单电子虚拟限制和电子团结果/产额权衡。原 `openscience-research-illustration` skill v14 在 science/review 增加机制配文先讲关系、明确标签预算核对、必要时拆成聚焦场景；既有 1–6 幕合同、SourceMap/Claim/Evidence 与 provider/状态机不变。定向加载测试 6/6、Worker typecheck、独立 High GO、CI 36175932790 success，生产 d296e922 部署；新两幕真任务 `3909a349` 在途，实际消费与图文质量尚待核，不能把指令注入冒称成图合格。
2026-09-26 v11 像素漏判与纠正效果：`21fefb6a` 真图 hash `4b989596…` 正式 5.6 Sol accepted、人工打开后独立 High NO-GO。上方“近场·倏逝”纹理远离圆孔且向外扩展，像第二束传播波；原 reader narration 说图“不暗示验证”，却未直述论文无近场独立实验/数值交叉核对。旧图/accepted 收据保留私有。原科学分镜普通修订 `6c384954` 把局域纹理移到孔缘、远场放远处，并直述证据限制；原 `generated-image-review.ts` 提示补查标签之外的距离/纹理空间语义，原配图 skill v13 补读者限制直述，未建新门/模型阶段。17 项定向测试、Worker typecheck、独立 High、CI 36170285819 通过，生产 91ab1628 部署。下一真图 `0fdd4e30` hash `f909e763…` 仍形成右向楔形尾迹，正式 5.6 Sol 已正确 blocked，给出压回孔缘一至两倍半径的修订指令；这验证该次错误被识别，不证明审图普遍无漏判。art-only 方案 `93507dd7` 仅改构图/材质，新图 `65ffbfee` hash `54484359…` 正式 accepted、人工原始像素与独立 High GO，已只选此图发布第一篇 v5；匿名实点图片/六维/完整配文，v4 保留。该样本证明旧漏判的同类楔形尾迹在后续真图上可被拦截并修正，不能推论所有论文/风格的科学叙事都已稳定合格。
2026-09-26 读者配文断点：第一篇真实图 `7f497502` 经正式像素 accepted 和人工看图后发布 v4，但 `9bf1a712` 的 scene `narration` 写成“读者看到…上线/下线…”，公开页沿原 `mediaReaders` 忠实展示，未解释论文科学关系。不是 PDF/全文理解器缺失，而是原 `.agents/skills/openscience-research-illustration/SKILL.md` 在 science/末审未明确把逐图叙述视为读者可见科学内容；v12 已在原位置补规则，`installed-media-skills.ts` 继续在 science/review 阶段加载，无额外模型或审批门。6 项定向路由测试、CI 36165405964 成功，生产 945788a5 部署；新私有 v11 真分镜 `b98c4735` 已写出科学配文，但图内十标签过密；同源修订 `2164dacd` 收敛四标签并 approved，真实 provenance 含配图 v12、科学批判 v3、科学视觉清晰度 v1 与 Baoyu infographic。历史媒体复制 `carryVersionMedia` 可保留已批准图字节与来源，但 `research-record-snapshot.ts` 只从**本版批准分镜的本版图**冻结 reader 配文，不能借复制旧图改写新叙事。第一篇 v4 成品仍可读，科学叙事质量未最终认可。
2026-09-24 当前正式像素审图：Hermes 复用已审科学证据与 review 阶段技能组织检查，Chat 网页 5.6 Sol 接收实际 PNG；独立 Codex CLI 审图仍关闭。第二篇编辑图公开 v2；第三篇淡彩新第二幕 4e64c389 在已审科学字段不变前提下修正逻辑箭头，正式 accepted、人工核图并只选该图与获称赞首图公开 v2，旧 v1 保留。旧同父 scene1 的 MOED 错字虽被模型误判 accepted，人工拦下；提示词不能替代像素核对。管理员内部额度自动补账、普通用户额度和供应商真实配额边界不变；精确 release 与图片身份见 CURRENT。
2026-09-24 逐图质量反馈：用户明确称赞第三篇 scene0 淡彩概念图，同时指出其他图仍有问题。`installed-media-skills.ts` 按选定 style 只加载相应风格段落，不等于整个 Baoyu 库逐图执行；学术、编辑、淡彩不是风格白名单。薄弱处仍是把来源支持的科学关系编码成未读者能一眼理解且有美感的画面。旧 1d154c10 点积/频率误解被人工拦下；d225 虽获像素 accepted，人工因板书感 NO-GO；59b 的发射箭头被正式 blocked。原 art 阶段修成 cd150、真实图 4e64 后正式 accepted/人工核图/公开 v2。桥接 URL 等待由 30 秒改为有界 120 秒且预留 broker 余量，定向模拟与真实正常任务通过；晚于 30 秒的真实案例未见。当前[研发观察台](../proposals/2026-09-23-hermes-development-live.html)展示逐图能力/效果；技能、自动审阅 accepted、实际画面与用户认可须分别判断。
2026-09-24 第一篇返工的真实能力缺口：同一 v3 来源/6 Claim/58 Evidence 已由现有 Hermes 配图 science→art 阶段复用，不再全文分析；单孔候选 1cd83d20（hash 081eb569…）虽通过计划审阅且实际生成，正式网页 5.6 Sol 像素审图 blocked，人工额外发现“侧视横带中嵌正视圆孔”投影不可能。现有 art-only 修订 1b977c43 科学字段不变、改斜视薄屏/椭圆孔；真实图片 7d92c8a6（hash 2b5e2094…）仍正式 blocked：扇形/强弱梯度/粉蓝混色，人工认为未讲清贡献。现有 `.agents/skills/openscience-research-illustration/SKILL.md` 有空间视角规则，`apps/agent-worker/src/presentation/illustration-review.ts` 与 `generated-image-review.ts` 有图文审阅，但没稳定识别该投影矛盾；另建风格/分析模块不能解决。用户仅认可旧 cb4 的 aged-academia 暖纸墨线美感，不认可物理含义。同源六标签概念分镜 c935 已批准，真实 PNG a38bda22（hash 723e8aef…）正式 blocked：方向箭头/验讫章感；人工另发现黑盘并非空孔。art-only 修订 abe8e1a0 已成功保存私有 draft，科学字段不变；原 encoding/narration 的“小印章”与新 composition/treatment 的去印章要求冲突，未批准或出新图。先统一科学语义与视觉标记，再做真实像素核验。先核实际像素，再针对已证实遗漏在原技能/审阅提示中最小修正并验证，不把本条当成新 gate。
2026-09-25 同源修订 63e60709 在原 art 阶段三次返回不合 `scenes` wrapper 的结构，任务失败且无新方案/PNG；这不是全文理解或供应商额度故障。原 `.agents/skills/openscience-research-illustration/SKILL.md` v11 明确装饰形状与来源支持的科学含义分层、跨字段审查；原 `illustration-planner.ts` 针对单幕显式风格在新请求、保存拒绝后续跑、Gateway 内部 schema 重试给出一致的无占位 `scenes` 合同。17/17 定向测试、Worker typecheck、独立 High GO；提交 f64674d7 已推送，CI/部署/真实效果见 CURRENT。仍缺针对原文疑点的回读与像素误判防漏；这次修复不宣称已解决视觉叙事质量。
2026-09-24 标签引用断点与最小处理：私有计划 f9dc 的5标签中引用 `label 5`，d8 的11标签中引用 `label 11`，原 `parseIllustrationBrief` 只验形状/长度且 LLM 末审放过。只在现有 `apps/agent-worker/src/presentation/illustration-planner.ts` 新 science/art 产出处检查显式 `label N` 是否越界，保留历史资产读取；定向真实行为测试先复现两处红，修复后 8/8 pass、Worker typecheck exit0。此 guard 不能证明标签语义或图像质量，CI/部署与真实新任务待观察；不建第二个审阅模块。
2026-09-24 用户纠正后的风格边界：学术/编辑/淡彩仅为旧图样本，唯一目标是让未读者看懂论文。固定上游 001–277 手绘风格中 21 条缺文字特征不能自动选择；Baoyu article 23、infographic 22 可作为构图/外观资源。fd612048 已将 `auto` 接入原 science→art→render/正式审图：science 不消费目录，art 选择，后续只加载选定资源并记 provenance；独立 `styleId` 修复经 CI 35957257382/真实任务证实，6b3fd9c9 选 Baoyu article:scientific，24b25c09 选手绘 #175 并各自实际生成 PNG。没有增加第二论文分析器、风格配额或自动 provider 切换。原有配图技能 v10 的定向修订（点积投影、频率/空间间距与艺术字段逐项比对）经 CI 35962018583、发布；真实 c8f6ada6 科学字段纠正了向量同向/频率错误，但候选线被写成已辐射且带任意条数，仍不得出图。该任务 provenance 将 v10 内容误标 v9，定位为 loader 硬编码；改从已加载技能元数据读版本，经定向测试/typecheck、CI 35963964135、发布后新方案 584cf22d 真实记录 v10，且区分候选/可能通道，图片效果待验。技能正确加载与科学/画面合格依然是不同事实。
...
历史（2026-09-22）叙事链路：Hermes全文/六维/Claims及science/art沿原能力，首篇已公开。原P2034/已保存结果恢复、科学反馈隔离、几何/条件分组提示已交付；第二篇原页面继续科学修订。技术失败子集沿原GET/retry-generation补接，真实202保留已审图/父方案，创建3生图+1已有PNG的review-only，三PNG及复用审阅均实际完成；混合终态后原科学返工入口也已实见。新故障为review编辑框只保留保护前缀及art主调用timeout；分别沿原同draft/同task修复，候选与实际部署见CURRENT。Gateway/broker复用private/started分开排队与10分钟执行，单oneshot不叠加慢恢复和新生成，旧deadline不改；不保证任意积压或多生产者。后两篇reader/新发布未完成。同图跨任务连续改稿未传parent task至spool、新assistant图片锚点未接；不冒称原会话改图，不另建会话平台。精确版本/状态/证据见[CURRENT](../handoff/2026-09-10-hermes-web-image-handoff.md)。
...
2026-09-22补接：第三篇实际5张淡彩图/6Pro完成2通过3拒收；父RO公开历史误挡新草稿与共享画幅要求已上线。两篇generic恢复创建任务后被共享parser遗留单图限制拒绝，零新模型调用。已复用parser至创建端、修原1–6范围，并沿原retry-generation仅恢复已证明零提交的原任务/原授权，禁止managed task单独retry；独立High PASS并交付，原页面两任务HTTP202恢复；精确状态及后续结果只见CURRENT。同一SourceMap全文已核实Box1方向索引及导言类比，原MQED疑点撤销。新正式6Pro发现FDT/电流至矢势关系、相位匹配方向图示等科学问题；另有原图审阅会话缺失完整正文的技术故障，先上传再填正文并逐字复核已随独立provider交付。原整组科学修订已支持有正式科学拒收的混合终态；自然broker补齐not-submitted事实凭据，080原页面完成只读审图结果恢复，两篇均由原入口进入新科学修订。尚待新实图与完整公开结果，实际状态见CURRENT。
...
| 科学视觉清晰度指导 | `.agents/skills/openscience-scientific-visual-clarity/SKILL.md` → `apps/agent-worker/src/skills/installed-media-skills.ts` 的 science/plan 固定章节与既有 designSkills provenance；基于 [K-Dense scientific-visualization](https://github.com/K-Dense-AI/scientific-agent-skills/tree/49c6e97775eaa18ba791bebe23162a70ae601c18/skills/scientific-visualization) MIT 方法适配 | 本机 `find-skills` 与项目 Skills CLI 早已存在，搜索本身不等于产品加载。新规则针对定量/概念符号区分、颜色冗余与未读者可理解性；独立High指出拆场景必须由science决定且末审原100k来源预算不可挤占，修复后PASS，正式review保持不变。不接上游脚本/OpenRouter、新模型阶段或数据绘图器。326f已部署、尚未观察真实任务消费与质量收益；固定输入来源窗口、正式5.6Sol审图和跨任务经验缺口仍在，详见CURRENT |
| 手绘风格匹配与表现 | `.agents/skills/openscience-handdraw-{router,style}/SKILL.md` → `installed-media-skills.ts` 的原 art plan / image render；适配 [handraw-style-router](https://github.com/lacemou/handraw-style-router) 与 [handraw-style](https://github.com/yang0/handraw-style) 的 MIT 方法，保留固定 provenance | 仅学术、编辑、淡彩；无上游图库/脚本、新 provider 或中途选择。v1三种真实图中仅编辑图获5.6Sol内部GO；学术图标尺端点错、淡彩图77→777 nm/多余波瓣均NO-GO。v2针对这三类通用失败补绘制指导，5项本地定向测试、worker类型检查和CI 35840121221通过，e0cf已部署；v2学术/淡彩真实重画仍误放20 nm标尺，淡彩另添无来源因果箭头，均NO-GO。技能消费或提示词不保证像素正确；定量元素需确定性绘制或产品像素门禁。正式产品审阅未接线。旧scene-image/presentation-generation fixture/断言及主线CI lint为单独债务，详见CURRENT。 |
| 候选画面审阅 | presentation/handler.ts → illustration-review.ts → gateway.reviewScientific；正式 PNG 审阅由 generated-image-review.ts 注入来源/技能，经 Chat 网页 5.6 Sol 看真实像素 | 第二篇编辑图及第三篇双幕均有正式 accepted 收据并公开；旧 MOED 错图出现正式审图假阳性，人工看图阻止发布，保留收据。新的逐字缩写/公式提示已上线，但不能证明消除误判。原来源、管理员、任务、PNG hash 和未提交证明门禁仍保留；见 CURRENT。 |
| Chat图像生成 / 参考图 | presentation/scene-image.ts → gateway.generateImage → infra/chatgpt-browser/ | 第二篇编辑图和第三篇两张同父淡彩图已真实生成并通过公开读者页；旧错图及 NO-GO 草稿继续私有。2026-09-24 CDP 在 22 页、约 6.49/8 GiB 时无响应，确认无活动任务后仅重启浏览器容器恢复；未根治上游内存问题，未自动切 provider 或重发 unknown。 |
...
| 公开发布 / 标准API | apps/api/src/routes/publications.ts、research.ts；Domain publication-evidence.ts 与固定版本快照 | 三篇已从站内入口正式审阅/发布，匿名读者页读取六维和对应已批准图；第三篇两张图轮播均 1280×720，PDF 仍 workspace_member 无公开下载。固定旧版本受保护，用户最终叙事/审美认可待反馈；精确链接与 release 见 CURRENT。 |
...
叙事科学与美术边界（2026-09-21）：ab456/de847真实图片均经6Pro blocked/null，M3修订后accepted仍不能代表像素质量。已部署science/art adaptive、拒收反馈只作问题、叙事终审只accepted/blocked、可见主旨标签与reader说明；5226在science阶段先16K截断，再prompt>1500，后encoding201>200失败，未到art/终审/图片。上一轮总长反馈修复只帮助满足旧限制，不能证明旧限制合理；原任务和调用证据只见CURRENT。
制作说明容量纠正（用户明确继续）：历史1500来自MiniMax image-01（官方控制台 https://solutions.minimaxi.com/debug/image 仍为该值），不能作为Chat限制。已将1500留供应商实现，Chat/Codex按原16KiB完整JSON字节资源保护，inner job沿用32KiB；compiler保留科学、已审art和repair，只按剩余字节裁可选风格指导。内部message/subjects/encoding/constraints/composition/treatment归一为原4000整份brief预算，图内labels/reader标题说明另按可读性约束；没有新身份、schema、模型或恢复槽。主要差异High静态PASS；独立receiver引用收口纯协议模块，避免总入口拉入新Undici而在仅含dist的bundle缺依赖，增量High静态PASS。先receiver后应用部署完成，独立receiver自然调度exit0，精确release读回与原页stopped39已观察；较长说明的真实模型/图片效果未验证，未新增付费请求。
拒收与渲染恢复：已有science/art checkpoint、原拒收、同任务art-only修正及预提交冲突恢复共用原Domain审计与worker私有证据；真实5226方案现accepted，Chat图763b88c3已生成并由6Pro看图。该次仅因首零参考线样式与背景不符而拒收，未新增科学缺陷。候选沿原retry-generation一次追加一张图，来源/已批计划/原图像素审阅精确绑定；数据库仅扩展grant格式，实际授权由同事务收据决定，不新增分析器或固定15/17额度。代码、部署与实际结果见CURRENT，历史证据保留交接/Git。

Worker关闭（2026-09-21）：旧信号仅停journal计时器而主poller继续，使正式发布等待600秒。已把停止新claim、已开始claim/handler/journal租约/cleanup排空和Redis/Prisma关闭收口到原入口，部分初始化失败也清理资源；API先close再关依赖。High静态PASS并正式构建部署，已观察空闲服务正常停止，在途模型排空尚未实际观察，不为补证重启或演练。
...
自动编排已部署：`research-run.ts`新增显式visual-narrative-v1初始授权；最多9项后续分析/审阅/媒体任务，旧profile行为保持。内部保存/证据核验/素材批准均走原Domain权限、CAS和来源约束，系统审计与用户确认分开。现有extract默认v4自检没有最终atomic Claims，旧canonical稿还可能缺六维字段；复用既有SourceMap composition与reviewOnly最终审阅，各最多一次，不手填科学内容。`automatic-review.ts`消费严格已审结果，`saved-source-commit.ts`统一人工与系统版本历史读取；确切key重放不回退其他run。源编排已完成High静态增量审查；最终v5仍为既有Gateway model self-check，不能称Chat独立科学审查。站内guide已成功；首次run创建因旧DB grant check拒绝新profile/9组合而500，210200向前迁移经High复核并已部署，原幂等请求恢复HTTP202、自动进入既有全文composition。实际composition六字段完整，但v5来源审查主请求300002ms超时、备用key401，无有效审校、无新图。已部署最终来源审查65k/600s和primary-only（不在不确定提交后换key），并用review_unavailable区分服务失败，科学内容不放行；High增量复核与服务器构建/启动完成；只读刷新原run无POST，未新请求验证600秒内响应。旧run可能已计费且按逻辑task计数，不能据ALL_PROVIDERS_FAILED自动重试或另建run绕额度。用户已授权一次可能计费的补审。本轮在原retry-generation实现同run一次来源恢复：服务审计及原v4/SourceMap身份、CAS与真实新task收费同事务，保留失败ordinal0、新增ordinal1，不重置总额度；分镜按剩余任务预算提前限图数并保留修订余量。独立High静态审查已收口主尝试故障识别和已保存后重启恢复两个边界；服务器构建部署已完成，从原run页面点击一次恢复HTTP202，新审校仍服务失败，仅主请求300638ms/provider_error，无fallback或有效审校。新证据：生产Node22.23.2/Undici6.28.0底层headers/body默认300秒，provider只配上层Abort600秒；catch丢失cause不能确证历史具体HTTP错误，请求级传输时限及白名单错误枚举已补并部署（复用锁文件已有Undici7.29.0、只对>300s请求用独立Agent且finally销毁），High静态复核/服务器构建完成，原run刷新POST0；修复后零新增模型，600秒内有效响应未实测。完整图组未产出，不能把任务结果保存succeeded当科学通过。

实际图片审阅与阅读已部署：`generated-image-review.ts`经既有Chat review spool发送保存图片的原字节；schema3支持PNG/JPEG/WebP≤10MiB、≤8192边、≤40MP，原图超规格只排除自动reuse，资产保留；schema1不扩限。先存draft再审、同身份重放、只有客观渲染缺陷可作有界修订，科学问题保留并停止。`historyMedia.reader`复用既有快照保存title/narration/order并入发布hash；私有Overview按精确run.versionId展示整组，旧快照不回写。用户收到完整结果后反馈，中间没有审批关卡；自动approved不等于审美认可或公开许可。真实Chat图片及6Pro附件审阅已有证据；763b88c3因两项客观渲染偏差拒收，尚未进入最终reader/新版本发布。静态审查和模型调用成功不能替代成品质量。
