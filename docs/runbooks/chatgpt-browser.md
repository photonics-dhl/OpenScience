...
- CURRENT 2026-09-11：应用release1cdd4602 / rollback17ebc7f7，browser provider f4832487；本轮移除旧工作台流程表单，自动保存与Hermes对话制作/审核/公开回调已部署，未安装新服务；批准的工作台/公开阅读与Hermes对话已部署，浏览器及登录复用。104条Evidence已确认，v6无合格图片/未发布。截图缩放后输出异常已停止重复尝试并恢复原视口；不据此判定Chat或产品不可用。完整状态见CURRENT handoff，下方旧任务不覆盖本段。
