...
2026-09-17历史release清理（用户授权，原因：磁盘达112G/148G=80%，判为不健康）：active `c1b895ca735fd81d4803e2b32c47e55a4bbab89e` / rollback `24ee226757e27ac62339182823424b7a78baa22c`。清理前83个release目录共43G（每个约2.9G，node_modules经pnpm硬链接共享，单目录`du`为表观值，跨目录实测43G）。根因是开发工具把不可变release目录当配置源挂载，release一旦转为inactive就被钉住，导致`--prune-unused`之外的目录持续累积。先解除依赖：catalog原从 `releases/e02c391d…/infra/development-platform/catalog/catalog-info.yaml` 单文件挂载，复制`compose.yaml`与`catalog-info.yaml`到稳定路径 `/opt/openscience-development/catalog/source/`，用 `CATALOG_IMAGE_TAG=e02c391d… CATALOG_SOURCE_DIR=/opt/openscience-development/catalog/source docker compose --env-file /dev/null --project-directory … up -d --no-build --force-recreate` 重建，`query.mjs codex entity component:default/agent-worker` 仍返回实体、restart=0。随后在FD9锁内执行受控清理事务（复用官方API，不绕过保护）：`journal-start --phase prepared` → `journal-update --phase published` → `retention prepare --prune-unused 1` → `journal-clear` → `retention complete`；顺序不可调换，因为prepare要求journal为published，而complete的publishRollbackMarker要求journal已清除（与真实部署事务一致）。计划81个release/0个image tag/56个capability，全部删除；工具的active/rollback排除与"被任何容器挂载即拒绝"保持生效。结果：release目录83→2，release总占用43G→3.6G，磁盘112G/80%→67G/48%；标记、无残留journal/pending/failure、11个生产容器healthy、公网`/__release` 200。

2026-09-17开发栈磁盘治理（同上授权）：①陈旧`openscience-*`镜像19个tag（media-demo/eval系列、serena与backstage与gateway-audit历史tag、chatgpt-browser `initial`/`executor`、`openscience/tts-audition`、`openscience/python-ml-cpu`），保留active/rollback/被容器引用者与全部第三方基础镜像，`docker builder prune -af`另回收构建缓存（`docker system df`曾将9.4G缓存低报为1.35G，不可信该字段）；②开发Langfuse的ClickHouse卷6.0G中，`system.trace_log`独占4.74G/3.04亿行而Langfuse自身数据仅208KiB，已TRUNCATE系统日志表并对`trace_log`/`text_log`设3天TTL、`metric_log`/`part_log`/`query_log`/`error_log`/`asynchronous_metric_log`设7天TTL（卷6.0G→112M）。注意 `query_profiler_real_time_period_ns` 属user-level设置，写入config.d会使容器启动失败（`Code: 137 UNKNOWN_ELEMENT_IN_CONFIG`，须置于users.xml的profiles），故改用表级TTL这一受支持且持久的方式；该次实验的配置已按备份逐字节还原。`/opt/openscience/node_modules`（2.5G）无容器引用，`/opt/openscience-evals`（1.6G）与netdata缓存（2.37G）按既有审计仍为INVESTIGATE，本轮未删。
