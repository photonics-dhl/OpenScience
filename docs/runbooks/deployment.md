...
- 来源支持失效恢复与前次缺陷补接（执行状态见CURRENT）：仅现有accepted父方案在原Evidence仍通过、但同一SourceMap确定为heading时，原GET/retry-generation可按`narrative-source-support-replan`授权同run的1计划+N图。复用原receipt/source identity/CAS；新计划不继承旧科学底稿，旧approved资产与unknown提交保持。若此source-support新稿被正式科学拒收，原plan-revision沿同root proof继续，不要求伪造pixel拒收；history仍最多两层科学修订，逐跳身份与原图片预留保持。approval按source-invalid cause跳过旧pixel拒收读取/图片supersede，普通pixel分支不变。worker将已核身份的来源问题或正式前次拒收传入同一次末审，报告不作科学依据。无迁移、Gateway或receiver更新，原M3计划末审保持；正常no-tests/skip-migrate构建启动。新收据消费后保留兼容Domain/worker、优先前向修复，不清审计或重发旧任务。原页面真实恢复和质量结果单独记录；未启用的Chat末审候选不在本发布内。
- 保存方案末审续接及配图来源投影（候选/执行版本见CURRENT）：正常no-tests/skip-migrate服务器构建启动，Gateway与Domain/worker有改动，不能复用旧dist；worker私有续接context不进入Chat spool，无独立receiver更新。原STORYBOARD_OUTPUT_RESUME/retry-generation按完整checkpoint、当前执行最后的primary review timeout及不变source/base/root/任务授权作显式同task新执行，charge1/逻辑任务数与预留不变，保留旧unknown；从16K/600秒开始，只有实际length才升级32K。旧pixel特殊32K/charge0路径优先兼容。入口核原始提示身份，JSON/schema拒收沿原loop携带失败输出与结构反馈；每次付费提交前重验记录、当前权限、实际provider/model与原候选。完整checkpoint私有executionAttempt仅标识本执行新产生的方案，预存方案重用必须有恢复收据，兼容旧exec2/exec3；重复client key/digest不再次dispatch，变更请求冲突。无完整checkpoint/来源变动/活动任务/未知中间调用不放宽，partial-art与NULL restart保持各自规则。配图SourceMap heading仅投影context，不改Evidence/公开快照；旧无效科学稿须返工，图片的技术重发仍要求not_submitted。新收据消费后保留兼容worker，优先前向修复，不清资产、审计或marker。
- Pixel科学返工的art超时恢复（执行版本见CURRENT）：仍沿原storyboard_planning_retry，同task一次exec2、原max/图片预留不变，只有精确science成功+art主调用timeout两审计及原pixel/source/scene身份才可用；priorSubmission保留超时结果未知，禁止伪造未提交或science checkpoint。无迁移，正常无测试服务器构建启动；授权执行后保留兼容worker，前向修复。review正文缺后缀的候选只在未发送同draft一次完整重填并重验全部守卫，不延旧deadline；独立provider仍用原空闲/三锁/备份安装，不重启浏览器。
...
- 叙事拒收根因修正：science/art沿原调用启用adaptive/300秒，token与重试上限不增加，实际单次消耗可能增加；审图反馈只报告缺陷，科学关系必须回原始证据确认。新叙事终审仅accepted/blocked，不再改写候选后自证；旧非叙事revised及已存完整候选验收仍兼容。labels沿原0–6上限保留一个来源支持的可见主旨，详细解释留reader；无新增分析器、模型步骤或provider。
- 已完成像素审阅的持久化恢复：无迁移、无新模型请求，正常应用构建/启动后沿原retry-generation续接；只允许唯一原completed-review收据绑定的exec2/已知Version写冲突进入exec3，既有PNG及来源身份保持。worker保存只重试P2034事务；失败step按实际任务全量收尾。已有完整job result的provider恢复直接经原校验发布，不重开会话。独立provider按原空闲/三锁/备份流程安装；开始exec3后保留新收据与结果，优先前向修复，不交给仅理解exec2的旧worker。精确部署与证据见CURRENT。
- 已接受计划仍被真实图片审查拒绝的续作：用户明确追加2项后，原grant/retry仅对完整且来源未变的11项谱系提供一次11→13；唯一旧授权/续作、accepted父计划、rejected图片及6Pro收据共同绑定。新plan清除旧修订引用并绑定当前拒收图片，只可再建1方案+1图，无剩余绘制修正或自动下一轮。迁移20260922020000保留旧组合，部署前双库备份，正常deploy不可skip-migrate；默认新run仍9。部署不自动提交模型，原页明确授权后继续一次，未知响应先查收据与run。已有13授权时数据库rollback拒绝收窄；在途13不得交给只理解11的旧worker，保留所有审计/历史资产并优先修复前进。
...
- 600秒文本传输补接：生产Node22.23.2内置Undici6.28.0仍默认headers/body各300秒；仅延长Abort不能覆盖。`ai-gateway`直接声明锁文件已有的Undici7.29.0，超过300秒的文本请求使用本请求Agent并在finally销毁，保留总Abort上限；只记录白名单transportCode。无迁移、无新模型/receiver请求，正常服务器构建部署后仅读原失败run确认未自动重开。应用回退保留所有任务/审计；修复后的长付费响应尚未实测，历史provider_error不能反推原始底层错误码。
- 来源审校职责修正：既有scientific-critical-thinking v3区分综合与核源指令，终审只因有证据的科学/来源差异修订；仅DB绑定visual-narrative来源审校要求输出有依据的核心Claim，普通人工v5仍可无建议。无外部协议、授权、额度、迁移或receiver变化；仍用正常部署的`--no-tests --skip-migrate --reuse-unchanged-capability-images`，部署完成仅读原失败run/任务身份，不自动补审。回退应用不改任务/审计/六维初稿，候选科学质量及延迟改善均需后续真实响应证明。
- 显式来源补审：同一run连续review谱系取代固定ordinal1，每次用户动作只创建一个新收费任务，历史尝试计入原9项并保留3项后续空间；未知provider_error仅可在完整无有效响应证据下显式补审，不作为自动瞬态错误。原始clientIdempotencyKey写入已有审计防改version重放，历史未记录原key的限制保留。无迁移/receiver变更，按正常no-tests部署。新增ordinal任务执行期间不能回到只认ordinal1的旧worker；回退需等当前任务结束并保留所有task/step/审计/原稿，不重发。真实验证只点原run一次并先落私有收据，未知响应先读状态。
- 六维已接受但主张缺失的合同恢复：仅原稿/全部证据未变、六字段accepted且无issues/补证、Claims属性缺失、成功Gateway审计严格匹配的前驱可沿原显式恢复追加review；成功审计与服务失败分开记录。后继一次schema耗尽仅在紧接唯一有效accepted锚点、原稿保留且1–2条成功主调用匹配时允许明确恢复，绑定锚点/失败任务/审计，仍保留原9项预算与3项后续空间，不自动重试。v5来源片段按P边界精确拆分，受控拒收收据只作私有证据，不成为已审Claims。无迁移/receiver变更，复用上述部署参数；等当前任务结束才回退，旧代码无法验证该前驱，不把在途任务交给旧worker。部署后沿原run一次性收据执行已授权恢复，不改历史结果或公开版本；模型新结果仍须全量来源与Claims校验。
- 既有流程入口修复：同actor/RO/source的只读找回、重复自动run创建拒绝及公共refresh绑定保护；无迁移、无receiver/模型变化。正常`--no-tests --skip-migrate --reuse-unchanged-capability-images`部署后从站内Hermes入口观察找回原失败run和零POST；不通过发起收费请求验证拒绝分支。应用可回退到本次部署前release，数据/失败记录不回写；回退会恢复旧入口缺口。
...
- 清理事务（FD9 锁内复用官方 API，未绕过保护）：`journal-start --phase prepared` → `journal-update --phase published` → `retention prepare --prune-unused 1` → `journal-clear` → `retention complete`。顺序不可换：prepare 要求 journal 为 published，complete 的 publishRollbackMarker 要求 journal 已清除（与真实部署事务一致）。计划并删除81个release/56个capability；工具的 active/rollback 排除与"被任何容器挂载即拒绝"始终生效。
- 其余已清：19个陈旧 `openscience-*` 镜像tag（保留 active/rollback/被引用者与全部第三方基础镜像；**回滚镜像不可删**——事务第595行回滚路径执行 `docker image inspect openscience-agent-worker:$PREVIOUS_RELEASE_SHA`，缺则 `rollback_ok=0`）；`/opt/openscience/{node_modules,apps}` 过期构建树（7-28/8-18，零引用，部署事务只用 REMOTE_ROOT 的标记文件）；`openscience-evals/local-image/flux-2-klein-4b-Q4_0.gguf.part` 1.09G 未完成下载；154个 private-cleanup 陈旧副本；Playwright 宿主缓存923M（零引用，可再生）；开发 ClickHouse 内部日志（见下）。
- **结果：112G/80% → 56G/40%（可用30G→86G）**；release目录83→2；标记完好、无残留journal/pending/failure、11个生产容器healthy、公网`/__release` 200。
- 保留（有引用，勿删）：`/opt/openscience-models` 4.9G（docling 611M 由 paper-analysis 挂载；qwen3-tts 4.3G 被 `infra/codex-image-runner/video-runner.mjs`、`video-tts.py`、`install.sh`、`infra/tts-audition/audition.py` 引用）。
- 未决（需策略决定，本轮未删）：netdata 监控卷 2.37G（含 dbengine 958M，属监控历史，既有审计标 KEEP）、Serena `code-intelligence/build` 1.1G（可再生但为工具工作目录）、`openscience-chatgpt-browser/jobs` 166M（受 private-cleanup 授权流程管辖，不擅自删）、`/var/backups` 333M（7套已校验数据库备份，KEEP）。
...
- Phase B first materializes the exact candidate, performs the server install/full build and exact Worker/Parser image build without switching production, exports the canonical 16-case corpus, and runs `accept-document-parser-release.sh <sha>` to produce the schema-v3 report. The current profile is `hermes-parser-13-3-v3`; it requires exactly 26 deterministic structured calls for the 13 successful single-window cases (source bridge plus scientific composition) and exactly three evidence-bound reviews: corrupt PDF, native formula PDF requiring visual transcription, and empty PNG. The formula review must retain both locators plus native `equation-markers.1` equation provenance; all prior schema/profile reports, including v1 and `hermes-parser-14-2-v2`, fail closed but remain historical receipts. The only success marker is `TASK8_PARSER_ACCEPTANCE_OK`; require the fixed report path `/opt/openscience-acceptance/document-parser/<sha>/report.json`, zero owned temporary containers/volumes/run directories, and unchanged production release.
