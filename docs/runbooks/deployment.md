...
- 六维已接受但主张缺失的合同恢复：仅原稿/全部证据未变、六字段accepted且无issues/补证、Claims属性缺失、成功Gateway审计严格匹配的前驱可沿原显式恢复追加review；成功审计与服务失败分开记录。无迁移/receiver变更，复用上述部署参数；等当前任务结束才回退，旧代码无法验证该前驱，不把在途任务交给旧worker。部署后沿原run一次性收据执行已授权恢复，不改历史结果或公开版本；模型新结果仍须全量来源与Claims校验。
