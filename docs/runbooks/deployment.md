...
- 托管叙事science/art阶段保存（执行状态见CURRENT）：复用AgentTask.result的私有`storyboardPlanningCheckpoint`，science经原materializer验证后保存，每次实际art提交前CAS为submitting，异步保存有界JSON/schema拒收正文。恢复沿原retry-generation，同task明确art-only或无中间产物的完整重规划，不新增逻辑任务/表；未知提交保持不可自动恢复。完整方案原子替换partial，公开投影隐藏中间结果；独立单图与既有完整checkpoint路径保持。Gateway逐次提交回调复用原内部授权并在失败时停止全部重试，必须正常构建Gateway及worker，独立Chat协议不变。无迁移/receiver更新，待原请求自然终态后按no-tests/skip-migrate正常构建启动；新增partial/收据已消费时保留兼容worker，优先前向修复，不回退到不识别partial的旧代码或清掉中间结果。实际恢复与产物质量另记，不以静态审查或构建代替。
- 来源支持失效恢复与前次缺陷补接（执行状态见CURRENT）：仅现有accepted父方案在原Evidence仍通过、但同一SourceMap确定为heading时，原GET/retry-generation可按`narrative-source-support-replan`授权同run的1计划+N图。复用原receipt/source identity/CAS；新计划不继承旧科学底稿，旧approved资产与unknown提交保持。approval按source-invalid cause跳过旧pixel拒收读取/图片supersede，普通pixel分支不变。worker将已核身份的来源问题或正式前次拒收传入同一次末审，报告不作科学依据。无迁移、Gateway或receiver更新，原M3计划末审保持；正常no-tests/skip-migrate构建启动。新收据消费后保留兼容Domain/worker、优先前向修复，不清审计或重发旧任务。原页面真实恢复和质量结果单独记录；未启用的Chat末审候选不在本发布内。
- 保存方案末审续接及配图来源投影（执行版本见CURRENT）：正常no-tests/skip-migrate服务器构建启动；Gateway有改动，不能复用旧dist，无独立provider协议更新。仅原pixel art-timeout恢复后的完整checkpoint及16K耗尽→32K超时审计可授权exec3，只续末审32K/600秒、primary-only、不新增逻辑任务，原超时仍为结果未知；每次provider提交重验收据。配图使用原SourceMap识别heading并投影context，不更改Evidence/公开快照；旧无效checkpoint拒绝续末审，必须走科学返工。混合终态科学返工允许将旧uncertain/submitted_without_result保留在原收据中，仍须真实科学拒收；不重发原任务、不延deadline、不影响技术恢复的not_submitted要求。新增收据消费后保留兼容worker，原资产/历史保留，优先前向修复。
