...
| 网页远程桌面 | `infra/chatgpt-browser/`；服务器 `/opt/openscience-chatgpt-browser` | 镜像已构建（272a5ed57d27），桥接服务active；浏览器容器启动报运行时错误，已停止自动重启。Chrome/Xvfb复用上行，仅补远程桌面组件依赖。未证明登录、生图、回传成功；入口见 [浏览器手册](chatgpt-browser.md) |
