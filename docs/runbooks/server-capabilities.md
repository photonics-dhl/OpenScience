...
| 能力 | 已有位置 / 入口 | 状态与复用方式 |
|---|---|---|
| 生产应用 | `/opt/openscience`；`openscience-prod-{web,api,agent-worker}-1` | 本次列表显示运行；release `c2d11326`，rollback `7f8e47d9` 继承 CURRENT 记录。不要混用本地候选与线上版本 |
...
| 网页远程桌面 | `infra/chatgpt-browser/`；服务器 `/opt/openscience-chatgpt-browser` | 镜像9f44e1267fba复用完成（加入既有Playwright-core），桥接active，浏览器running，noVNC HTTP200；启动seccomp兼容故障已修。用户已登录，服务器Playwright执行一次网页生图并保存PNG成功；通用runner与生产Hermes队列尚未整体接入。Chrome/Xvfb复用上行，仅补远程桌面组件依赖。已证明登录与单次生图保存，未接产品回传；入口见 [浏览器手册](chatgpt-browser.md) |
