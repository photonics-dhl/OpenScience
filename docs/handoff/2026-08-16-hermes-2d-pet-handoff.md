...
> **CURRENT active-memory，2026-09-01。** 公网 release `2019f8a` 全绿、rollback `9eeb8d5`；Cookie 快照路径已部署，但此前 CARSI `ready` 是 publisher 403 false-positive。独立 strict CPU browser spec 已批准、计划 `cf114e7` 完成并进入 TDD；机构 PDF 未完成，Task 11 阻断，Landing/Hermes 视觉冻结。
...
- Candidate branch / plan commit / origin main: `codex/scansci-browser-institutional-gate` / `cf114e7` / `2019f8a2d9dcd7d5241de231ae95e2aedf238f47`
...
27. PR #28 / CI `33461988607` 合并并 canonical 部署 `2019f8a`，快照 `cache_dir` 已正确；生产取证证明 pinned 登录把 publisher 403 当成功且 `try_carsi` 要求缺失 browser runtime。用户已批准独立 UID/container、相反只读 job tmpfs、无 fallback Patchright、browser proof 设计；唯一实施计划 commit `cf114e7` 已完成，TDD 执行中。
...
1. 按 `cf114e7` 计划 TDD 实现 browser job protocol、严格 Patchright adapter、proof-backed session、独立 browser image/topology 与 release gates。
