...
- 新范围覆盖 Personal Workspace 的 Dashboard → RO 创建 → 导入/六字段 SDF → Hermes diff → Commit 主链；2026-08-17 已在 ECS 以真实 arXiv PDF 与 MiniMax-M3 贯通并部署，未改数据库 schema。正式任务继续复用 credit/queue/audit/approval、提交限流和服务端上下文授权。
...
- 用户浏览器所在 Windows 当前关闭系统动画，Codex Chromium 因此命中 reduced-motion；旧录屏强制 `no-preference`，不能代表用户页面。当前唯一动态验收入口是 `http://127.0.0.1:3194/dashboard?hermes-motion=full`：该显式偏好同时恢复 mesh、CSS 整体动作和 pointer input；无参数时仍尊重系统并保持静态。禁止再交付使用自动化 media override 的视频作为用户环境证据。
...
- RO 创建和编辑页注册语义锚点；bubble 提供 Explain/Draft/Check/Take me there。Draft 复用 `sdf.extract`，建议在显式 Apply 前不改 SDF；同步重复 Draft 只创建一个 task。
...
- 尚未完成：Drawer 内 Quiet/Balanced/Active 与 sound/particle/proactive 设置 UI；两信号主动提示与逐字段 cooldown 的完整浏览器矩阵。不得把这两项写成已交付。
...
1. 继续 Task 9 的持久模型审计独立断言、Task 10 的逐字段 accept/edit/reject + source UI + 六字段 gold rubric，以及 Task 11 的同一真实流程逐状态角色像素动作归因/reduced/resource cleanup。
