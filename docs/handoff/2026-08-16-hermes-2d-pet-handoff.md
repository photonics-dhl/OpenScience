...
- Docling `2.123.0`/MIT 候选已 source-lock 到官方 wheel SHA-256 `95c0a4d…fde9c`，本地无 Docker runner `3/3`、评测脚本 `14/14`、Bash 语法 GREEN；OCR/remote/plugin disabled，模型只在 ECS image build 获取并生成 package/model lock。首次 ECS build 因临时 wheel 文件名不合法而在 corpus 前 fail-closed，已改官方完整文件名；尚无 image/质量/RSS 证据，下一步必须从修复提交 SHA 重跑同一 7-PDF gate。
