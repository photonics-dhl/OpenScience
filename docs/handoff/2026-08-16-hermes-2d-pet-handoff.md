...
- Tasks 1–7 本地实现与审查已完成；reviewed implementation `c8903d5` 已推送，包含 evidence UI、durable checkpoint、幂等恢复、一次安全 retry、真实可读性/blank-RO 门禁与 migration 27。
...
- 当前 next action 是只读 ECS checkup 与 deployment dry-run；无新的云写确认不得备份、迁移或部署。
...
- Remote candidate: `origin/codex/readable-hermes-guidance`（部署前从 clean branch HEAD 解析完整 SHA）
- Local main: `c60ffdd`
...
2. 运行只读 ECS checkup 与带 `--rollback-ref` 的 dry-run，核对版本元组。
