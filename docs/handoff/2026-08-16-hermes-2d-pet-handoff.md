...
- Branch / local implementation / repository main: `codex/scansci-default-capability` / `c0bcf0a0af4cf4f58d62303b9fa5ceeb947eeddf` / `463c8e3a2a80138cda2d669c370c0481ed4c0877`；远端仅保留 `main`。
...
20. Task 8 fix round `c0bcf0a`：explicit A key/fingerprint 在 generic target history 前 exact replay，active B 不得 suppress/replace A；browser A-vs-B `5/5`、Web `464 + 5 Node`、product `72/72`、Hermes `19/19 + 8/8` green。Task 10 部署前须只读确认 targetless durable ScanSci tasks = 0；非零阻断，不自动 migration。
...
2. Task 8 四入口本地候选已完成；继续 Task 9 安全/UX 复审与 Task 10 ECS 真实 OA/CARSI、375px、一次性下载验收，不重做 Task 7/8 状态机。
