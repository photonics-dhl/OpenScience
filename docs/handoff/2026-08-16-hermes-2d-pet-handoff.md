...
- Task 1 建立能力台账、初始 13-case 自著权 corpus 与现状 parser baseline；Task 4 已将 corpus 升为 schema-v2 16-case。生产现有隔离 parser/Tesseract/ClamAV/source-map 与 BGE-M3，但候选 layout/GROBID/PaddleOCR 尚未晋升，完整文档级联未完成。
...
- Docling `2.123.0`/MIT 候选已 source-lock 到官方 wheel SHA-256 `95c0a4d…fde9c`，本地无 Docker runner `3/3`、评测脚本 `16/16`、Bash 语法 GREEN；ECS 构建尝试未取得可接受 image/质量/RSS 证据，失败候选容器/镜像已按授权清理，不得视为已接受。
...
- 双库集合 `db-set-20260827T155422Z-1676593` 已恢复验证；core/search schema 与 data hash 比对通过，6 个恢复/测试临时库随后按用户明确授权精确清理。
