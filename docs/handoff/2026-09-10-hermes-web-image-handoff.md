...
| 第二篇编辑图 / 2、5 | RO `c896802c`，7 Claim/27 Evidence；正式 5.6 Sol approved 图 `6233e663` 被单独选入 [公开 v2](https://openscience.428312321.xyz/research/OSR-2026-000022/v/2)。匿名实见六维与图，旧 v1 保留；学术/淡彩旧候选的标尺或多余因果错误仍私有 NO-GO。实看公开图参数清楚却偏设备结构说明，尚未一眼讲清“空间局域→短时间脉冲”。 | 已从公开 v2 同源恢复私有 v13 `fdd7ef10`，原 SDF/7 Claim/27 Evidence 复用。Hermes auto 初稿 `52ae14c2` hash `1f2d27d7…` 八标签、公式过密且配文先报数字；普通科学修订 `7a8fc8df` 被原末审 blocked：单幕 120 字配文容不下机制、单电子虚拟限制和独立电子团工况/产额权衡。下一步将两类科学关系分成两幕，先核方案与画面，v2 不动。 |
...
| 能力与历史 / 4 | 生产 91ab 的 `installed-media-skills.ts` 原 science/art/review 阶段消费配图 skill v13；`generated-image-review.ts` 空间语义核对已在第一篇 blocked→修正→accepted 的真图上验证。第二篇再次出现标签超预算/机制配文被指标挤占；原 skill v14 的最小规则候选已通过本机 6 项定向加载测试与 Worker typecheck，独立复核、CI、部署、真实 v14 任务均待完成；未建新模型阶段。 | 原文疑点定向回读、跨任务经验检索、定量几何核验尚未完成，见[能力台账](../runbooks/hermes-capability-registry.md)。Fig.2 重复 plan 与悬空 `d5087b03` copy 未清理，先按原只读影响面与授权边界处理；Fig.1 原字节展示已被否定。 |
