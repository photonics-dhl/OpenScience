...
## CURRENT 体验改造进行中（2026-09-11）
- 新任务：研究桌面、编辑器资料入口、公开阅读/探索/首页及跨页会话；Hermes能力扩展在这轮交付后，不重生图/视频。
- 候选基于HEAD19e3e765；服务器实读仍378a9506，rollback9c5088ea，provider d1630135不变。
- 代码已改：有效/me续Cookie、跨路由会话/跨标签失效同步；任务分组与可见后台刷新；编辑器一个资料入口；公开大图+精炼正文；探索thumbnail同公开版、首页真实成果。
- Chat6Pro本轮完整产品分析已取回并采纳，来源同下方会话；独立Sol High已指出并修复续期接线、401/账号切换竞态、缺失字段确认入口。未运行测试。
- 19/20/21确定Task/E2E遗留公开记录将在既有安全归档脚本内精确匹配处理；22保护。尚未执行，归档不是删除。
- 下一步：独立安全复核收尾→服务器必要build/start→实际浏览这些已变页面；尚未部署，不标完成。
## 目标与规则
