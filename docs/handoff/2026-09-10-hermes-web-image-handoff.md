...
- 工作区治理（本轮）：根 main 原为 `b9616cb9`（落后 origin/main 536 个提交）且带 47 项未提交残留。已先把旧状态固化为 `fd2ed334` 并推成归档分支 `codex/root-navigation-archive`，再把 main 重定位到 `origin/main`（`1b974dd6`）并叠加导航与卫生改动，提交 `6cceba03`；现根 main `git status` 为空、领先 origin/main 1 个提交。删除 22 个已推送的临时/部署 worktree 与 12 个已注销孤儿目录（原 40 → 7 个注册 worktree；另有 2 个已注销目录只剩被占用/拒绝 ACL 的桩文件 `hermes-3d-scholar\.tools\uv-cache\...whl` 与 `readable-hermes-guidance\.pytest_cache`，合计约 0.1 MB，需在占用进程退出后手工删除）；保留交付树、`art-direction-release-41ae8902` 部署树，以及 4 个含真实未提交工作的 worktree（`hermes-constellation-dragon`、`token-smart-live-control`、`token-smart-live-workflow`、`token-smart-task2`）。`codex/hermes-3d-scholar`、`codex/optical-native-prototype`、`codex/product-web-task1` 与交付分支均已推送 origin。`.gitignore` 新增 `.Codex/`、`docs/live_2D/`、`docs/user_ideas/**/*.{png,zip,xlsx}`；根下误建的 `token-smart-live-workflow/` 已移出仓库到本机临时归档。
- 生产身份事实（当前部署阻塞）：服务器 `.release-id` 与 `.release-source` 均为 `48814fe00640022ea188c45067994fda60508939`，运行中 api/web/agent-worker/document-parser 容器镜像 tag 也是该值；但该提交在本机对象库（`cat-file --batch-all-objects` 命中 0）、origin 全部 144 个 ref（含 PR head）和 GitHub 页面（404）都不存在，`3906b93d…` 同样。内容比对：其 `scene-image.ts` = 我方 `bdf0d3fa` 版本（sha256 `8397653b…`），`illustration-planner.ts` = `f1b8fc74…`，即生产应用代码等于交付分支 `bdf0d3fa`；其 `AGENTS.md` 比交付版多两行（本轮已并入两棵树）。后果：`deploy.sh` 要求 `--rollback-ref` 能 `git rev-parse` 且等于 `.release-id`，因此**当前无法用该身份发起部署**，`e86bd39f` 的输出窗口修复仍未上线。须按 docs/runbooks/deployment.md 故障态恢复流程先核对实际容器/Compose/release 目录再恢复可信身份，不得直接改标记或盲目重跑。
