...
- 用户已明确指定第二Chat账号并授权切换；服务器未显示Add account，已按正常UI退出旧账号。正常登录转Google OAuth，bridge先缺accounts.google.com，再缺认证脚本www.gstatic.com；仅补这两个exact host:443，Sol High逐项复核通过。桥接服务已重启active，浏览器未重启；现到Google密码验证页，等用户亲自完成身份验证，尚不能声称切换成功。不记录目标个人邮箱/密码或复制本机凭据。
- OAuth bridge单文件补丁独立于应用release/provider runner bundle；实际文件/opt/openscience-chatgpt-browser/scripts/host.mjs，回滚原件host.mjs.before-google-oauth-20260911。保持network-none、loopback UI、端口443限制，未放行Google通配符、搜索、字体或头像域。两账号仍可能分别限流，不承诺轮换永不限额。
...
## Next actions
1. 等用户在服务器Google密码页完成身份验证，按可见账户信息确认新Chat登录后再继续；fd719902已成功，勿重发。无需重跑共编/OCR。
