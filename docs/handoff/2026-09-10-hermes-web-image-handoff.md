...
## Git与部署
- application production=24b3f8c1cb5c8d1e579f3c92020f495939ba3205，rollback=e7e677180a791c19e30d05a5297319183f8e6976；canonical release/onchip-production-line的后续文档HEAD=2b252591，根main=62b83372e51285b31a65f89bc0fabeb88d7c09bf。网页审图切换[定向CI 35872438506](https://github.com/photonics-dhl/OpenScience/actions/runs/35872438506) success、应用部署exit0；Worker `CODEX_SOL_IMAGE_REVIEW_ENABLED=false`。第二篇原私有图cebcef87仍draft、hash bbfaf594；正式review-only任务6233e663的副本仍私有draft，未调用模型/未公开。手工收据修复候选待发布。
- e7e中的独立CLI审图实现仍在应用镜像内但Worker开关false；隔离服务已停用，未执行CLI正式审图。24b3复用既有 `gateway.reviewScientific`→Chat网页review broker：仅schema3图片请求显式5.6Sol，旧6Pro收据按原模型恢复；Hermes原skill/来源/PNG附件照常注入。用户授权管理员额度，经原Domain topupCredit同事务审计补1 credit（receipt 5aac0cbc），产品正式任务6233e663 HTTP202并扣1；Worker在provider前将手工副本误套托管技术收据而blocked，DB实证正式收据1、provider调用0。候选补手工收据/当前管理员验证及同任务安全重试，定向用例/类型检查及独立High GO，待CI/部署/真实审图。旧6Pro和unknown绝不重放。
- Chat provider bundle=24b3f8c1cb5c8d1e579f3c92020f495939ba3205；live review-runner及bundle review-broker与该提交源SHA256一致、两timer active，浏览器未重启。备份 `provider-backup-20260923-web-sol-24b3f8c1`；辅助脚本尾CR使外层exit127但逐文件核对安装成功，禁止重跑。旧a6a27ef5 bundle/备份保留，renderer sha256:4a30b091d4bdeb7dd7670a01521d30d7b32694e1f3b34977a2aa26e91669559f，sceneImage fallback=false。管理网页仍受Nginx Basic Auth，前次超时加额核DB未发生；本次改用同审计Domain服务端补账，未读/打印凭据。管理员长期免内部额度尚未实现；供应商额度独立。
...
- 三张新图仍为私有草稿，未有有效产品正式审图、读者页与新公开版本；本机5.6Sol独立咨询GO不能冒充正式审核。网页5.6Sol provider已接线，但第二篇首个正式任务6233e663在模型前遇到手工收据断点，待候选发布并原位重试。用户要求用好Hermes skill并展示实时开发细节；技能接线不自动重审旧图。管理员先前和本次均沿Domain `topupCredit`+审计补账，实际任务按产品规则扣额。
...
- 2026-09-23三风格真实效果：基于第二篇批准方案9a1b与原7 Claims，art-only学术1b9f、编辑99b9、淡彩修正版d3b5方案均获批准用于私有生图；三方案科学字段/标签/叙述与父方案逐项相同，provenance实录handdraw-router/style v1。v1私有Chat PNG：学术8140（656018 bytes，SHA256 6b09a42529463fc4d05518815d48637e97abc09b47a86273ee0fc64777c5de9d）、编辑61aa（713661，bce066284ab74901507a4a5a5cc547969dba2643270ec2a8f334e4ebd1df51cc）、淡彩430e（1248711，a28df766a72b6e8dd16d7edfd97d929108cac7e1c2f15282e0a63b86bba8f2d4）；本机ignored tmp同名前缀图片已核哈希并看像素。5.6Sol High内部对照仅编辑GO；学术20 nm标尺落在偏心斜侧、淡彩77误成777 nm且有无来源脉冲波瓣，均NO-GO。淡彩首份方案be15错误地让电子不接触光带，生图前修正为d3b5；be15保留。手绘表现skill v2加入数值/标尺端点/无来源装饰指导，本地5项测试、worker类型检查及CI 35840121221通过，e0cf已部署。v2学术图472a（761893 bytes，SHA256 aa3b34e890bd63d238cceeb6665cb65c3a8ddfb445ec8df2bac0039d0b050177）仍误放20 nm标尺；v2淡彩图f627（1446001 bytes，SHA256 36584741efd0b194b67eb935dab3ded83f61f67bb647110d441bbedb63b2001e）修正了77 nm及波瓣，但标尺仍偏心，且新添一条无来源的结果箭头；两图独立5.6Sol均NO-GO，均保留私有draft且未正式产品审图/公开。提示词不能保证精确科学几何；下一步应将定量标尺/标签交给确定性绘制或可复核像素门禁，不继续盲重生。原认可/旧图保留。Domain管理员按已授权额度使用幂等账本：2+4+1+2 credits，均有审计；实际任务各扣1，当前余额0。旧scene-image/presentation-generation fixture/断言及主线CI lint 72项为独立债务，定向CI不覆盖。
