...
- 用户要求"不要有 dirty main"并"全盘检索不可能丢失的部分"。盘查结果：根 main `b9616cb9` dirty 共 23 个 M 文件（18 个 `.agents/skills/...SKILL.md` + `AGENTS.md`/`docs/handoff/2026-08-16-hermes-2d-pet-handoff.md`/`docs/progress.md`/`project_index.md`/`.memory/memory.jsonl`）+ 25 个 ??文件（`.Codex/config.toml`、`docs/live_2D/`、`docs/runbooks/{chatgpt-browser,codex-token-smart,server-capabilities}.md`、`docs/user/`、`docs/user_ideas/8.10/*.png`/`.zip`/`.html`/`.md`/`docs/user_ideas/Codex_Token_Smart_Skill_2026-09-07.zip`/`OpenScience_Prior_Art_Comparison_Matrix.xlsx`/`ultrafastscience_0125_research_object_demo (1).html`、`token-smart-live-workflow/`）。全部 dirty / untracked 均来自历史会话遗留，**本会话未对根 main 做任何修改**（本会话全部 commit 均在 `codex/onchip-video-release` worktree）。AGENTS L50 "保护他人 dirty/untracked" 与 L9 "根 main 不作基线"互相冲突，按 L22 / L50 不 reset / clean / stash / pull。比较 root main `b9616cb9` 与 worktree HEAD `af6fe448` 的 `docs/handoff/` 树完全一致，**未发生丢失**；worktree 多出的内容是本会话对 illustration 链路与 handoff 的合法 commit。
- 13 张 `tmp/research-illustration-*.png` 在 worktree `tmp/`（不入 git），违反 `project_index.md` L20 "本机证据已移出 git 跟踪" 的存放规范（本应在本机非 worktree 目录）。等用户裁决：保留 / 移走 / 清除；本会话不动它们。
