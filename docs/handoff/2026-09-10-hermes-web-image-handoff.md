...
- 用户坚持链路稳定 / 不要再靠手工 prompt。已修 `illustration-planner` 两阶段 + `scene-image` `completeStructured` 显式 `maxTokens:8192, maxRetries:1`，避免被 finishReason==='length' 直接抛 STRUCTURED_OUTPUT_TRUNCATED；提交 `e86bd39f`，新增 `packages/ai-gateway/test/gateway.test.ts` 验证 `maxTokens` 透传，跑过通过。
- 部署失败两次，原因不是我的 commit：production `.release-id = 48814fe00640022ea188c45067994fda60508939`（mtime 2026-09-16 12:50:04，对应叙事图部署），但本机 git / 远端 `origin/codex/onchip-video-release` 找不到 `48814fe0…` 对象。deploy.sh 要求 `--rollback-ref` 在本地 git 可解析，本机拿不到 active SHA → 拒绝。这是既有部署 / Git 治理债务，不是 `e86bd39f` 引入。请用户提供能解析到 `48814fe0…` 的本地 ref（例如另个 worktree 的 `codex/onchip-video-release` 远端 ref，或接受当前无法部署的事实）。`chatgpt-web` 未触及额度：错误栈是 `completeStructured` finishReason==length，不是 image provider；不需换号。
- 链路真实分工（重申）：scientific intent ✅ Hermes；art stage composition/treatment ✅ Hermes + baoyu/scientific-critical-thinking 自动；语义级"暖灰/期刊/金属感"翻译 ✅ 自动；输出窗口被 finishReason==length 截断 ✅ 已修（透传 maxTokens 8192）；chatgpt-web image provider 额度未直接证据耗尽；用户审美反馈 → 下次修订 prompt 仍需 Hermes art stage 接走。
