...
## 当前锚点
- 生产 `ac79c8ea9550773981174d61b67e96cf4318d8f3` / rollback `a7379e1fe0ebc3e5179392f5ca600bf611ebeb1a`；显式结构编号与整对比例的科学守卫经 43/43、Worker typecheck、独立 High GO、[CI 36195532262](https://github.com/photonics-dhl/OpenScience/actions/runs/36195532262) success；定向部署 exit0、公网 `/__release` 精确一致。a737 起沿既有 Gateway 拒收回调写私有数值字段/来源摘要（不存候选/原文，不改任务 checkpoint）；原配图 skill v14、多幕 auto wrapper、正式像素审图沿用，不宣称全链重新验收。
...
| 第二篇编辑图 / 2、5 | RO `c896802c`，7 Claim/27 Evidence；正式 5.6 Sol approved 图 `6233e663` 被单独选入 [公开 v2](https://openscience.428312321.xyz/research/OSR-2026-000022/v/2)。匿名实见六维与图，旧 v1 保留；实看仍偏设备结构说明，未一眼讲清“空间局域→短时间脉冲”。 | 同源私有 v13 `fdd7ef10` 复用 SDF/7 Claim/27 Evidence。两幕 `0b5e5db7` 原 M3 accepted 但独立核原文 NO-GO（19/99 as、270 未绑结果、伪曲线），无新 PNG。08d 的 `0386efbe`/`a8c04e9d` 与 a737 的 `1d1ec917` 均 science 拒收；ac79 真任务 `5089edf7` 仍三次拒收：19_as_description → 1_bare_description → 19_as_source。原文 s6 确实写 `(FWHM_T, i.e., τ₁) of 19 as`，旧词法在 i.e. 句点丢失变量。新候选识别该别名，subject 可省变量但确切原文必须支持它；44/44、typecheck、独立 High GO，CI/部署/真任务待做。公开 v2 与旧任务均保留。 |
...
## 当前执行与保护
- ac79 的独立真任务 `5089edf7` 在 science 三次拒收，`AgentTask.result=null`、无资产；私有 AuditLog 三条哈希/摘要及 Worker 安全错误码依次是 `19_as_description`、`1_bare_description`、`19_as_source`，不是 Chat 生图桥失败。原文 s6 `FWHM_T`/19 as 真实同段，但旧解析在别名 `i.e.` 处截断，无法核变量；s0 的 1 MeV 另有来源。新候选仅让 subject 在有相同数值/单位时省略变量，绑定原文仍必须明确同一变量；FWHM_S 反例拒收，私有摘要仅增加白名单变量标记和首/尾数字，不保存原文。44/44/typecheck、独立 High GO，CI/部署/一次真任务待做。裸 `1` 的具体语义仍需下一轮有界收据诊断，不能直接豁免；保留全部失败收据，公开 v2 不动。
