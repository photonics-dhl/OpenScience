...
- 工作区治理（本轮）：根 main 原为 `b9616cb9`（落后 origin/main 536 个提交）且带 47 项未提交残留。已先把旧状态固化为 `fd2ed334` 并推成归档分支 `codex/root-navigation-archive`，再把 main 重定位到 `origin/main`（`1b974dd6`）并叠加导航与卫生改动，提交 `6cceba03`；现根 main `git status` 为空、领先 origin/main 1 个提交。删除 22 个已推送的临时/部署 worktree 与 12 个已注销孤儿目录（原 40 → 7 个注册 worktree；另有 2 个已注销目录只剩被占用/拒绝 ACL 的桩文件 `hermes-3d-scholar\.tools\uv-cache\...whl` 与 `readable-hermes-guidance\.pytest_cache`，合计约 0.1 MB，需在占用进程退出后手工删除）；原 4 个含未提交工作的 worktree（`hermes-constellation-dragon` 的 `.blend` SHA 与 project_index 记录一致；`token-smart-live-control` 的 LiteratureAcquisition UI + 测试；`token-smart-live-workflow` 45 文件；`token-smart-task2` 的 domain 测试）已把各自 WIP 提交到原分支并推送到 origin（`8f27a415`/`08432675`/`7edcc8fd`/`d1473d86`），随后删除这些 worktree；这些属已淘汰的 token-smart 线，不部署。当前注册 worktree 仅 3 个：根、交付树、`art-direction-release-41ae8902`。`codex/hermes-3d-scholar`、`codex/optical-native-prototype`、`codex/product-web-task1` 与交付分支均已推送 origin。`.gitignore` 新增 `.Codex/`、`docs/live_2D/`、`docs/user_ideas/**/*.{png,zip,xlsx}`；根下误建的 `token-smart-live-workflow/` 已移出仓库到本机临时归档。
- **部署链路已打通（一次性修复完成）**：服务器无 git 仓库（只有 `git archive` 解包内容 + 一个无关 publication bare repo），故不能 `git fetch`；改为从 release 目录重建源码 tree 并提交 `311c980f` → 分支 `codex/production-state-48814fe0`（已推送）。比对：生产线是交付分支的**严格超集**——生产独有 81 文件、交付独有 **0**、内容不同 54。
- 阻塞根因（已纠正早前判断）：**不是设计缺陷，而是我方交付分支带着旧版 `infra/scripts/deploy.sh`**。生产线版本对 `--rollback-ref` 增加了裸 40 位 SHA 直通分支，注释明写「另一台工作站部署了本地不存在的提交」，因此无法本地解析的 active 身份不再阻塞部署（远端事务仍强校验 active marker、rollback release 源、capability/image 身份与回滚 Compose）。
- 修复动作与结果：构建候选 `4099078b`（生产线 `311c980f` + 我方 `e86bd39f` 的输出窗口修复；并剔除违反 release 守卫 `isGeneratedPath` 的 `packages/search/generated` 21 个误提交文件），用候选自带的新版 `deploy.sh` 以 `--rollback-ref 48814fe0…` 部署成功。现 `.release-id` 与公网 `/__release` = `4099078b…`（**已推送 GitHub，可 `git rev-parse` 解析**），`.rollback-id` = `48814fe0…`，无 `.release-failed`/事务 journal，全部容器 healthy；journals 与学术身份等同事功能完整保留，Web 构建产出 `/journals`、`/journals/manage`、`/admin/journals` 等路由。**后续可正常发版**。
