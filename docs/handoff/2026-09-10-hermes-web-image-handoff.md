...
## CURRENT 体验改造已部署，视觉收尾待部署（2026-09-11）
- 当前应用c0fdc389d06d0dfd49213945d0641644854e7090 / rollback4909e11bdf1802987a9e1e84518f16fb4c35651d；provider d1630135不变。下方378发布节点是历史科学成果记录。
- 服务器build/start完成exit0，无测试/CI/本机运行。实际研究桌面与OSR22/v10打开，公开图片naturalWidth1280。
- 已落地共享会话/Cookie续期、任务分组与静默更新、编辑器一个资料入口、公开大图、真实缩略图和首页成果。
- Chat6Pro本轮完整产品分析已取回并采纳；Sol High会话/公开数据/归档独立复核已完成。
- 19/20/21三条Task/E2E已由既有脚本精确匹配可恢复归档，实际公开index只剩22。原文件/公开标识/历史保留。
- c0fd已部署，实际桌面→编辑器→探索→公开页会话保持DHL，/me200且Set-Cookie续604800秒；公开图已加载。实际截图又发现旧global fallback头像在桌面重复、初次任务暂显空；当前候选关闭重复渲染、区分loading/unavailable/empty、修专用任务读取失败时global回退；单条探索结果使用已有横卡。
- 下一步：部署视觉收尾→实际浏览→同步最终release；随后再做Hermes能力提升，不重生图/视频。
## 目标与规则
