...
## 当前锚点
- 生产 `08d281a2d3076abf46106595683c8c7f6eae3f07` / rollback `72d2243fbb1b7a7bce996b246ac99f9fd0005dd8`；数值绑定守卫与中文单位/图号等价、`_description`/`_source` 诊断经 39/39、Worker typecheck、独立 High GO、[CI 36189783888](https://github.com/photonics-dhl/OpenScience/actions/runs/36189783888) success；定向部署 exit0、公网 `/__release` 精确一致。原配图 skill v14、多幕 auto wrapper、正式像素审图沿用；不宣称全链重新验收。
...
| 第二篇编辑图 / 2、5 | RO `c896802c`，7 Claim/27 Evidence；正式 5.6 Sol approved 图 `6233e663` 被单独选入 [公开 v2](https://openscience.428312321.xyz/research/OSR-2026-000022/v/2)。匿名实见六维与图，旧 v1 保留；实看仍偏设备结构说明，未一眼讲清“空间局域→短时间脉冲”。 | 同源私有 v13 `fdd7ef10` 复用 SDF/7 Claim/27 Evidence。两幕 `0b5e5db7` 虽获原 M3 末审 accepted，独立核原文 NO-GO：19 as/99 as/270 没有同幕 subject 原文支撑、美术要求画无数据曲线；无新 PNG、公开 v2 不动。08d 实际两幕 `0386efbe` 在 science 因 `19_as_source` 拒收；进一步缩至单幕机制的 `a8c04e9d` 因 `1_bare_description` 三次拒收。旧失败任务/方案均保留；停止猜测重发，先用不含原文的私有诊断记录定位确切字段。 |
...
## 当前执行与保护
- 08d 已排除中文单位/图号词法假阳性；`0386efbe` 定位为 `19_as_source`，单幕 `a8c04e9d` 是 `1_bare_description`，均无新方案/PNG，M3 在 Gateway science 3 次 Schema 重试后停止，非 Chat 桥故障。当前候选只在原 Gateway 拒收回调把数值所在字段、合法 sN 绑定、数值摘要/哈希写私有 AuditLog，不保存候选/原文或改 AgentTask checkpoint；定向 42/42、Worker typecheck、独立 High GO，CI/部署/一次真任务待做。保留旧失败收据，不盲重试或批准旧坏案。
