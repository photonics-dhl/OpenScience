...
- 生产身份事实与部署阻塞（已定性）：服务器 `.release-id`/`.release-source` = `48814fe00640022ea188c45067994fda60508939`，运行容器镜像 tag 同值；该提交在本机对象库、origin 全部 144 个 ref（含 PR head）与 GitHub 页面都**不存在**（404），`3906b93d…` 同样。**用户已确认这条线是同事开发的 journals 功能线**：其 release 目录含整套本仓库任一 ref 都不存在的 Journals 子系统（迁移 `20260915000000_journals`、schema 的 Journal/JournalWork/JournalArticle/JournalJob/JournalRelease/JournalApplication 等、`apps/api/src/routes/journals.ts`、`journal-worker.ts`、`apps/web/components/journals/*` 14 个组件、`.github/workflows/journals.yml`、`docs/handoff/2026-09-15-journal-onboarding-handoff.md`）。该线同时含交付分支 `bdf0d3fa` 的 `scene-image.ts` 内容，故两者共享部分代码但**不是同一线**。
- **部署阻塞根因**：release 身份 = git 提交 SHA；`deploy.sh` 要求 `--rollback-ref` 能在本仓 `git rev-parse` 且**精确等于** `.release-id`，事务再 CAS 切换标记。同事只在自己 clone 部署、从未 push，导致该 SHA 永远无法在本仓解析 → 守卫 fail-closed，**任何部署都被硬阻塞**（`e86bd39f` 的输出窗口修复因此仍未上线）。
- **正确修复路径（勿用变通）**：请同事把该分支（含 `48814fe0`）push 到 GitHub，再 `git fetch` → SHA 可解析即可正常部署，同时把 journals 功能纳入仓库恢复事实源。**不得**把 `.release-id` 改成 `bdf0d3fa` 再部署：那会把同事的 journals 功能从生产上抹掉（本轮已识别并避免此险）。
