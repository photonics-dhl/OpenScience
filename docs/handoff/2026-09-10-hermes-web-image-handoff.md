...
## 当前锚点
- 生产 `d296e9228f4efcd86295f584c5eb513b3b2b86f4` / rollback `91ab16281ae98b8dc551cdc57136f67694e7959d`；原配图 skill v14 的机制配文/显式标签预算/必要分幕规则通过 6 项定向加载测试、Worker typecheck、独立 High GO、[CI 36175932790](https://github.com/photonics-dhl/OpenScience/actions/runs/36175932790) success，定向部署 exit0，公网 release 精确核对。v13 正式审图空间语义修复与第一篇真图验证随生产保留；部署跳过无关 Parser/ScanSci/embedding 功能探针与数据库迁移，不宣称这些重新验收。
...
| 第二篇编辑图 / 2、5 | RO `c896802c`，7 Claim/27 Evidence；正式 5.6 Sol approved 图 `6233e663` 被单独选入 [公开 v2](https://openscience.428312321.xyz/research/OSR-2026-000022/v/2)。匿名实见六维与图，旧 v1 保留；学术/淡彩旧候选的标尺或多余因果错误仍私有 NO-GO。实看公开图参数清楚却偏设备结构说明，尚未一眼讲清“空间局域→短时间脉冲”。 | 已从公开 v2 同源恢复私有 v13 `fdd7ef10`，原 SDF/7 Claim/27 Evidence 复用。Hermes auto 初稿 `52ae14c2` hash `1f2d27d7…` 八标签、公式过密且配文先报数字；普通科学修订 `7a8fc8df` 被原末审 blocked：单幕 120 字配文容不下机制、单电子虚拟限制和独立电子团工况/产额权衡。生产 v14 的两幕任务 `3909a349` 未产方案，结构化重试耗尽；定向日志先见科学标签越界、随后新结构化调用三次 `object_required`。规划器仅对单幕显式风格附末尾 `scenes` 输出提醒，当前“两幕 auto”缺口候选已补，12 项定向测试和 Worker typecheck 通过，独立 High/CI/部署/真任务待做；没有新 PNG，公开 v2 不动。 |
...
| 能力与历史 / 4 | 生产 d296 的 `installed-media-skills.ts` 原 science/art/review 阶段消费配图 skill v14；`generated-image-review.ts` 空间语义核对已在第一篇 blocked→修正→accepted 的真图上验证。第二篇标签超预算/机制配文被指标挤占复发，v14 已通过定向测试/CI/独立 High/部署；两幕 auto 任务 `3909` 在结构化输出失败，原 `illustration-planner.ts` 艺术阶段缺多幕 auto 的末尾 `scenes` 形状提醒，最小候选本地测试通过、尚未部署/真任务验收。未建新模型阶段。 | 原文疑点定向回读、跨任务经验检索、定量几何核验尚未完成，见[能力台账](../runbooks/hermes-capability-registry.md)。Fig.2 重复 plan 与悬空 `d5087b03` copy 未清理，先按原只读影响面与授权边界处理；Fig.1 原字节展示已被否定。 |
