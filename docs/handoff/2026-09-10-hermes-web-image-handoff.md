...
- **隐患 #2/#3 已完成并部署（`5bb93e50`，回滚 `2bd3ed65`）**：隐患 #2 把设计段截断改为按语义边界。隐患 #3 给 `generateImage` 加有界回退——仅在"确定未提交"时前进，不确定失败立即抛出以免第二账号重复付费；回退由显式 `HERMES_SCENE_IMAGE_FALLBACK_PROVIDER` 开启，缺省行为不变。独立 High 审查结论 **GO（限定）**，并据此修掉两项 High：**带参考图时绝不回退**（否则静默丢掉用户指定的风格参考，等同交付另一张图）；把能力边界说清并加启动告警——**目前只有 chatgpt-web 能发出"确定未提交"信号，minimax 作主时回退永不触发**，启用前须以 chatgpt-web 为主。审查遗留 Medium（已登记台账）：恢复/续跑路径仍只看 `imageProviders[0]`（minimax 备有重复付费风险）、`USAGE_LIMIT` 是未文档化字符串契约、新 env 未进 prod compose `environment:` 段。
