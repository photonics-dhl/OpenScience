...
- **隐患 #3 已实现待审**：`generateImage` 改为按序尝试，**仅在"确定未提交"**（provider 禁用/不支持参考图/`USAGE_LIMIT`）时前进，不确定失败立即抛出以免第二个账号重复付费；回退 provider 由显式 `HERMES_SCENE_IMAGE_FALLBACK_PROVIDER` 开启（缺省不变）。提交 `3e6ee3bb`，**独立 High 审查中，通过后部署**。隐患 #2（设计段按字符硬截断）已改为按语义边界截断（`b3302d3e`）。
