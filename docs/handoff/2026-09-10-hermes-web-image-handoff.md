...
## Git、部署与恢复边界
- application production=b1fb77460a596b5f17708c0265f83197fe24cfe1；rollback=8c0edd6c989f2acebb7162d70fecb00fd9e849e2。正常服务器build/start exit0，无迁移/测试，独立marker一致、journal/failed=false。日志/opt/openscience/observations/deploy-b1fb7746-20260922.log；HEAD按Git定锚。root main=acd13a712549e62f8d4b0f3c2f8f064549e226b0。
...
- 自有runtime Skill v8已随b1正常部署，尚无新模型消费；它去过时Chat固定路由/无条件art correction文字。新v9候选把坐标投影、条件归属、可见结果、量化尺度/定性编码的方法集中为Scientific encoding，science/review共同加载；独立High静态PASS，未部署。私有检索context的小补丁仍只设计，尚未实现/调用；完整sourcePassages另传，不能把18K窗口当已证全部误判根因。
...
- 最新第二篇run stopped/v53/max37，四图已实看/正式6Pro审阅完成：d6fc87fe/ dd07aa6a科学blocked/null，1ea36453渲染blocked/仅圆点等径repair，38b488fd accepted。仍有标尺比例/坐标/频率方向/来源数字及调参条件/高产额标签缺失，未发布；terminal证据tmp/hermes-stage-recovery-scenes-terminal-20260922.json。第三篇400无可恢复science或invalid art正文；通用私有science/art检查点及原retry-generation恢复已High静态PASS并随b1部署，尚未付费恢复。Gateway补每次真实提交前CAS钩子；独立单图不启用partial，跨版本Skill消费保留各自版本。当前NULL只可明确完整planning restart，不冒称art-only；不新建分析器或按task特判。证据/详情见09-18交接顶部。
- b1对art提交未知保守不提供恢复入口；正在补已知末次provider_timeout的显式fresh_art_after_unknown，新付费art执行保留science/原unknown审计，绝不冒认未提交或自动重发。两文件小增量High静态PASS；原页面GET已200证实第三篇44/max43/storyboard-planning/1、第二篇53/max37/scientific-replan/5（tmp/b1-two-paper-product-read-20260922.json）。新更新部署后各一次原按钮继续；暂未创建新模型请求。
