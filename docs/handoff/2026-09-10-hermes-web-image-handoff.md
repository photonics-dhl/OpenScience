...
## 当前锚点
- 生产 `a7379e1fe0ebc3e5179392f5ca600bf611ebeb1a` / rollback `08d281a2d3076abf46106595683c8c7f6eae3f07`；沿既有 Gateway 拒收回调写私有数值字段/来源摘要（不存候选/原文，不改任务 checkpoint）经 42/42、Worker typecheck、独立 High GO、[CI 36193303110](https://github.com/photonics-dhl/OpenScience/actions/runs/36193303110) success；定向部署 exit0、公网 `/__release` 精确一致。原配图 skill v14、多幕 auto wrapper、正式像素审图沿用；不宣称全链重新验收。
...
| 第二篇编辑图 / 2、5 | RO `c896802c`，7 Claim/27 Evidence；正式 5.6 Sol approved 图 `6233e663` 被单独选入 [公开 v2](https://openscience.428312321.xyz/research/OSR-2026-000022/v/2)。匿名实见六维与图，旧 v1 保留；实看仍偏设备结构说明，未一眼讲清“空间局域→短时间脉冲”。 | 同源私有 v13 `fdd7ef10` 复用 SDF/7 Claim/27 Evidence。两幕 `0b5e5db7` 原 M3 accepted 但独立核原文 NO-GO（19/99 as、270 未绑结果、伪曲线），无新 PNG。08d 的 `0386efbe`/`a8c04e9d` 分别被 19_as_source/1_bare_description 拒收；a737 真实单幕 `1d1ec917` 三次拒收的私有摘要证明 19 as 已绑 s6 原文，卡点为配文/encoding/标签里的无单位 0/1 编号。新窄候选只识别显式主体/步骤/圈号编号，裸 0/1、无量纲结论、带单位的 1 MeV 和无来源 270 仍校验；43/43、typecheck、独立 High GO，CI/部署待做。公开 v2 与旧任务均保留。 |
...
| 能力与历史 / 4 | 生产 08d 的原 science/art/review 消费配图 skill v14；第一篇真图验证正式审图空间语义修复。第二篇 v14/多幕 wrapper 已能保存两幕，但原末审漏检“结果数值未绑 subject 原文”；数值守卫现能拒收，尚未让 M3 自修。未建新模型或分析阶段。 | 原文疑点定向回读、跨任务经验检索、定量几何核验尚未完成，见[能力台账](../runbooks/hermes-capability-registry.md)。Fig.2 重复 plan 与悬空 `d5087b03` copy 未清理，先按原只读影响面与授权边界处理；Fig.1 原字节展示已被否定。 |

## 当前执行与保护
- a737 的独立真任务 `1d1ec917` 在 science 三次拒收，`AgentTask.result=null`、无资产；私有 AuditLog 三条候选哈希与有界数字摘要显示场景 subject s0 支持 1 MeV、s6 支持 19 as，标题/配文/标签 19 as 并非本次拒收点；配文、encoding、标签反复含 `1:bare`，encoding 另有 `0:bare`。不保存原候选，不猜其原句。窄候选只识别显式图号/主体/步骤/圈号编号，保留裸 0/1 与无量纲结果校验，并把 0:1 等比例当作整体数值，43/43/typecheck、独立 High GO，CI/部署/一次真任务待做。保留全部失败收据，公开 v2 不动。
