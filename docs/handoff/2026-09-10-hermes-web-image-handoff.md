...
## Git与部署
- application production=24b3f8c1cb5c8d1e579f3c92020f495939ba3205，rollback=e7e677180a791c19e30d05a5297319183f8e6976；canonical release/onchip-production-line已推送该完整SHA，根main=62b83372e51285b31a65f89bc0fabeb88d7c09bf。网页审图切换[定向CI 35872438506](https://github.com/photonics-dhl/OpenScience/actions/runs/35872438506) success、应用部署exit0；Worker `CODEX_SOL_IMAGE_REVIEW_ENABLED=false`，正式审图尚无结果。第二篇原私有图cebcef87仍draft、hash bbfaf594且无review copy，未公开。
- e7e中的独立CLI审图实现仍在应用镜像内但Worker开关false；第三次设备登录已成功，隔离服务曾启动，用户质疑重复能力后已停止并disable，未执行任何CLI正式审图或读取凭据。24b3已复用既有 `gateway.reviewScientific`→Chat网页review broker：仅schema3图片请求显式5.6Sol，旧6Pro收据按原模型恢复；Hermes原skill/来源/PNG附件照常注入。新页无发送选择实证Sol=true、输入框`5.6 High`、编辑器为空；正式任务仍待1次内部额度。旧6Pro收据和unknown提交绝不重放。
- Chat provider bundle=24b3f8c1cb5c8d1e579f3c92020f495939ba3205；live review-runner及bundle review-broker与该提交源SHA256一致、两timer active，浏览器未重启。三锁/空闲安装备份 `provider-backup-20260923-web-sol-24b3f8c1`；安装器已报完成且逐文件核对，但辅助脚本尾CR使外层exit127，禁止重跑。旧a6a27ef5 bundle及备份保留，renderer sha256:4a30b091d4bdeb7dd7670a01521d30d7b32694e1f3b34977a2aa26e91669559f，sceneImage fallback=false。当前网页管理接口受Nginx Basic Auth返回401；1 credit加额请求超时后查DB幂等键null、余额0，未重放，等用户选择登录或同审计server-side操作。
