...
- 工作区治理：根 main 原落后 origin/main 536 提交且带 47 项残留；旧状态固化为 `fd2ed334`（归档分支 `codex/root-navigation-archive`），main 重定位到 `origin/main` 并叠加导航/卫生规则 → `6cceba03`，现干净且与远端同步。worktree 40→2（根 + 交付树）：删除 22 个已推送临时树与 12 个孤儿目录，4 个含 WIP 的树先推分支（`8f27a415`/`08432675`/`7edcc8fd`/`d1473d86`）再删除；另有 2 个被占用/ACL 拒绝的 ~0.1MB 桩目录待进程退出后手工删。`.gitignore` 新增 `.Codex/`、`docs/live_2D/`、`docs/user_ideas/**/*.{png,zip,xlsx}`。
- **部署链路已打通**：服务器无源码 git 仓库，故从 release 目录重建源码 tree 为提交 `311c980f`（分支 `codex/production-state-48814fe0`）。比对发现生产线是交付分支的**严格超集**（生产独有 81 文件、交付独有 0、内容不同 54）。真正阻塞不是设计缺陷，而是**我方交付分支带着旧版 `deploy.sh`**：生产线版本对裸 40 位 `--rollback-ref` 跳过本地解析（注释：另一台工作站部署了本地不存在的提交），远端仍强校验 active marker/rollback 源/capability/image 身份/回滚 Compose。
...
- **隐患 #2/#3 已完成并部署（`d3a0da3f`，回滚 `5bb93e50`）**：隐患 #2 设计段改为按语义边界截断。隐患 #3 给 `generateImage` 加有界回退——仅在"确定未提交"时前进，不确定失败立即抛出以免第二账号重复付费；回退由显式 `HERMES_SCENE_IMAGE_FALLBACK_PROVIDER` 开启（缺省不变，compose 已声明）。**两轮独立 High 审查**：第一轮 GO（限定），据此修掉"带参考图绝不回退"（否则静默丢掉用户指定参考＝交付另一张图）并说清能力边界（只有 chatgpt-web 能发出"确定未提交"信号，minimax 作主时回退永不触发）；第二轮**不 GO**，静态审查指出我修 M1 时把"确定未提交"的证明从"付款方无痕迹"弱化为"任一 provider 无痕迹"，会经 API 把已付费任务判成"复用预留"而导致付款方二次计费——**已按审查修回**（`before_submission` 只认付款方）并加正负用例，复核 GO。**遗留 F2（未修，启用 fallback 前必做）**：API 侧恢复判定仍只用主 provider 的单个 spool，备 provider 已完成的结果在端到端上仍进不了 resume 路径，会被推向显式新增付费的新生成。台账措辞已如实区分"已做/未做"。
