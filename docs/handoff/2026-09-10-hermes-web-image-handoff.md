...
## Git、生产与独立能力版本
- 应用production=89a68f762a80965486d08c252ec210f24eeddd90，rollback=93523ac1ea2ed22fc91da8efbc4d7033f5610b50；服务器编译/构建/启动exit0，独立读回.release-id与Git一致，日志tmp/accepted-claims-recovery-deploy-20260921.log（此前部署日志保留）。本次--no-tests --skip-migrate --reuse-unchanged-capability-images，前两次新增迁移已按正常CLI应用；未测试/预检/CI/本机构建。6cd/effa构建类型错误已由effa/caee精确修正。旧API/Worker停机卡住且无running AgentTask后，按精确旧ID/StartedAt停止，部署完成；未称根治停机故障。HEAD按Git定锚；根main acd13a712549e62f8d4b0f3c2f8f064549e226b0，worktree仅根+交付树。
...
## 下一动作与交接入口
- 优先任务5（原e3ef004f run，PDF/SourceMap/e7初稿保留）：d35c31fb终审v5/科学Skill v3六字段accepted、821字/证据与e7相等、无issues/补证，但无Claims；主M3两次成功96.139/110.490秒，旧后置丢弃原因无法回溯。必需Claims guard已部署；accepted-only合同恢复经High审查与服务器构建部署，11:17:14Z原页一次HTTP202（收据/jobs/visual-narrative-source-contract-recovery-20260921.json），ordinal3=a6f877d6-5b2e-4e8a-b619-cef94ff777a0。11:22:24Z a6两次成功198.065/110.598秒仍schema拒绝，首次claims_source_unmaterializable、次未定位，无fallback；run failed/version16/versionId=null，共5项source task。11:27Z只读原SourceMap证明P33[0,959)与P34[959,1868)被合并，导致合法独立关系不可映射；不能断言是a6的确切失败主张。当前候选按P边界拆分（逐片段原文与覆盖不变，聚合展示按既有契约插入段间换行），字段23/17/21/18/22/25段、沿用既有64段/24000字符限制；结构合格但无法物化的候选只留私有身份绑定JSON收据，不自动接受。a6后继恢复须紧接唯一accepted锚点、核验原稿/审计/单次schema失败，保留原CAS/预算（下一source共6+后续3=9）；独立High增量静态PASS，web v5同表示校验已补齐；尚未部署/新发请求。下一步审查收口→正常服务器部署→原run单次续作→Claims/成组图解/实际像素审阅/私有阅读；不新run、不切provider、不放松科学校验、不向e77写入。旧一次性写脚本禁止重跑，完整图组仍待交付。
