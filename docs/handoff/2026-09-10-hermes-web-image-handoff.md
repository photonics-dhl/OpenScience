...
## 当前锚点
- 生产 `72d2243fbb1b7a7bce996b246ac99f9fd0005dd8` / rollback `909cc3e3f08fece12c748b6426198e425c7aa6c7`；新分镜数值须由同一 subject 描述及其原文支持的守卫经定向 36/36、Worker typecheck、独立 High GO、[CI 36186477404](https://github.com/photonics-dhl/OpenScience/actions/runs/36186477404) success；定向部署 exit0、公网 `/__release` 精确一致。原配图 skill v14、多幕 auto wrapper、正式像素审图沿用；部署跳过无关 Parser/ScanSci/embedding 功能探针与迁移，不宣称全链重新验收。
...
| 第二篇编辑图 / 2、5 | RO `c896802c`，7 Claim/27 Evidence；正式 5.6 Sol approved 图 `6233e663` 被单独选入 [公开 v2](https://openscience.428312321.xyz/research/OSR-2026-000022/v/2)。匿名实见六维与图，旧 v1 保留；学术/淡彩旧候选的标尺或多余因果错误仍私有 NO-GO。实看公开图参数清楚却偏设备结构说明，尚未一眼讲清“空间局域→短时间脉冲”。 | 同源私有 v13 `fdd7ef10` 复用 SDF/7 Claim/27 Evidence；单幕 `52ae` 八标签、修订 `7a8` 被末审 blocked，首个两幕 `3909` 结构化失败。生产 909 后 `4660140f` 两幕 auto draft 跨过结构卡点，但混用固定算例与扫描、跨工况比宽，未批准。`117fe82c`、`1cd14e06` 科学阶段只有 thinking，分别耗尽 16k/32k，无新资产。收窄为单电子机制＋独立电子团固定结果后 `0b5e5db7` 两幕 draft 完成、原 M3 末审 accepted；独立核原文 NO-GO：19 as/99 as/270 的场景 subject 未绑结果段，美术画无数据的钟形曲线。无新 PNG、公开 v2 不动；数值来源守卫已在生产 72d，两次新方案仍因 19 as 绑定拒收；诊断细分候选待 CI/部署，详见当前执行。 |
...
| 能力与历史 / 4 | 生产 72d 的原 science/art/review 消费配图 skill v14；第一篇真图验证正式审图空间语义修复。第二篇 v14/多幕 wrapper 真任务已成功保存两幕，但 `0b5` 显示模型末审漏检“结果数值未绑 subject 原文”。原规划器新方案逐数值单位/变量绑定已部署，真实两任务被正确拒收但未自修；中英单位/图号与诊断候选待 CI/部署；未建新模型或分析阶段。 | 原文疑点定向回读、跨任务经验检索、定量几何核验尚未完成，见[能力台账](../runbooks/hermes-capability-registry.md)。Fig.2 重复 plan 与悬空 `d5087b03` copy 未清理，先按原只读影响面与授权边界处理；Fig.1 原字节展示已被否定。 |

## 当前执行与保护
- 新生产上同源 v13 两个独立规划 `c071c8fc`、`8a366d63` 均在 science 三次 `unbound_numeric_19_as` 耗尽，无方案/PNG；M3 调用均 stop 非桥故障。第二次已显式指向原文 s4/s18、s1/s2，仍拒收；旧科学拒收正文未持久化，不能确定是 subject 缺数值、原文绑定错误还是“阿秒/as”词法差异。待发布候选补中文单位/图号归一化和 `_description`/`_source` 诊断，39/39 定向、独立 High GO；CI/部署/再验证待做。保留两次失败收据，不盲重试或批准旧坏案。
