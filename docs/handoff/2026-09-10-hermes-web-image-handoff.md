...
- **未决治理项**：现在存在两条线——我方 `codex/onchip-video-release`（缺 journals/学术身份更新）与已部署的 `release/onchip-production-line`（= `4099078b`，生产线+我方修复）。需要用户决定哪条作为 canonical 交付分支；在决定前不要用 `onchip-video-release` 发版。
