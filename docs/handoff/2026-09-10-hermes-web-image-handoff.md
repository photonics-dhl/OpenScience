...
- 用户坚持服务器 Chat 生图路线：图必须传递论文核心信息。`d45bea27` / `3930c143` 是 art-only 修补产物，已触及天花板。完整 Hermes storyboard.generate `d0b36138-e66e-4f4d-9f49-ff60095f7eeb` 成功，reviewScientific `decision:accepted, issues:[]`；subjects=Bethe 等效源 (m₀/p₀) + 形状因子 f=3 j₁(k⊥a)/(k⊥a)，labels=6 项含 a≪λ₀、k₀、传播、倏逝、形状因子；构图=上半圆孔/下半 Weyl 角谱。已批准；scene-image `145af7bf-f9a5-46ca-a346-4914fdcbf17f` 已 succeeded/draft，用户接受当前叙事风格。
- 用户纠正：链路不应被"手工模板 prompt"绑死。每篇论文应自动生成 narrative prompt，链路应能根据用户语义级审美反馈自动决定下一个修订 prompt。后续不要再人工写 hex 色值 / 详细材料描述。当前唯一真实缺口是把"用户审美反馈 → 下一个 prompt"交给 Hermes art stage + design skill，而不是再让人补写。已停止手工 art revision `01640253-aa45-416b-9df8-0696112795fe`，该任务因 `Provider exhausted output allowance before producing text` 失败、asset 未保留、不污染数据库。
- 链路真实分工与债务：scientific intent ✅ Hermes 自动；art stage composition/treatment ✅ Hermes + baoyu/scientific-critical-thinking 自动；语义级"暖灰/期刊/金属感"翻译 ✅ 自动（见 `d0b36138` planned.treatment）；实际生图 ⚠️ chatgpt-web provider 在某些复杂 prompt 上偶发 `exhausted output allowance`；审美反馈闭环 ❌ 仍由操作员写 hex。下次会话先把"用户审美语义 → 修订 prompt"这条缺口交还给 Hermes 设计 skill。
