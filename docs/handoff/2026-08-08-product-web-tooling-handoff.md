...
- **Next action:** 先让用户确认 Step 8 首批组件截图；若确认，继续 Dialog/Tabs 与 RO Card、SDF Node、Artifact Card、Review Row、Version Diff、Hermes Rail；若未确认，只针对单一视觉问题迭代，不扩展组件数量。不得提前修改网页代码或调用 MiniMax。
