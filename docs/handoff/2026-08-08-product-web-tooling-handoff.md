...
- **Current goal:** Task 2 Workspace shell 已完成首轮；下一轮接入真实 workspace/permission 上下文与 overview 摘要。
- **Done:**
...
- **Next action:** 接入真实 `workspaceId` 与权限来源，新增 `/research-objects/[id]/workspace` overview 摘要；随后补 Dashboard/API 的 loading/error/empty/permission 测试和 Public RO/Collection 真实链接。Figma entry batch 结构转换仍独立处理。
