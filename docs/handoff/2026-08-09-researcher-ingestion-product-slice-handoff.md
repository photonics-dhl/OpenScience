...
> **Update 2026-08-10:** MiniMax domestic Token Plan endpoint, review/confirm, transactional consume, Tesseract worker image and ClamAV service are deployed; EICAR is fail-closed. Read the newest progress entry before continuing the remaining restore/OCR fixture evidence.
...
- **Next action:** 在隔离临时容器中验证对象快照恢复和真实图片 OCR 文本匹配；通过后关闭本轮 ingestion 生产切片，进入发布/评审 UI 细化。
