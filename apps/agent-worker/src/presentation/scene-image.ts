...
TeachingPoint: the one relationship the viewer should understand, not a slogan. Subjects: identify the few concrete objects essential to that scene, including its receiving surface or visible outcome when specified. Preserve each retained object's Claim-supported physical subtype, material, geometry and scale, including contextual or background objects; never replace specialized scientific hardware with a visually familiar but physically different object. Resolve ambiguous nouns from the supplied Claims: for example, a nanoscale optical antenna or tip remains a nanoscale solid-state structure, never a radio receiver or satellite dish. Arrangement: place each object explicitly within one readable composition, with clear depth, separation and focal hierarchy. Mechanism: describe visible interactions connecting cause to outcome; translate motion into a legible still moment, not detached decorative streaks. Fidelity: relevant conditions, limitations and uncertainty, including what must not be inferred. The selected scene and its sourceClaimIds define the picture content; other supplied Claims constrain fidelity and must not add unrelated objects. Check ALL supplied Claims for contradictions.
Preserve the approved scene's objects and scientific meaning. Overall framing, camera placement and artistic texture are permissible; preserve supplied dimensions and relative scales, and invent neither missing scale nor apparatus, measurements, numerical results, mechanisms or evidence. If the scene specifies an output screen, keep it visibly connected to the propagation path. Represent bright/dark intensity on that surface rather than free-floating color clouds. Do not imply wavelength separation or changed light frequency just through decorative colors. If a scene asks for a banner, tag, caption, label or other writing, express only its scientifically necessary meaning through nonverbal object relationships, shape or color, or omit it; never draw the writing or its typographic container. Use the selected style as surface treatment while preserving clear contours, contrast and causal layout. Avoid posters, title cards, charts of invented data, text overlays and abstract atmosphere replacing the mechanism. If faithful depiction cannot fit the bound, return empty fields to block generation.`},
