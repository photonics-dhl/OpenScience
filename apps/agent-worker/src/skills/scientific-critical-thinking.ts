 ...
 */
const scientificEvidenceRules = [
  '对每项拟保留断言，检查来源支持的对象、强度、条件、参数和算例是否一致；保留会改变解释的限制与相反材料。背景介绍不是本研究结果，作者解释不是已证因果；未知算例归属保持未知，不将不同算例的参数拼在一起。',
  'reported只表示作者如此陈述，不表示已核正确。区分强度与有符号场、概率与幅度、频率与能量、FWHM与其他宽度定义；保留实际叠加对象和相位/归一化约定。不同来源公式不一致时分存其位置与疑义，不拼成新式；某个算例吻合不能消除推导或量纲疑义。',
  '按研究实际类型检查适用项：理论关注假设、推导和量纲约定；实验关注对照、校准与不确定性；仿真关注模型、边界、参数和收敛依据。未提供实验或收敛报告不自动成为所有研究的缺陷，不能套用临床证据等级或固定缺项清单。',
  '原文未报告、当前材料未取得、解析有疑误是三种状态。sourceQuality警示与乱码观察保留为内部待回读信息，不写进limitations/reproducibility或推断论文缺页；P编号也不是物理页号。公式识别或排版成功不能证明物理正确；不猜补、改造原式。',
  '列出代表参数不等于所有算例完整参数化或已经独立复现。推断必须有来源建立的依赖关系，不能将不同分析目标的条件互相移用，也不能把求平均等运算改写成额外模型假设。作者限定模型下的极限、近似或数值结果不可泛化为普适定理、严格上界或实验事实。',
];
