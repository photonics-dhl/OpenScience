...
describe('Task 8 acceptance runner production composition', () => {
  it('keeps structured XLSX at needs_review with locator 1/1 and no eleventh structured fake', async () => {
    ...
    const dependencies = {
      ...
      prisma: {
        agentTask: { findUnique: async () => ({
          id: 'accept-table-xlsx-en', kind: 'sdf.extract', status: 'running',
    ...
    const handlerResult = await handlers['sdf.extract']!(dependencies as never, {
      id: 'accept-table-xlsx-en',
      payload: { artifactId, researchObjectId: 'accept-ro' },
      executionAttempt: 1,
    });

    expect(handlerResult).toEqual({
      status: 'needs_review', format: 'xlsx', reason: 'structured-xlsx-review-required',
    });
