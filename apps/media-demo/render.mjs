...
async function main() {
  ...
  await writeFile(resolve(output, 'storyboard.json'), JSON.stringify({ fps, total, source: 'https://arxiv.org/abs/1804.08711v2', notice: 'Conceptual animation; intensities illustrative, not measured.', narration: 'Pre-generated Windows Huihui WAV inputs; no server TTS.', scenes }, null, 2), { flag: 'wx' });
    ...
    encoder = startEncoder(ffmpeg, ['-n', '-hide_banner', '-loglevel', 'error', '-protocol_whitelist', 'file,pipe', '-i', resolve(output, 'silent-v2.mp4'), ...audioInputs, '-filter_complex', filters, '-map', '0:v', '-map', '[a]', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-shortest', resolve(output, VIDEO_FILE)]);
    encoder.child.stdin.end();
    await encoder.done;
    if (interrupted) throw new Error('Render interrupted');
    ...
    if (video?.codec_name !== 'h264' || video.width !== 1280 || video.height !== 720 || video.pix_fmt !== 'yuv420p' || !probe.streams.some(stream => stream.codec_name === 'aac') || Math.abs(Number(probe.format.duration) - total) > 0.1) throw new Error('Rendered media failed format validation');
