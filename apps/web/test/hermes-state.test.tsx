...
describe('Hermes dashboard guidance', () => {
  ...
  it('renders one honest Hermes mount and keeps approval/reduced fallback still', () => {
    ...
    expect(markup).toContain('data-hermes-renderer="layered-pet"');
    expect(markup).toContain('data-hermes-state="awaiting_approval"');
    expect(markup).toContain('data-hermes-pet="true"');
    expect(markup.match(/data-hermes-frame=/g) ?? []).toHaveLength(3);
    expect(markup.match(/data-hermes-node=/g) ?? []).toHaveLength(6);
  });
  ...
  it('exposes all six visual states without depending on a licensed binary', () => {
    for (const state of ['idle', 'guiding', 'scanning', 'suggesting', 'awaiting_approval', 'failed'] as const) {
      ...
      expect(markup).toContain('data-hermes-renderer="layered-pet"');
      expect(markup).toContain('hermes-pet-idle.png');
      expect(markup).toContain('hermes-pet-blink.png');
      expect(markup).toContain('hermes-pet-working.png');
