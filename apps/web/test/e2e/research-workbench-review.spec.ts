...
test('Dashboard exposes the real-size Hermes contextual menu and assistant click', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /研究工作台|research workbench/iu })).toBeVisible();
  ...
  await expect(page.getByRole('menu', { name: /这一步想怎么继续|How should we continue/u })).toBeVisible();
  await page.getByRole('menuitem', { name: /安静陪我读|Read quietly with me/u }).focus();
  await expect(page.getByRole('menuitem', { name: /安静陪我读|Read quietly with me/u })).toBeFocused();
  await page.screenshot({ fullPage: true, path: `${outDir}/dashboard-menu-focus.png` });
  await page.keyboard.press('Enter');
  await expect(page.locator('[aria-live="polite"]')).toContainText(/我在这页陪你|I’ll stay with this page/u);
...
test('keyboard menu access, quiet editor metrics and review acknowledgement work', async ({ page }) => {
  ...
  await expect(reading).toHaveCSS('font-size', '19px');
  await expect(reading).toHaveCSS('line-height', '32.68px');
...
test('mobile long press opens the compact menu without horizontal overflow', async ({ page }) => {
  ...
  const trigger = page.locator('[data-review-hermes-trigger="true"]');
  await expect(page.locator('[data-review-hermes-size="200"]')).toHaveCSS('width', '200px');
  await trigger.scrollIntoViewIfNeeded();
  await expect(page.locator('[data-hermes-rig="live2d-wanko"]')).toHaveAttribute('data-hermes-rig-status', 'ready', { timeout: 20_000 });
