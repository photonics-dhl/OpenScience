...
export default async function CollectionPage({ params }: { params: { slug: string } }) {
  try {
    const { collection } = await getPublicEditorialCollection(params.slug);
    return <main><EditorialCollection collection={collection} /></main>;
  } catch (error) {
    if (error instanceof PublicServerApiError && error.status === 404) notFound();
    return <main className="min-h-screen bg-os-paper px-6 py-24 text-os-ink"><h1 className="font-editorial text-5xl">Collection unavailable.</h1><p className="mt-5 text-os-muted-paper">Please return to the Research Index and try again.</p></main>;
  }
}
