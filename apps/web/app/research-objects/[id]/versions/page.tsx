...
export default function VersionsPage({ params }: { params: { id: string } }) {
  ...
  const activeVersion = queryVersion ? versions.find((version) => version.versionId === queryVersion) : versions[0];
  ...
  return <ResearchSurfaceShell active="versions" object={object} actions={<button className="min-h-9 rounded-panel border border-os-rule-dark px-3 text-xs text-os-paper disabled:opacity-40" disabled={selected.length !== 2} onClick={compare}><GitCompareArrows className="mr-2 inline h-4 w-4" />{t('versions.compare')}</button>}>
    ...
    {versions.length === 0 ? <div className="mt-12" data-surface-state="empty"><h2 className="font-editorial text-3xl text-os-paper">{t('versions.emptyTitle')}</h2><p className="mt-3 text-sm text-os-muted-dark">{t('versions.emptyBody')}</p></div> : <div className="mt-10 border-t border-os-rule-dark">{versions.map((version) => { const checked = selected.includes(version.versionId); return <label className="grid cursor-pointer gap-3 border-b border-os-rule-dark py-5 sm:grid-cols-[2rem_5rem_1fr_9rem] sm:items-center" key={version.versionId}><input checked={checked} className="accent-os-vermilion" onChange={() => setSelected((current) => checked ? current.filter((id) => id !== version.versionId) : [...current.slice(-1), version.versionId])} type="checkbox" /><Link className="font-editorial text-lg font-normal text-os-paper underline" href={`/research-objects/${encodeURIComponent(params.id)}/versions?version=${encodeURIComponent(version.versionId)}`}>{t('files.snapshotVersion', { version: version.versionNo })}</Link><span className="text-sm text-os-muted-dark">{version.commitId}</span><span className="font-data text-[10px] uppercase text-os-paper">{version.status}</span></label>; })}</div>}
    {queryVersion && !versions.some((version) => version.versionId === queryVersion) && <p role="alert">{t('versions.notFound')}</p>}
    {activeVersion && <VersionRecord key={activeVersion.versionId} researchObjectId={params.id} versionId={activeVersion.versionId} versionNo={activeVersion.versionNo} />}
