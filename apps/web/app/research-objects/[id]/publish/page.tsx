...
export default function PublishPage({ params }: { params: { id: string } }) {
  ...
  if (error && !object) return <ResearchSurfaceStateShell active="publish" detail={error.message} kind={error instanceof ApiClientError && error.status === 403 ? 'forbidden' : 'error'} objectId={params.id} title={t('state.errorTitle')} />;
  if (!object) return <ResearchSurfaceStateShell active="publish" detail={t('state.loadingBody')} kind="loading" objectId={params.id} title={t('publish.title')} />;
  return <ResearchSurfaceShell active="publish" object={object} rail={<div><p className="font-data text-[10px] uppercase tracking-[0.14em] text-os-muted-dark">{t('publish.permanence')}</p><p className="mt-4 text-sm leading-6 text-os-muted-dark">{t('publish.permanenceBody')}</p></div>}>
    ...
    <Dialog onOpenChange={setConfirmOpen} open={confirmOpen}><DialogContent className="max-w-xl rounded-panel border-os-rule-paper bg-os-paper-strong p-6 text-os-ink sm:p-8" data-review-changes="true"><DialogClose className="absolute right-4 top-4 rounded-panel p-2" aria-label={t('publish.close')}><X className="h-4 w-4" /></DialogClose><DialogTitle className="font-reading text-4xl font-normal">{t('publish.confirmTitle')}</DialogTitle><DialogDescription className="mt-4 text-base leading-7 text-os-muted-paper">{t('publish.confirmBody')}</DialogDescription><div className="mt-7 flex justify-end gap-3"><DialogClose className="min-h-11 rounded-panel border border-os-rule-paper px-4 text-sm">{t('publish.cancel')}</DialogClose><button className="min-h-11 rounded-panel bg-os-vermilion px-4 text-sm font-semibold text-white disabled:opacity-40" disabled={working} onClick={publish}>{t('publish.confirm')}</button></div></DialogContent></Dialog>
