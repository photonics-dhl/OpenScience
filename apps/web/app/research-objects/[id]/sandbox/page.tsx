...
export default function SandboxPage({ params }: { params: { id: string } }) {
  ...
  if (error && !object) return <ResearchSurfaceStateShell active="sandbox" detail={error.message} kind={error instanceof ApiClientError && error.status === 403 ? 'forbidden' : 'error'} objectId={params.id} title={t('state.errorTitle')} />;
  if (!object) return <ResearchSurfaceStateShell active="sandbox" detail={t('state.loadingBody')} kind="loading" objectId={params.id} title={t('sandbox.title')} />;
