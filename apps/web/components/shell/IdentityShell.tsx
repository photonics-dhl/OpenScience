...
function IdentityShell({ children, className, context, mainClassName, skipLabel, ...props }: IdentityShellProps) {
  return (
    <div className={cn('surface-workbench min-h-dvh', className)} data-os-surface="identity" {...props}>
      ...
      <div className="grid min-h-[calc(100dvh-3.5rem)] lg:grid-cols-[minmax(18rem,0.8fr)_minmax(30rem,1.2fr)]">
        <aside className="hidden border-r border-os-rule-dark p-8 lg:flex lg:items-end xl:p-12" aria-hidden={!context}>
          {context}
        </aside>
