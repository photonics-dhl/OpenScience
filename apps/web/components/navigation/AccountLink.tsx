...
export function AccountLink({ user, active = false }: { user: CurrentUser | null; active?: boolean }) {
  ...
  return (
    <Link href="/me" aria-current={active ? 'page' : undefined} aria-label={t('accountLink', { name: user.displayName })}
      className="inline-flex min-h-11 max-w-48 items-center gap-2 rounded-control px-2 text-sm text-os-ink transition-colors hover:bg-os-paper-2 hover:text-os-vermilion-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring" data-account-link="true">
