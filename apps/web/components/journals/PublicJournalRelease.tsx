...
function Evidence({ value }: { value: { quote: string; locator: string } }) {
  return <p className="mt-2 min-w-0 break-words border-l-2 border-os-rule-paper pl-3 text-sm text-os-muted-paper [overflow-wrap:anywhere]">证据：{value.quote} <span>（{value.locator}）</span></p>;
}
...
export function PublicJournalRelease({ value }: { value: PublicJournalPackageData }) {
  ...
  return <section className="mt-8 min-w-0 max-w-full border-y border-os-rule-paper px-4 py-6 [overflow-wrap:anywhere] sm:px-6 lg:px-16" aria-label="期刊衍生解读信息" data-journal-derivative="true">
    ...
    <dl className="mt-3 grid min-w-0 gap-2 text-sm sm:grid-cols-[9rem_minmax(0,1fr)]">
      <dt>原论文</dt><dd className="min-w-0 break-words">{value.metadata.title}</dd>
    ...
    {value.draft ? <>
      ...
      <section className="mt-7 min-w-0"><h2 className="text-xl font-normal">主张与证据</h2><div className="grid min-w-0 gap-4">{value.draft.claims.map((claim, index) => <article className="min-w-0 border border-os-rule-paper p-4" key={`${claim.text}-${index}`}><p className="m-0 break-words"><span className="text-sm text-os-muted-paper">{claim.kind} · </span>{claim.text}</p><Evidence value={claim.evidence} /></article>)}</div></section>
      {value.draft.figures.length ? <section className="mt-7 min-w-0"><h2 className="text-xl font-normal">图卡</h2><div className="grid min-w-0 gap-4 sm:grid-cols-2">{value.draft.figures.map((figure, index) => <article className="min-w-0 border border-os-rule-paper p-4" key={`${figure.label}-${index}`}><h3 className="m-0 break-words text-lg font-normal">{figure.label}</h3><p className="break-words">{figure.purpose}</p><p className="break-words">{figure.finding}</p><Evidence value={figure.evidence} /></article>)}</div></section> : null}
