...
import styles from '@/app/research-objects/[id]/presentation/presentation-page.module.css';
...
function standaloneUrl(roId: string, versionId: string, taskId?: string): string {
...
export function ResearchPresentation({ params, embedded = false, selectedVersionId, onAskHermes }: { params: { id: string }; embedded?: boolean; selectedVersionId?: string; onAskHermes?: (kind: 'image' | 'video') => void }) {
  ...
  const requestedVersionId = selectedVersionId ?? search.get('version') ?? '';
  const taskId = search.get('task') ?? '';
  function scopedUrl(roId: string, versionId: string, nextTaskId?: string): string {
    if (!embedded) return standaloneUrl(roId, versionId, nextTaskId);
    const query = new URLSearchParams(search.toString()); query.set('stage', 'media'); query.set('version', versionId);
    if (nextTaskId) query.set('task', nextTaskId); else query.delete('task');
    return `/research-objects/${encodeURIComponent(roId)}/edit?${query}`;
  }
  ...
  useEffect(() => {
    ...
    if (!embedded && !requestedVersionId) router.replace(scopedUrl(params.id, version.versionId));
  }, [bootstrapLoading, embedded, params.id, requestedVersionId, router, version]);
  ...
  if (bootstrapLoading && embedded) return <p className="p-6 text-os-muted-paper" role="status">{t('loading')}</p>;
  if (bootstrapLoading) {
  ...
  const content = (<>
      {!embedded && <div className={styles.navigation}><ResearchWorkspaceNav active="presentation" objectId={params.id} /></div>}
      <div className={embedded ? undefined : styles.layout}><div className={styles.content}>
      {invalidRequestedVersion ? (
        ...
        <>
          {!embedded && <div className="flex flex-wrap items-center justify-between gap-4 border-b border-os-rule-paper px-4 py-3 sm:px-8 lg:px-12">
      ...
      </div>{!embedded && assistantObject?.id === params.id ? <aside className={styles.companion}>
