...
export function PresentationResultGallery({ researchObjectId, versionId, assets, allAssets, claimsById, canWrite, working, onTransition }: Props) {
  ...
  return <div className="grid min-w-0 gap-6 lg:grid-cols-2">{assets.map((asset, index) => {
    ...
    return <article key={asset.id} className="surface-folio-sheet min-w-0 overflow-hidden" data-presentation-result={asset.id}>
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-os-rule-paper px-5 py-4 sm:px-6"><div className="min-w-0"><p data-reading-role="caption" className="m-0 text-os-vermilion-ink">{t('imageTitle')}</p><h3 className="m-0 mt-1 text-balance text-lg font-semibold leading-6">{title}</h3></div><span className="rounded-control border border-os-rule-paper px-2 py-1 text-xs text-os-muted-paper">{t(`assetStatus.${asset.status}`)}</span></div>
      <figure className="m-0 bg-os-paper px-4 py-4 sm:px-6 sm:py-5"><div className="relative aspect-video max-h-[32rem] w-full"><img className="absolute inset-0 h-full w-full object-contain outline -outline-offset-1 outline-black/10" src={sourceUrl} alt={title} width={1200} height={720} loading="lazy" /></div><figcaption className="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm"><span className="text-pretty leading-6 text-os-muted-paper">{t('notEvidence')}</span><a className="inline-flex min-h-11 items-center font-semibold text-os-vermilion-ink underline" href={sourceUrl} target="_blank" rel="noreferrer">{t('viewFullSize')}</a></figcaption></figure>
