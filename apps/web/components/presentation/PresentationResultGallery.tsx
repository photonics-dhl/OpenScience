...
export function PresentationResultGallery({ researchObjectId, versionId, assets, allAssets, canWrite, working, onTransition }: Props) {
  ...
  return <div className={`grid min-w-0 items-start gap-6 ${assets.length > 1 ? '2xl:grid-cols-2' : ''}`}>{assets.map((asset, index) => {
    ...
    const title = (asset.label !== 'presentation_not_evidence' && asset.label.trim()) || parent?.document.scenes[asset.sceneImage?.sceneIndex ?? -1]?.title || (isVideo ? t('videoTitle') : t('imageNumber', { number: index + 1 }));
    ...
    return <article key={asset.id} id={`illustration-${asset.id}`} className="surface-folio-sheet min-w-0 scroll-mt-6 overflow-hidden" data-presentation-result={asset.id}>
      ...
      <figure className="m-0 bg-os-paper px-4 py-4 sm:px-6 sm:py-5"><div className="relative aspect-video max-h-[32rem] w-full">{isVideo ? <video className="absolute inset-0 h-full w-full bg-os-ink object-contain" src={sourceUrl} controls preload="metadata" aria-label={title} /> : <img className="absolute inset-0 h-full w-full object-contain outline -outline-offset-1 outline-black/10" src={sourceUrl} alt={title} width={1200} height={720} loading="lazy" />}</div><figcaption className="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm"><span className="text-pretty leading-6 text-os-muted-paper">{t('notEvidence')}</span><a className="inline-flex min-h-11 items-center font-semibold text-os-vermilion-ink underline" href={sourceUrl} target="_blank" rel="noreferrer">{t(isVideo ? 'openVideo' : 'viewFullSize')}</a></figcaption></figure>
