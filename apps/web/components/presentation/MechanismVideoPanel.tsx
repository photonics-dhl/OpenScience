...
export function MechanismVideoPanel({parent, assets, disabled, onGenerate}: {
  ...
  const roles: PresentationVideoRequest['sceneRoles'] = ['driver_signal', 'tip_enhancement', 'emission_collection', 'delay_scan', 'field_reconstruction'];
  const [selected, setSelected] = useState<string[]>(Array(5).fill(''));
  if (parent.canGenerateVideo !== true || parent.status !== 'approved' || parent.storyboard?.document.scenes.length !== 5) return null;
  ...
  const complete = chosen.every(Boolean) && new Set(chosen).size === 5;
  return <section aria-label={t('title')} className="border-t border-os-rule-paper px-5 py-5 sm:px-6">
    ...
    <fieldset disabled={disabled} className="grid gap-3 border-0 p-0">
      {options.map((items, index) => <label key={index} className="grid gap-1 text-sm">
        {index + 1}. {t(roles[index])} — {parent.storyboard!.document.scenes[index].title}
      ...
      <button type="button" disabled={!complete || disabled} onClick={() => onGenerate(parent.sourceClaimIds, {profile: 'onchip-field-sampling-v1', sceneRoles: roles, storyboardAssetId: parent.id, sceneImageAssetIds: chosen})} className="min-h-11 rounded bg-os-ink px-4 text-sm font-semibold text-os-paper disabled:opacity-40">{t('generate')}</button>
    </fieldset>
  </section>;
}
