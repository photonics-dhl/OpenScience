...
export function HermesPresentationAction(props: Props) {
  ...
  return <section className="min-w-0 rounded-xl bg-os-paper p-4 text-os-ink" data-hermes-presentation-action="true">
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <button type="submit" className="min-h-11 w-full rounded bg-os-ink px-4 py-2 text-sm font-semibold text-os-paper disabled:opacity-40" disabled={busy || bridgeBusy || !canWrite || ready!==scope || !validSources || (action!=='scene.image' && !validPresentationInstruction(instruction))}>{t(busy?'submitting':uncertain?'retry':'confirm')}</button>
    </form>
