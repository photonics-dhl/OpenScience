...
import { getVersionEvidenceSource, listVersionEvidence, verifyVersionEvidence, type HermesResearchRun, type VersionEvidence } from '@/lib/api';
...
export function HermesClaimEvidenceReview({ researchObjectId, run, onDone }: { researchObjectId: string; run: HermesResearchRun; onDone(): void }) {
  ...
  React.useEffect(() => {
    ...
    void listVersionEvidence(researchObjectId, versionId, controller.signal).then(({ evidence: rows }) => {
      if (!controller.signal.aborted) setEvidence(rows.filter((row) => run.sourceClaimIds.includes(row.claimId)));
  ...
  return <section className="surface-folio-sheet mt-7 max-w-3xl border-y border-os-rule-paper px-5 py-6" aria-labelledby="hermes-claim-review-title">
    ...
    {evidence.map((row) => <article key={row.id} className="mt-4 border-t border-os-rule-paper pt-4"><p className="font-semibold">{row.title}</p>{row.exactQuote ? <blockquote className="mt-2 border-l-2 border-os-rule-paper pl-3 text-sm leading-6">{row.exactQuote}</blockquote> : null}<p className="mt-2 text-sm text-os-muted-paper">{row.locator.page ? t('sourcePage', { page: row.locator.page }) : t('sourceLocation')}</p><button type="button" disabled={sourceLoading} onClick={() => void viewSource(row)} className="mt-2 min-h-11 text-sm font-semibold text-os-vermilion-ink underline disabled:opacity-40">{sourceLoading && sourceRowId === row.id ? t('loadingSource') : t('viewSource')}</button>{sourceRowId === row.id && !sourceLoading ? sourceText ? <blockquote className="mt-2 border-l-2 border-os-rule-paper pl-3 text-sm leading-6">{sourceText}</blockquote> : <p className="mt-2 text-sm text-os-muted-paper">{t('claimSourceUnavailable')}</p> : null}{row.verifiedByUserId ? <p className="mt-3 text-sm text-os-muted-paper">{t('evidenceVerified')}</p> : <button type="button" disabled={Boolean(busy)} onClick={() => void verify(row)} className="mt-3 min-h-11 rounded-panel border border-os-vermilion-ink px-4 py-2 text-sm font-semibold text-os-vermilion-ink disabled:opacity-40">{busy === row.id ? t('verifying') : t('verifyEvidence')}</button>}</article>)}
