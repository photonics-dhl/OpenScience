...
function HermesWorkspaceStage({ fallbackAssistantOpen, fallbackOnInvoke, guideTarget, onDismissGuide, presentation, registry, registryVersion, routeState, writing }: {
  ...
  useEffect(() => {
    if (!guideTarget && !speech.cue) return;
  ...
  }, [anchorRect, guideTarget, position, protectedGeometryVersion, speech.cue, viewportSize]);
  ...
  useEffect(() => {
    ...
    const plan = planHermesTravel({
      ...
      editable: target,
      footprint: {
        bottom: Math.max(1, union.bottom - stageCenter.y),
        left: Math.max(1, stageCenter.x - union.left),
        right: Math.max(1, union.right - stageCenter.x),
        top: Math.max(1, stageCenter.y - union.top),
      },
      from,
