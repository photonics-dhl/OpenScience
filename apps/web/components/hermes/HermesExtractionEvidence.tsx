...
export function HermesExtractionEvidence({ field, result }: { field: string; result: unknown }) {
  ...
  const diagnostics = result && typeof result === 'object' ? (result as { fieldDiagnostics?: Record<string, unknown> }).fieldDiagnostics : undefined;
  const unresolved = typeof diagnostics?.[field] === 'string';
  const unverifiedSummary = result && typeof result === 'object'
    ? (result as { unverifiedSummaries?: Record<string, unknown> }).unverifiedSummaries?.[field] : undefined;
  ...
  return <details open={unresolved} className="mt-2 text-sm" data-hermes-source-evidence={field}>
    ...
    {unresolved ? <p role="status" className="mb-3 leading-6 text-os-muted-paper">{t('sourceReconciliationFailed')}</p> : null}
