...
export function PresentationAssetGallery({ assets, leading = false }: { assets: PublicPresentationAsset[]; leading?: boolean }) {
  ...
  return <section className={`pub-presentation-gallery${leading ? ' pub-presentation-gallery--lead' : ''} ${styles.gallery}${leading ? ` ${styles.leadingGallery}` : ''}`} data-presentation-gallery="true" aria-label={leading ? t('title') : undefined} aria-labelledby={leading ? undefined : headingId}>
    {!leading && <div className={styles.sectionTitle}>
