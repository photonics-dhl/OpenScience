...
export type LoginErrorKind = 'credentials' | 'rate' | 'unavailable' | 'generic';
...
export function classifyLoginError(cause: unknown): LoginErrorKind {
  ...
  if (cause.status === 401 || /CREDENTIAL|PASSWORD|LOGIN/i.test(cause.code)) return 'credentials';
    ...
    <section className="w-full max-w-xl surface-folio-sheet px-5 py-8 sm:px-9 sm:py-10" data-auth-flow="login">
      ...
      <form className="mt-8 grid gap-6" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
          <button type="button" className="text-os-vermilion-ink hover:underline focus-visible:ring-2 focus-visible:ring-focus-ring" onClick={() => setShowPassword((value) => !value)}>
            {t(showPassword ? 'login.hidePassword' : 'login.showPassword')}
          </button>
          <button type="button" className="text-os-vermilion-ink hover:underline focus-visible:ring-2 focus-visible:ring-focus-ring" aria-expanded={showRecovery} aria-controls="login-recovery" onClick={() => setShowRecovery((value) => !value)}>
