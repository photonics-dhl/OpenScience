  ...
  const content = <div className="mx-auto max-w-4xl text-os-ink" data-workbench-publication="true">
    ...
    {ready && core && <>
      <article className="rounded-panel bg-white px-5 py-8 shadow-sm sm:px-8" data-publication-preview="true">
        ...
        <div className="research-publication-media">{assets.filter((asset) => ['image', 'chart', 'video'].includes(asset.kind)).map((asset) => <figure className="my-7" key={asset.id}>{asset.kind === 'video' ? <video className="h-auto w-full" controls preload="metadata" aria-label={asset.label} src={presentationAssetContentUrl(researchObjectId, selectedId, asset.id)} /> : <img className="h-auto w-full object-contain" src={presentationAssetContentUrl(researchObjectId, selectedId, asset.id)} alt={asset.label} />}<figcaption className="mt-2 text-sm text-os-muted-paper">{asset.label}</figcaption></figure>)}</div>
        {SDF_FIELDS.filter((field) => field !== 'insight').map((field) => <section className="mt-7" key={field}><h3 className="research-section-title">{t(`fields.${field}`)}</h3><p className="mt-2 whitespace-pre-wrap leading-8">{core[field]}</p></section>)}
      </article>
