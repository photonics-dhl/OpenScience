...
import { Boxes, FileClock, FileText, FlaskConical, Gauge, Send, Sparkles, Users } from 'lucide-react';
import Link from 'next/link';
import { useTranslations } from 'next-intl';

import { researchSurfaceHref, type ProductSurfaceId } from '@/lib/product-surfaces';

type WorkspaceNavId = Exclude<ProductSurfaceId, 'settings'> | 'hermes';
    ...
    <nav aria-label={t('navigation')} className="flex min-w-max items-stretch px-2 sm:px-4" data-research-workspace-nav="true">
      {items.map(({ id, icon: Icon }) => (
        <Link
          ...
          href={id === 'hermes' ? `/research-objects/${encodeURIComponent(objectId)}/hermes` : researchSurfaceHref(id, objectId)}
          key={id}
        >
