...
const generationBody = z.object({
  ...
  storyboard: z.object({ output: z.enum(['image', 'video']).default('video'), locale: z.enum(['zh', 'en']), style: z.string().min(1).max(100), instruction: z.string().max(1000).trim().min(1), baseAssetId: z.string().uuid().optional(), revisionTaskId: z.string().uuid().optional(), revisionMode: z.literal('art').optional(),
