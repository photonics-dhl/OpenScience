...
export function registerPublicationRoutes(app: FastifyInstance, deps: PublicationRouteDeps): void {
  ...
  app.post('/versions/:versionId/publish', async (req, reply) => {
    ...
    const { versionId } = versionParams.parse(req.params);
    const body = z.object({
      r3Confirmed: z.boolean(),
      allowArtifactDownloads: z.boolean().default(false),
      presentationAssetIds: z.array(z.string().uuid()).optional(),
    }).parse(req.body);
    const published = await publishVersion(
      deps,
      { versionId, userId: user.userId, r3Confirmed: body.r3Confirmed, allowArtifactDownloads: body.allowArtifactDownloads, presentationAssetIds: body.presentationAssetIds, publicIdPrefix: deps.publicIdPrefix ?? 'OSR' },
      auditCtx(req),
    );
