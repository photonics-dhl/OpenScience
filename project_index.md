...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；聚合覆盖 runtime/performance、产品交互、12 项 action/motion/zh/en 锁，以及 §13.4 单路径/slender tail、帽顶净空、可见嘴部、`24–48px`、分组重算、renderer-ready 截图和 desktop/mobile 内容/控件排除；default production harness 仍 404 | **LOCAL GATES GREEN / USER VISUAL ACCEPTANCE PENDING** application `9a7263e`；production `cc6cff6` pending hotfix |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko renderer、`360/200px`、Research Folio；§13.3 锁定 12 项动作/Live2D/双语短句；§13.4 纠正气泡/嘴部/页注/遮挡并补 slender-tail；§13.5 固化 design-skill reject/accept 与原尺寸人工审图门 | **CURRENT Hermes visual/guide design / §13.4 LOCAL CANDIDATE `9a7263e`**；production `cc6cff6` awaiting hotfix/user review |
...
| `docs/plans/2026-08-25-hermes-continuous-speech-correction-plan.md` | §13.4 视觉纠错实施链：单闭合 SVG、真实口部、帽顶净空、slender tail、菜单 `24–48px`、阅读避让、原尺寸审图与 ECS 验收 | **HOTFIX LOCAL GATES COMPLETE / DEPLOYMENT PENDING**；application `9a7263e`，production `cc6cff6` |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：§13.4 视觉纠错、§13.5 design-skill gate、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；local application `9a7263e`；production/rollback `cc6cff6` / `8ed2f3c` |
