...
| `docs/specs/2026-09-18-figure-plan-reuse-binding.md` | `figurePlan.reuse` 的论文原图绑定设计提案：`paper_original_figure` 新 subtype、`(researchObjectId, versionId, figureId)` 查找、版本作用域、`paper_original_missing_<id>` 失败模式、parser 检测 vs 手动上传 | 设计提案，未实现；figurePlan 接线（`dfbcc593`）已先定 `reuse = 不生成场景` 的暂定语义 |
