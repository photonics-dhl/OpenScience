...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 高保真生产共享规范：Task 19 建立透明 overlay 基线；Task 20 由同计划的增量合同将 idle 调为 `.05/4.5px/5s`，其余 70ms、700ms local zero、5px follow、10px combined、.18 gain、纵向 .20/横向 .14 与 overlay `<=.04` / final `<=.08` 不变 | 物理手机门禁已豁免；release `28c7789` 已部署并通过公网回归 |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、deploy.sh/DNS-01 实证）；§5.3–5.4 登记 Task 19 shared overlay 与 Task 20 idle `.05/4.5px/5s` 增强、手机风险豁免及上线前后门禁 | 已部署：`28c7789`，rollback `b6a41da`，backup `280K 7/7`，公网回归 GREEN |
