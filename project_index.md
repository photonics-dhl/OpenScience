...
> **CURRENT source/deployment anchor（2026-08-25 17:26 +08）：** working branch `codex/hermes-wanko-live2d`；deployed application source / immutable release 为 `1b3bada` / `8ed2f3c`；local main / origin main `c60ffdd` / `7eb2f5b`；ECS release / rollback 为 `8ed2f3c` / `7165e9b`。只有本锚点与主题唯一 CURRENT handoff 可决定当前状态。
...
| `apps/web/components/ui/context-menu.tsx` / `apps/web/components/hermes/{HermesVisualAdapter,HermesPresenceControl}.tsx` / `apps/web/lib/hermes/context-menu-actions.ts` | 无主题 Radix context-menu 行为原语、一张不遮挡角色的 carried tool sheet、12 项 action/motion/双语真源与 original/compact/quiet 控制；桌面右键、Shift+F10/Menu、移动长按，普通点击保留 assistant drawer；研究动作进入真实 Hermes/Files/Versions surfaces | **CURRENT DEPLOYED `1b3bada` / release `8ed2f3c`**；不继承 shadcn 默认视觉 |
...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；现行聚合覆盖 runtime/performance、19 个 runtime/guide E2E、产品交互、12 项 action/motion/zh/en 锁，以及 §13.4 单一路径气泡、可见嘴部、`24–48px` 双向间距、分组重算与 desktop/mobile 内容/控件排除；default production harness 仍 404 | **LOCAL GATES GREEN / USER VISUAL ACCEPTANCE PENDING** application `a404ca7`；production `8ed2f3c` remains visually rejected |
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与五条 Hermes 入口/输入抑制/真实研究导航/反馈几何的 62 案 production browser 发布门禁；覆盖 landmark/focus/overflow/error/LCP/transfer/DOM 与 actor/menu/口部反馈不遮挡 | **CURRENT local release gate `62/62` + public Hermes `5/5` GREEN**；公网全矩阵 `59/62`，仅生产 Basic Auth 在应用前拦截 `/admin` 三视口；application `1b3bada`，ECS `8ed2f3c` |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko renderer、`360/200px`、Research Folio；§13.3 锁定 12 项动作/Live2D/双语短句；§13.4 纠正断裂气泡、伪嘴部锚点、悬空页注与菜单遮字；§13.5 固化 design-skill reject/accept 与原尺寸人工审图门 | **CURRENT Hermes visual/guide design / §13.4 LOCAL CANDIDATE `a404ca7`**；production `8ed2f3c` VISUALLY REJECTED pending replacement/user review |
...
| `docs/plans/2026-08-25-hermes-continuous-speech-correction-plan.md` | §13.4 视觉纠错实施链：单闭合 SVG 气泡轮廓、真实口部连接、菜单 `24–48px` 有界间距、阅读区避让、原尺寸审图与 immutable ECS 验收 | **IMPLEMENTATION + LOCAL GATES COMPLETE / DEPLOYMENT PENDING**；application `a404ca7`，production `8ed2f3c` visually rejected |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：§13.4 视觉纠错、§13.5 design-skill gate、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；local application `a404ca7`；production `8ed2f3c` visually rejected，rollback `7165e9b` |
