...
| `apps/web/test/{landing-page,optical-field,landing-motion-policy,central-particle-dependencies}.test.ts{,x}` | Landing 基线/路由/单一标题/朱红记忆点、glyph safe zone、指针时间插值、对称狭缝衍射波前、Open RO 六节点与 reduced-motion 回归门禁；中央粒子原型的精确 Three/postprocessing/opentype pins、所有 dependency scope 的 GSAP/ControlKit/glslify/额外 React wrapper/hosted runtime 排除与 production Landing import-isolation 合同 | 活文档 |
