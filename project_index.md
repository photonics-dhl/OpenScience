...
> **CURRENT source/deployment anchor（2026-09-08 +08）：** branch codex/open-research-publication；application/production/public 134ebdea796b922ac07d9f29b6148b458b5e515f 已上传并部署，rollback 97aa06a542febf558a47cecbf6559bd0a6c876aa。本轮M1/M2核心及merge连续性验收完成；后续docs HEAD不代表另一应用版本。
...
| 路径 | 用途 | 状态 |
|---|---|---|
| `docs/specs/2026-09-07-open-research-publication-prd.md` | 开放研究出版物 PRD v1.1：四阶段建设，每 RO AI 友好 API、固定版本/权限合同及独立客户端验收 | M1/M2核心已部署134ebdea；M3/M4后续阶段 |
| `infra/migrations/20260907010000_frozen_research_record/` / `packages/domain/src/commit/{research-record-snapshot,research-record}.ts` / `apps/api/src/routes/{research-record,research-record-schema}.ts` | 提交时冻结记录、固定版本读取/来源/JSON 导出、机器 Schema/OpenAPI；nullable 迁移 37 与回滚 SQL | DEPLOYED134ebdea；core37/search2，线上固定记录验收通过 |
| `apps/web/components/research/VersionRecord.tsx` / `apps/web/lib/research-materials.ts` | 固定版本预览、材料恢复与并发附件保护 | DEPLOYED134ebdea；Web564与产品浏览器125通过 |
| `docs/specs/2026-09-07-research-record-api.md` | 每 RO 固定研究记录 API：权限、引用、来源、JSON 导出与机器合同 | DEPLOYED134ebdea；权限/导出/固定引用线上通过 |
| `apps/agent-worker/src/parsers/native-pdf-text-items.ts` / `apps/agent-worker/src/parsers/cascade-orchestrator.ts` / `apps/agent-worker/src/parsers/text-extractor.ts` | 保留科学文本的 PDF 几何与 CMSY 否定符恢复，无法证明保真时保持待核查 | 保留于DEPLOYED134ebdea；parser16验收通过 |
| `apps/agent-worker/test/extraction-confirmation.test.ts` | 真实 extractHandler → 确认 → 冻结 API 的组合回归；位置歧义与错误身份不能升级为确定证据 | 134ebdea；17组合回归通过并独立批准，含canonical block IDs与多段显式缺失 |
| `docs/plans/2026-09-07-open-research-publication-plan.md` | 原子导入确认、材料一致性、来源核查和每 RO 只读 API；独立审查与精确 SHA 发布 | COMPLETED本轮M1/M2核心；production134ebdea/rollback97aa06a5 |
| `packages/domain/src/ingestion/{ingestion-service,ingestion-evidence}.ts` / `packages/domain/src/commit/commits.ts` / `apps/api/src/routes/ingestion.ts` / corresponding ingestion tests | 确认原子创建真实 Version、幂等重放、材料恢复与保守来源匹配；保留既有草稿图谱编辑 | DEPLOYED134ebdea；含merge连续性修复，真实PG确认/合并/编辑/删除/回滚通过 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT：研究记录/API、保留媒体功能和上线验收 | **CURRENT active-memory**；production134ebdea/rollback97aa06a5，发布与线上流程验收完成 |
