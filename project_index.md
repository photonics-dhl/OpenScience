...
研发状态可视化：[`docs/proposals/2026-09-23-hermes-development-live.html`](docs/proposals/2026-09-23-hermes-development-live.html) 是只读 HTML 展示缓存，按论文展示各阶段产物与质量边界，优先读取 ignored `tmp/hermes-development-live-feed.js`，15 秒轮询并标记过期；页面内有离线审查快照。feed 只在开发者核对 CURRENT/资产后按检查点更新，不是后台自动遥测、独立任务库或产品正式审阅入口；准确执行状态仍以 [Hermes CURRENT](docs/handoff/2026-09-10-hermes-web-image-handoff.md) 为准。
...
Pixel science成功/art主调用超时的精确同task一次恢复复用`research-run.ts`/worker `handler.ts`原planning-retry及pixel/source审计；已保存方案的末审timeout续接复用原output-resume及Gateway内部授权，原提示绑定与内部结构修复分离、逐次核当前权限/来源/provider/调用审计；旧zero-call与legacy收据保持。配图标题证据的消费投影共用于Domain `illustration-brief.ts`与worker `handler.ts`，复用原SourceMap，不改Evidence身份；`presentation-asset.ts`/`research-run.ts`原GET与retry-generation识别来源支持失效、保存原receipt并创建同run新计划，旧approved图不改；source-support root可沿原科学拒收revision链继续，复用原root proof与有界history。`illustration-review.ts`消费同chain原反馈，仍沿原M3末审，真实效果见CURRENT。`review-runner.cjs`负责未发送同draft正文丢后缀时的一次完整重填与原全文/附件/模式/deadline确认。图片技术子集恢复沿`research-run.ts`/`presentation-asset.ts`/worker `handler.ts`原入口，已存PNG只审阅、未提交失败另建替代任务；`presentation-asset.ts`原科学返工单独保留unknown旧提交，技术恢复仍拒绝unknown。原动态授权/来源/存储引用与审计保持；`review-broker.mjs`、`science-review.ts`提供只读未提交凭据，`gateway.ts`内部primary-only限制新恢复，原zh/en恢复文案显示任务数。队列期限与固定执行时间沿`codex-image-protocol.ts`/`codex-image.ts`、worker `index.ts`及既有image core/broker/runner，复用private/started、不新增请求字段；边界见部署runbook，候选/部署见CURRENT。

混合终态科学修订与已完成图审回收沿用`research-run.ts`原retry-generation；终态凭据沿`presentation-asset.ts`与`infra/chatgpt-browser/broker.mjs`现有spool，Gateway只读consumer保持原prompt/source身份与授权。`illustration-review.ts`原final阶段核对叙事各字段科学一致性，无新增分析器或模型阶段；候选/部署/真实结果只见CURRENT。共享storyboard parser在创建端和worker复用，原retry-generation支持精确模型前零提交失败恢复原task/step/run，managed task不走独立retry；零新增逻辑额度、来源与root receipt校验沿原research-run，候选/部署状态见CURRENT。无候选技术失败的同任务显式planning重试亦沿原retry-generation、task执行次数与审计，worker只认唯一收据，保持原逻辑额度；随后存成的checkpoint可沿原入口仅续接末审。拒收后的同任务art-only修正沿同一Domain授权/审计与worker私有storyboardArtCorrection，原拒收不覆盖，付费提交CAS与成功证据保留也在原handler；公开任务投影在agent.ts隐藏这些内部证据。文本方案审阅与Chat像素协议容量分别由`packages/ai-gateway/src/science-review-protocol.ts`导出。视觉叙事、原任务分镜checkpoint恢复及已拒绝图片的有界科学返工沿用 `packages/domain/src/agent/research-run.ts`，真实图片反馈绑定在 `packages/domain/src/assets/presentation-asset.ts`；新叙事终审只判定accepted/blocked，历史修订后的完整候选验收仍由 `apps/agent-worker/src/presentation/{handler,illustration-review}.ts` 原终审完成；叙事模型输入的紧凑来源投影见 `apps/agent-worker/src/scientific-writing-source.ts`（不改变服务器完整来源身份）；内部已审来源消费见 `ingestion/automatic-review.ts`、人工/系统保存历史见 `ingestion/saved-source-commit.ts`，显式有界来源补审/连续历史谱系及真实审计/原稿身份见 `ingestion/source-review-recovery.ts`（均在domain/src），实际图片审阅见 `apps/agent-worker/src/presentation/generated-image-review.ts`，私有阅读叙事标题/说明沿用 `apps/web/components/presentation/PresentationWorkbench.tsx` 的原场景投影；thinking-only与有正文截断共用显式输出升级、长文本请求级传输时限/白名单错误码见 `packages/ai-gateway/src/{provider,gateway}.ts`（复用已锁定Undici）。运行设置及grant约束迁移分别在 `infra/migrations/20260921010000_visual_narrative_settings/`、`infra/migrations/20260921020000_visual_narrative_grant/`；一次科学返工额度约束见 `infra/migrations/20260922010000_visual_narrative_scientific_correction/`，真实图片拒收后明确追加2项的11→13约束见 `infra/migrations/20260922020000_visual_narrative_accepted_image_correction/`；追加单张渲染任务的格式约束及保护性回滚见 `infra/migrations/20260922030000_visual_narrative_dynamic_render_repair/`；原retry-generation/Domain审计负责实际授权，Web `generationRecovery=image-render`复用原恢复按钮。交付/回退顺序见部署runbook相应节，实际效果只见CURRENT及能力台账。多场景正式像素反馈的通用科学返工仍沿research-run/presentation-asset的原retry-generation与handler消费，Web只扩generationRecovery及原恢复按钮；planning.supportingSourceIds/全场景绑定诊断沿illustration-planner，原supports守卫不变，实际候选/部署状态查CURRENT。共享scene-image wrapper显式传递既有1280×720横向画幅要求，仍由原像素审阅检查补边/背景连续性，不改变科学/艺术字段或receiver。Worker主入口独立顺序maintenance复用原scheduler/outbox，避免长图占用lane0时全站状态延迟；常规narrative science沿illustration-planner原调用直接65K/600s/primary-only，art与非narrative不变。Worker/API停止领取、在途排空和部分初始化失败的资源回收分别沿用 `apps/agent-worker/src/{index,journal-worker}.ts`、`apps/api/src/index.ts`，部署边界见原runbook，不另设清理服务。
...
长文恢复沿 `packages/domain/src/ingestion/ingestion-service.ts` 的原retry与内部source-composition refresh、`apps/agent-worker/src/index.ts` 私有sourceMap checkpoint与 `extractor.ts` 既有分窗归并，UI复用原Hermes来源页；`extractor.ts`两条section-map共享同一结构说明/guard诊断/有界反馈，首个失败后停止领取并等待在途完成。精确旧全文错误仅一次恢复、审计保存失败历史，不另建任务/分析器/API或SourceMap协议。实现/部署/真实第三篇规模分开记录于CURRENT与详细交接。
...
| `apps/web/components/presentation/PaperFigureUpload.tsx` / `apps/web/lib/api.ts` / `packages/domain/src/assets/paper-figure.ts` | 真实论文原图上传入口、作用域与反馈、原图格式/存储/幂等登记；候选/部署/真实证据仅见 CURRENT |
| `docs/specs/2026-09-18-figure-plan-reuse-binding.md` | 原图复用实现合同：Artifact上传→artifactId登记draft→人工来源审核→approved reuse；版本/Blob身份、同图号修订、来源与成品的发布边界。实际版本与真实论文证据只见CURRENT。 |
...
| `apps/agent-worker/src/skills/scientific-critical-thinking.ts` / `apps/agent-worker/src/extractor.ts` | 共享科学规则；综合与逐主张来源审校分别注入，P编号回填原文，终审按实际消费目的提供主张 | 历史真实67观察/94段曾经Chat6Pro复核；当前Skill版本、部署与真实质量见CURRENT，调用不等于科学通过 |
...
| `apps/web/components/hermes/{HermesResearchRunPanel,HermesSourceReview,HermesClaimEvidenceReview}.tsx` / `apps/api/src/routes/research-runs.ts` | 用户启动、来源绑定流程找回与未完成生成恢复入口 | 当前候选/生产/实际路径及剩余阻塞仅见CURRENT；来源绑定找回不自动新建或重发 |
...
| `infra/scripts/disk-cache-maintenance.sh` / `infra/systemd/openscience-disk-cache-maintenance.{service,timer}` / `infra/scripts/install-disk-cache-maintenance.sh` | 每日回收**可再生产**缓存：docker build cache、dangling 镜像、journal 超限部分；并只读报告历史 release 数量与体积。**刻意不回收历史 release**——`production-release-retention.mjs` 按要求绑定发布事务、不作独立清理入口，历史清理仍为"用户授权 + 留收据"（流程见 `docs/runbooks/deployment.md`）。`install-*.sh` 为受控安装/漂移检查驱动（无参=只读报告漂移，`--confirm`=按 cloudflared 先例同步版本化 unit+脚本：备份 `.pre-deploy`、`bash -n` 与 `systemd-analyze verify` 后才 enable、装后 SHA-256 读回比对） | 已部署云上（`/usr/local/sbin/openscience-disk-cache-maintenance`，timer enabled+active，2026-09-17；漂移检查 NO_DRIFT） |
