...
科学与美术制作说明容量：`packages/domain/src/assets/illustration-brief.ts`复用原整份brief容量，`apps/agent-worker/src/presentation/{illustration-planner,illustration-review,scene-image}.ts`区分内部说明与读者文字；`packages/ai-gateway/src/{image,codex-image-protocol}.ts`及`infra/chatgpt-browser/runner.cjs`区分MiniMax专属1500与现有spool字节保护，部署先receiver后应用。实际状态只见CURRENT。
