...
> **CURRENT source/deployment anchor（2026-08-29）：** branch `codex/hermes-wanko-live2d`；implementation/production application/release `28a3d5ca681b7744fae521dfa9154100a24e8845`；rollback `c5817121bddbd065c5ecb38811da8e707e6e5d17`。Exact CI `33235948918`、schema 3 `hermes-parser-14-2-v1` 14/2/0/0、structured fake 14/external 0、production runtime/migrations/BGE/public/journal 全绿；Taskmaster 6/12，Tasks 7/10 dependency-ready。Docs-only closeout HEAD 不改变 production 身份。
...
| `docs/plans/2026-08-29-hermes-parser-acceptance-debt-closeout-plan.md` | Taskmaster Task 4 follow-up 的 TDD 计划：共享 MIME、受控 Notebook/Python、正式 table locator、schema-v3 14/2 acceptance、ECS immutable deploy 与卫生证据 | **COMPLETED / ECS DEPLOYED `28a3d5c`**；rollback `c581712`，Tasks 1–6 checked |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、CPU parser 14/2 验收、磁盘分类、固定约束与 version tuple | **CURRENT active-memory**；branch/implementation/production `codex/hermes-wanko-live2d` / `28a3d5c`，rollback `c581712` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.38 acceptance/FD9 transaction，§5.39 parser recovery，§5.40 14/2 closeout/disk inventory | **CURRENT**；application/release `28a3d5c`，rollback `c581712`，production acceptance GREEN；cleanup 待 exact whitelist 批准 |
