...
> **CURRENT source/deployment anchor（2026-08-25 02:05 +08）：** working branch application source `codex/hermes-wanko-live2d@eb55820`；immutable deployed release `3010903`；local main / origin main `c60ffdd` / `7eb2f5b`；ECS release / rollback 为 `3010903` / `33418fd`。只有本锚点与主题唯一 CURRENT handoff 可决定当前状态。
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与三条 Hermes 右键/键盘/长按/反馈几何的 60 案 production browser 发布门禁；覆盖 landmark/focus/overflow/error/LCP/transfer/DOM 与 actor/menu/口部反馈不遮挡 | **CURRENT Research Folio release gate `60/60` GREEN**；application `eb55820` fresh，production focused `3/3` |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：mouth-anchored speech、Research Folio release、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；application `eb55820`；production `3010903`，rollback `33418fd` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows Git Bash 专用密钥调用与版本化资产）；§5.3–5.18 登记 Optical、Hermes 与 Research Folio 生产发布 | CURRENT；最新 ECS release/rollback `3010903` / `33418fd`；公网 Hermes focused `3/3` |
