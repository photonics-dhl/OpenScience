...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/*.ts` | 精确 `/_visual/optical-lab` 隔离比较页：reference/current/candidate 三联屏、SSR selectable h1、fresh-canvas transactional WebGL2→WebGL1 half-float→DOM/static、固定 aperture flow/glyph displacement、WebGL2/ANGLE instanced edge particles、policy/context recovery 与可测 diagnostics | Task 8 Steps 3–5 + review fix round 1 完成；route chunk 与生产 `/` 隔离，Step 6 待用户选择 |
...
| `apps/web/test/optical-lab-model.test.ts` / `apps/web/test/optical-lab-contract.test.tsx` / `apps/web/test/visual/optical-lab-gate.mjs` / `apps/web/test/visual/optical-lab-shots.mjs` | Optical Lab TDD/production-browser 门禁：语义/隔离、candidate-only case-qualified pointer frames、真实像素差与 forbidden geometry/ghost probe、transactional WebGL2 failure→WebGL1、真实 instanced draw 拦截、context loss/restore 与 GL create/delete cleanup | 测试工具；输出位于已忽略 `test/visual/out/optical-lab/` |
...
| `docs/handoff/2026-08-11-optical-lab-task8-steps3-5-handoff.md` | Optical Lab Task 8 Steps 3–5 + session freeze 交接：commit、独立 review pending、人工视觉否决点、Git Bash 服务器预检、隔离部署/回滚顺序与 Step 6 用户选择门禁 | 当前 handoff；下个 session 先读 |
