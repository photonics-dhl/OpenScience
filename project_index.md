...
> **CURRENT source/deployment anchor（2026-08-28）：** branch `codex/hermes-wanko-live2d`；Task 4 local/remote candidate implementation `85ba051f8b080161c1d9766ccd761782181e2d3b`；production application/release `e2c0eaf3b13a220a8bc2cd49b2c1dfe40a6fd61f`；rollback `8163f8b4218e529ee4be41bb9fc732ff6497931a`。Task 4 evidence HEAD 为 docs-only，不改变 candidate 或 production application 身份。
...
| `scripts/verify-release-source.mjs` / `scripts/release-sync-command.mjs` / `scripts/cloud-sync.mjs` / `scripts/evaluation-source-sync.mjs` / `scripts/evaluation-source-sync-command.mjs` / `scripts/evaluation-source-sync-command.test.mjs` | 发布源守卫与云同步：生产 release 要求 clean `HEAD == release ref` 并写入 write-once `/opt/openscience-releases/<sha>`；评测同步仅允许 exact `git archive <sha>` 写入 `/opt/openscience-evals/document-parser/<sha>/source`，拒绝 active/release/mismatched target | **CURRENT release/evaluation boundaries**；完整输入、精确路径与 failed-stage cleanup 合同 GREEN |
...
| `apps/agent-worker/Dockerfile` / `apps/agent-worker/Dockerfile.parser` / `apps/agent-worker/parser-image/{package.json,package-lock.json}` / `apps/agent-worker/src/{clamav,ingestion-parser,parser-job-isolation,parser-service}.ts` / `apps/agent-worker/src/parsers/{page-quality,ocr-parser}.ts` | Worker 与 lockfile 固定的自包含 document-parser；构建期经 ECS Squid，运行时无网络/Secret、非 root、只读、2 CPU/512MiB/64 PID、128MiB bounded IPC；Tesseract `eng+chi_sim` 选页 OCR 具 stage-wide 资源预算、串行执行与 kill-and-close；ClamAV fail-closed | **TASK 5 CODE `622cc24` / ECS RUNTIME PENDING**；独立 review Ready、exact-SHA full build GREEN，packaged scan/CPU responsiveness 尚未验收；production 仍 `e2c0eaf` |
...
| `apps/agent-worker/src/parser-acceptance-runner.ts` / `infra/scripts/accept-document-parser-release.{sh,test.mjs}` | Task 8 独立生产验收：exact-SHA worker/parser 镜像、artifact-backed `sdf.extract` 真实 cascade、schema-v2 16-case、stage/confidence/locator/false-ready/P50/P95/CPU/RSS/image ID 证据；无网无 capability、只读非 root、bounded tmpfs/volume、原子报告与 exact-run cleanup | **PREDEPLOY REVIEW READY ONLY**；本地仅跑无 Docker contract；ECS 执行仍需 Phase B 明确授权 |
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 5/12 done，Tasks 4、7 dependency-ready，Task 10 等待 Task 4 | 工具自管；Task 1–3、5、6 已关闭，进度 42% |
...
| `docs/plans/2026-08-27-hermes-cpu-parser-cascade-plan.md` / `.superpowers/sdd/2026-08-27-hermes-cpu-parser-cascade-plan/task-5-implementer.md` | Taskmaster Task 4 的 ECS-only 实施计划与 Task 5 Tesseract/page-quality 实施证据：结构化 sidecar v2、确定性 source map、候选 bake-off、坐标 OCR、受控 LLM candidate、真实 corpus/资源/部署门禁 | **CURRENT / TASK 5 CODE COMPLETE, ECS RUNTIME PENDING**；`622cc24` independent Ready，production 未变，MiniMax Vision disabled |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence Tasks 1–3、5、6、ECS 验收、固定约束与 version tuple | **CURRENT active-memory**；branch/application `codex/hermes-wanko-live2d` / `8163f8b`，rollback `f9659668` |
...
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` / `scripts/verify-embedding-runtime.mjs` | ECS-only immutable release 部署；core/search 隔离、双账本、按需 BGE profile、逐 release capability/rollback image、strict identity + embedding canary、原子 backup 脚本刷新 | **CURRENT PRODUCTION `8163f8b`**；rollback `f9659668`，合同 `19/19` GREEN |
...
| `infra/migrations/` | Prisma 迁移 1–29（29 = `agent_tasks.execution_attempt` worker fencing；28 = Research Intelligence core metadata；27 = `agent_tasks.retry_count`），各附 rollback.sql | **CURRENT PRODUCTION 29/29**；active ledger 30 含需保留的历史 `20260809010000_ro_create_idempotency` |
