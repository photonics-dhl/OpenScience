...
> **CURRENT source/deployment anchor（2026-08-30）：** repository main `cf63392a617d117773a350a90ec7e59b439919d4`；application source/production release `2fa10aa8ac3a541e794bdb40456b4f8dee826e4c`；rollback `4c73469fe24abe685054f1d917d452adc5371d35`；production core/search `31/31` / `2/2`。Task 9 已部署、真实生产验收并合并；Taskmaster 9/12，Task 10 dependency-ready。merge/docs-only HEAD 与 ECS application source 不得混写。
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 9/12，Tasks 1–9 done | 工具自管；Task 10 dependency-ready |
...
| `docs/plans/2026-08-30-hermes-external-retrieval-lifecycle-plan.md` | Taskmaster Task 10：Semantic Scholar/Tavily/ScanSci legal-only adapter、rights provenance、72h 私有缓存、10min 认证签名链接、租约 GC 与 ECS canary | **CURRENT IMPLEMENTATION PLAN**；ScanSci 上游灰色来源/Tor 默认路径禁止原样进入生产 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、Task 9 验收、磁盘卫生、固定约束与下一动作 | **CURRENT active-memory**；main `cf63392`，production `2fa10aa`，rollback `4c73469`；Task 10 next |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.38 acceptance/FD9 transaction，§5.43 Task 8 hygiene，§5.45 Task 9 production journey/cleanup | **CURRENT**；application/release `2fa10aa`，rollback `4c73469`，根盘 35G/25% 且 production GREEN |
