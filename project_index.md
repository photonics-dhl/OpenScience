...
| `apps/web/components/brand/{OpticalHeadline,OpticalField}.tsx` / `apps/web/lib/optical-field/*.ts` | Optical Editorial Landing 品牌媒介：保留原标题布局与单一可访问 DOM 标题；固定 aperture、真实 glyph alpha 粒子采样、上游压缩/下游重构及 pointer energy-only modulation；无 pointer disk/turbulence duplicate/quadratic fan | `cd5be36` ECS 已部署；本地 161/161 与公网独立 60/150/300ms active-frame 全矩阵通过；`b45e002` 为已否决基线 |
