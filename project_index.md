...
| `infra/scripts/backup.sh` | core PostgreSQL + 对象存储备份；当前 `--db` 尚不覆盖独立 search DB | 已部署；Task 2 backup `436K files=7/7`，search backup/restore 须在写入业务数据前补齐 |
...
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` | ECS-only immutable release 部署；core/search isolation、双迁移账本、继承 env、无 URL 日志、回滚与运行验收 | **CURRENT / DEPLOYED `f965966`**；rollback `ef043eb`，最新 Task 5 使用 `--skip-migrate`，合同 `17/17` GREEN |
