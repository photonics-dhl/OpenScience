...
> **CURRENT source/deployment anchor（2026-08-30）：** repository main `463c8e3a2a80138cda2d669c370c0481ed4c0877`；application source/production release `689331845574612130f223d08c92e61721c16586`；rollback `c435c4c8b2800bb20998fd9a9a93f2db96328661`；production core/search `32/32` / `2/2`。Task 10 因 ScanSci 默认能力缺口重新打开；Taskmaster 9/12，Task 11 blocked。docs-only HEAD 与 ECS application source 不得混写。
...
| `infra/scripts/deploy.sh` / `infra/scripts/production-deploy-lock.mjs` / `infra/scripts/production-deploy-transaction.sh` / `infra/scripts/production-deploy-transaction-state.sh` / `infra/scripts/verify-document-parser-acceptance.mjs` / `infra/scripts/deploy.test.mjs` / `scripts/verify-embedding-runtime.mjs` | ECS immutable release：锁外只物化 exact archive；锁内单一远端 runner 持 FD9 完成 active/source/report/runtime/image、双账本、switch/health、expected-only CAS、公网验收与 durable-journal rollback；same-SHA 也重验正式 acceptance 与实际容器 image | **PRODUCTION CONTRACT GREEN `6cabe422`**；CI `33240457443` / job `99068791412`，rollback `28a3d5c`，no migration |
| `infra/compose/docker-compose.{prod,dev}.yml` / `infra/scripts/{provision-scansci-secrets,verify-scansci-runtime}.{mjs,test.mjs}` / `infra/scripts/production-{deploy-transaction,release-retention}*` | ScanSci SHA-tagged legal/auth topology；root-only host files → bounded networkless init → UID10001/0400 分权 named Secret volumes；persistent session、pre-Worker runtime gate、exact previous/absent rollback 与 active+rollback tag retention | **TASK 4 LOCAL CONTRACT GREEN / ECS PENDING**；生产 release/rollback 仍为 `6893318` / `c435c4c` |
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 9/12，Tasks 1–9 done、Task 10 reopened | 工具自管；Task 11 blocked |
...
| `docs/specs/2026-08-26-hermes-research-intelligence-platform-design.md` | Claim-first RO、可定位 Evidence、身份静默路由、CPU parser/OCR、BGE-M3 hybrid retrieval、数据库分离、富媒体与 Hermes 受控权限的统一产品/技术合同；§7.2.1 固定 parser debt closeout 为 14/2/0/0、正式 table locator 与 v3 原因码报告 | **CURRENT / TASKS 1–9 DONE / TASK 10 REOPENED**；production `6893318`，rollback `c435c4c`，Taskmaster 9/12 |
| `docs/specs/2026-08-30-scansci-default-capability-design.md` | 一次浙江大学 CARSI 认证→持久 Hermes 默认文献下载；可选 root-only 账号 Secret、隔离 legal-only 服务/认证 helper、统一 acquisition API、Personal Space/Hermes/RO 多入口与 ECS 验收 | **CURRENT APPROVED / IMPLEMENTATION PLAN READY**；Task 10 completion gate，Task 11 blocked |
...
| `docs/plans/2026-08-30-hermes-external-retrieval-lifecycle-plan.md` | Taskmaster Task 10：Semantic Scholar/Tavily/ScanSci legal-only adapter、rights provenance、72h 私有缓存、10min 认证签名链接、租约 GC 与 ECS canary | **PARTIAL / TASK 10 REOPENED**；metadata/rights/temp lifecycle production，默认 ScanSci capability pending |
| `docs/plans/2026-08-30-scansci-default-capability-plan.md` | 重开 Task 10 的 10-task TDD 执行链：source-locked Python legal service、持久 CARSI/auth helper、Compose/release recovery、统一 acquisition API、四类 Web 入口、安全复审与真实 ECS 旅程 | **CURRENT IMPLEMENTATION PLAN**；upstream archive SHA-256 `db537914…9208b9`，Task 11 blocked |
...
| `apps/web/components/dashboard/LiteratureAcquisition.tsx` / `apps/web/lib/{api,literature-acquisition-state}.ts` / `apps/web/app/dashboard/page.tsx` / `apps/web/test/literature-acquisition-state.test.ts` / `apps/web/test/e2e/auth-dashboard.spec.ts` / `packages/domain/src/retrieval/{browser-result,retrieve-payload,literature-acquisition}.ts` / `packages/domain/src/agent/agent.ts` / `apps/agent-worker/src/retrieval/handler.ts` | ScanSci plan Task 7 Personal Space acquisition：server `canRetry` shared predicate、retry marker/payload/authority、全历史 recovery、pre-await retry lock、409 reconcile、permanent poll recovery、44px/inert link 与临时下载 | **LOCAL TASK 7 REVIEW CANDIDATE `3e829db`**；7 条 component journey + Domain/API/Web gates green；ECS 375px/one-use download acceptance pending |
| `apps/agent-worker/src/retrieval/` / `apps/api/src/routes/temporary-documents.ts` / `packages/domain/src/retrieval/` / `infra/migrations/20260830010000_external_retrieval_lifecycle/` / `packages/storage/src/minio-adapter.ts` | Task 10 provider-normalization、rights、tenant-bound task、72h temp object、600s one-use cookie、租约 GC、SeaweedFS checksum metadata 兼容 | **CURRENT PRODUCTION `6893318`**；Semantic Scholar real 3-source canary、PDF hash/download/replay/GC/provenance 全绿 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、Task 10 重开、磁盘卫生、固定约束与下一动作 | **CURRENT active-memory**；main `463c8e3`、production `6893318`、rollback `c435c4c`；Task 10 ScanSci next |
...
| `docs/decisions/ADR-012-scansci-default-literature-acquisition.md` | ScanSci legal/auth 网络拓扑、file-Secret UID workaround、持久 session、不可变部署/回滚与精确保留边界 | Accepted；Task 4 local，ECS/CARSI acceptance pending |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.38 acceptance/FD9 transaction，§5.46 Task 10 provider/PDF/GC/hygiene journey | **CURRENT**；application/release `6893318`，rollback `c435c4c`，根盘 36G/25% 且 production GREEN |
...
| `infra/migrations/` | Prisma 迁移 1–33（33 = ScanSci provider state + Notification 幂等；32 = 外部来源/权利/临时文档；31 = 账号 Evidence 阅读偏好），各附 rollback.sql | **REPOSITORY 33 / PRODUCTION 32**；search 2/2，migration 33 ECS pending，历史迁移账本保留 |
| `infra/schema.prisma` | core Prisma schema：既有平台模型 + Research Intelligence/Identity/Reading/External Retrieval/ScanSci provider state scoped relations | **LOCAL CANDIDATE migration 33 / PRODUCTION 32** |
