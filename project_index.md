...
> **CURRENT source/deployment anchor（2026-09-03）：** branch / code release / production release `codex/scansci-upstream-mcp` / `80db41e7ee0a1c8158d3f335dc1b2fbf6f2bb2bf`；rollback `761b93d4bbce77e70d676be78de0bba128974fe6`；core/search `34/34` / `2/2`。两者均为官方 `v1.13.1` MCP 单服务；旧 browser/auth/server runtime、旧 release/验收/eval/dangling image 与 inactive cache 已清理；官方 cookie import、机构 PDF 与四入口 pending。Taskmaster 9/12。
...
| `docs/specs/2026-09-02-scansci-upstream-mcp-design.md` | 上游 `v1.13.1` 官方 17-tool MCP、Worker 直连、官方 cookie import、持久数据/瞬态论文卷、`source_retrieval` provenance 与旧实现清理 | **CURRENT / PRODUCTION**；official-only ECS accepted，institutional/four-entry pending |
...
| `docs/plans/2026-09-02-scansci-upstream-mcp-plan.md` | 五步执行：ECS 官方 MCP 正向试点、`source_retrieval`、Worker MCP、官方 cookie import/immutable deploy、生产验收后精确清理 | **CURRENT PLAN / TASK 5**；official-only deploy/cleanup done，institutional/four-entry journey pending |
| `apps/scansci-mcp/` / `apps/agent-worker/src/retrieval/scansci-mcp.ts` / `infra/scripts/verify-scansci-mcp-runtime*` | 官方 `scansci-pdf==1.13.1` 单一 MCP runtime、17-tool Worker client、durable acknowledge/discard、持久 `scansci-data` 与瞬态 `scansci-papers` | **PRODUCTION BASE + OFFICIAL-ONLY CANDIDATE**；OA green，第二 auth/browser image 已从候选删除；ZJU cookie import/机构 PDF pending |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、官方 ScanSci MCP、磁盘卫生、固定约束与下一动作 | **CURRENT active-memory**；production `80db41e`、rollback `761b93d`；official-only deploy/cleanup done |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE/ScanSci 隔离、Windows Git Bash）；§5.49–5.63 为历史 incident，现行操作以 §5.64 official-only 章节为准 | **CURRENT**；production `80db41e` / `761b93d`，official-only accepted |
