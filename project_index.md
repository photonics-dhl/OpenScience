...
| `infra/scripts/backup.sh` / `docs/runbooks/backup-restore.md` | core/search PostgreSQL 独立 dump、SHA-256 与集合 manifest + SeaweedFS volume 非破坏备份入口；先校验 active release 与数据库隔离 | **TASK 6 CANDIDATE**；静态合同 GREEN，待 ECS 备份/恢复演练 |
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` | ECS dry-run/confirmed release 部署；完整 Git archive、SHA root/image、非 root 只读 mount、显式 rollback ref、双账本迁移、BGE-M3 模型卷验证与回滚 | **TASK 6 CANDIDATE**；禁止 `--skip-build`，公网成功前保留可验证上一 release |
...
| `test/research-intelligence/search-evaluation.json` / `apps/agent-worker/test/search-evaluation-corpus.test.ts` / `infra/embedding-candidates/bge-m3/` / `infra/scripts/evaluate-embedding-models.sh` | Task 6 ECS-only BGE-M3 dense 候选 gate：16 chunk/24 query 自著双语 corpus、query/corpus encoder 分流、exact wheel/model lock、content-free nDCG/Recall/latency/RSS 与 network-none/read-only/CPU 资源限制 | **CURRENT / LOCAL HARNESS GREEN / ECS PENDING**；未下载模型、未 build image、未启用生产 route |
| `apps/agent-worker/src/index.ts` / `apps/agent-worker/test/search-runtime.test.ts` / `infra/compose/docker-compose.{dev,prod}.yml` | `BGE_M3_ENABLED` fail-closed runtime 接线；精确 model identity 注入 SearchStorage/EmbeddingClient；模型卷一次性校验、internal-only worker、非 root/只读/2 CPU/6 GiB/128 PID | **TASK 6 CANDIDATE**；139/139 worker、17/17 deploy contract GREEN，待 ECS Compose/模型验收 |
...
| `infra/scripts/backup.sh` | core/search PostgreSQL 独立 dump + SHA-256/集合 manifest 与对象存储备份 | **CANDIDATE**；生产仍运行旧 core-only 脚本，待 Task 6 ECS 备份/恢复演练后切换 |
...
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` | ECS-only immutable release 部署；core/search isolation、双迁移账本、BGE-M3 image/model-volume gate、继承 env、无 URL 日志、回滚与运行验收 | **CURRENT CANDIDATE / PRODUCTION `f965966` UNCHANGED**；rollback `ef043eb`，合同 `17/17` GREEN |
