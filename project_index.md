...
| [OpenScience Web Design System — canonical](https://www.figma.com/design/gjhowMG7cG4clKwvhvF08E) | 长期账号 Full 席位下的产品网页唯一设计事实源；迁移 parity 与净零写探针通过；Step 9 六屏已建立，Auth→Dashboard→Workspace 与 Dashboard→Public RO→Collection↔Public RO 真实路径已连通；第三检查点 Section `2069:99` 含 Public RO `2069:100`、Collection `2069:101`、状态条 `2069:102`；Landing→Auth 结构补链待收尾；Code Connect 因 student tier 未验证 | Canonical, Step 9 in progress |
...
| `apps/web/app/tokens.css` | 视觉 token 单一事实源（颜色/WCAG；motion/easing/z-index/shadow；2026-08-08 追加 spacing 2–64、四级 radius、Noto 双字体族、九级 type ramp 与 tracking，并镜像至 Figma） | 活文档 |
...
| `apps/web/lib/api.ts` + `test/api-client-contract.test.ts` | 同源 `/api/*` client、内存 CSRF token、受保护写请求与 Nginx/Next routing 合同（1ab4b6d） | 活文档 |
...
| `docs/design-assets/figma/2026-08-09-step9-{public-ro,ultrafast-collection,public-states}-v1.png` | Figma canonical `04 Screens` 的 Step 9 第三检查点本地截图：Public RO 纸张学术阅读面、Ultrafast Science 策展层与公开状态/交互契约；用于 API 中断恢复、视觉确认和原型对照 | Awaiting third-checkpoint approval |
...
| `docs/runbooks/deployment.md` | 部署 runbook（已填充：cloud-sync/迁移/seed/nginx/验证；2026-08-09 已用于 release `3b5a34e` 生产上线；含 API node:22 healthcheck 与 Web force-recreate 说明） | 已填充 |
