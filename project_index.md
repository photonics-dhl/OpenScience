...
> **CURRENT source/deployment anchor（2026-08-29）：** branch `codex/hermes-wanko-live2d`；Task 8 predeploy candidate implementation `82e99869a87ac025000aa3edefc851fe85b03303`；production application/release `e2c0eaf3b13a220a8bc2cd49b2c1dfe40a6fd61f`；rollback `8163f8b4218e529ee4be41bb9fc732ff6497931a`。Candidate exact Ubuntu CI `33192991542` 全绿；ECS Phase B 未启动，docs-only HEAD 不改变 production 身份。
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、CPU parser cascade predeploy 状态、固定约束与 version tuple | **CURRENT active-memory**；branch/implementation `codex/hermes-wanko-live2d` / `82e9986`，production/rollback `e2c0eaf` / `8163f8b` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.35 lexical drill，§5.36 BGE，§5.37 卫生/DOCX，§5.38 Task 8 acceptance + single-runner FD9 transaction/journal | **CURRENT**；candidate `82e9986` predeploy CI green，application/release `e2c0eaf`，rollback `8163f8b` |
