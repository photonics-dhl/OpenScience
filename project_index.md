...
| `apps/web/test/{landing-page,optical-field,landing-motion-policy,central-particle-dependencies,central-particle-geometry}.test.ts{,x}` | Landing 基线/路由/单一标题/朱红记忆点、glyph safe zone、指针时间插值、对称狭缝衍射波前、Open RO 六节点与 reduced-motion 回归门禁；中央粒子原型的依赖/import 隔离，以及字体 geometry 的 viewport/center/baseline/kerning/grid/hash/SVG parity 合同 | 活文档 |
