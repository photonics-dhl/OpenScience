...
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` / `scripts/verify-embedding-runtime.mjs` | ECS immutable release 部署；双账本迁移、BGE profile/显式 deploy gate、逐 release 回滚资产、模型卷与严格 identity/真实向量 canary | **TASK 6 CANDIDATE**；禁止 `--skip-build`，18/18 合同 GREEN，生产仍为 `f965966` |
