...
| `apps/web/components/brand/{OpticalHeadline,OpticalField}.tsx` / `apps/web/lib/optical-field/*.ts` | Optical Editorial Landing 品牌媒介：当前生产为固定 aperture 的 Canvas glyph-particle renderer；第四轮改进基线为连续字形 GPU displacement + shared flowmap + sparse instanced particles | `cd5be36` ECS 工程门禁通过但用户视觉否决；Task 4 已重开，生产替换前先做独立 Optical Lab |
