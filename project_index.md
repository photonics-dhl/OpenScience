...
| `apps/web/app/%5Fvisual/central-particle/page.tsx` / `apps/web/components/optical-prototype/*` / `apps/web/lib/optical-prototype/{runtime-policy,lifecycle,renderer,particle-grid,touch-texture,shaders/*}.ts` | no-index Three.js WebGL2 隔离原型：唯一 SSR h1 + 同源 SVG、route-only dynamic import、2,259 实例规则标题点阵、clean-room TouchTexture、双轴 contain/pointer 同构、abort/timeout/init/context/resize/visibility/cleanup ownership | Native prototype Task 4；真实 instanced draw/非空 framebuffer/宽高受限 resize GREEN；尚未加入 field/curtain/postprocessing，生产 Hero/ECS 未修改；Task 5 前先修正 Task 2 标题 seam-anchor geometry |
...
| `docs/superpowers/plans/2026-08-12-optical-native-particle-prototype-plan.md` | 隔离 Three.js 原型：依赖/许可、opentype 单一几何、规则粒子/TouchTexture、中心弧形场/纵向幕、有限 postprocessing、固定证据与 ECS 发布 | 用户已确认 spec 和服务器部署；等待执行方式确认后安装依赖 |
