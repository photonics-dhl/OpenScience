...
> **CURRENT source/deployment anchor（2026-08-27）：** branch `codex/hermes-wanko-live2d`；application/immutable release `f9659668b237b70b4c018b866e20498689d327c2`；exact implementation CI run `33002216562`；rollback `ef043ebb8e51332effe75a5639cb207aec7bfc47`。后续 docs-only HEAD 不改变 application 身份。
...
| `infra/scripts/backup.sh` / `docs/runbooks/backup-restore.md` | core/search PostgreSQL 私有单飞原子备份集合、独立 SHA-256/manifest + SeaweedFS 原子快照；校验 active release、数据库隔离与双库恢复顺序 | **TASK 6 CANDIDATE**；旧格式双库 ECS restore drill GREEN，新原子格式待最终 SHA 演练 |
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` / `scripts/verify-embedding-runtime.mjs` | ECS immutable release 部署；双账本迁移、BGE profile/显式严格 deploy gate、逐 release 完整运行时身份与回滚资产、停用收敛、模型卷及切换前后/same-SHA identity + 真实向量 canary | **TASK 6 CANDIDATE**；禁止 `--skip-build`，19/19 合同 GREEN，生产仍为 `f965966` |
...
| `apps/agent-worker/src/parser-evaluation.ts` / `apps/agent-worker/test/{parser-evaluation,parser-evaluation-corpus-export}.test.ts` / `infra/scripts/evaluate-document-parsers.sh` / `infra/parser-candidates/liteparse/` / `infra/parser-candidates/docling/` | Task 4 ECS-only 候选 bake-off：content-free/false-ready-safe 报告、schema-v2 16-case/7-PDF gate、无网络/只读/非 root/CPU-RSS-PID-time-64KiB-output 硬限制与原子 evidence；两候选禁用 OCR，Docling 另在 build/preflight 拒绝 GPU 包并输出 package/model aggregate lock | **CURRENT**；LiteParse ECS gate 完成，Docling CPU source-lock local GREEN/ECS pending；均为 `APPROVED_PILOT`，不修改 production Compose |
| `test/research-intelligence/search-evaluation.json` / `apps/agent-worker/test/search-evaluation-corpus.test.ts` / `infra/embedding-candidates/bge-m3/` / `infra/scripts/evaluate-embedding-models.sh` | Task 6 ECS-only BGE-M3 dense gate：评测镜像继承生产 runtime、完整 67-package hash lock、16 chunk/24 query 自著双语 corpus、content-free nDCG/Recall/latency/RSS 与 network-none/read-only/CPU 限制 | **CURRENT / LOCAL HARNESS GREEN / ECS FINAL-SHA PENDING**；`76e0a7a` 下载模型后被 classic-builder `COPY --chmod` 拒绝，兼容修复已完成，生产 route 未启用 |
| `apps/agent-worker/src/index.ts` / `apps/agent-worker/test/search-runtime.test.ts` / `infra/compose/docker-compose.{dev,prod}.yml` | `BGE_M3_ENABLED` fail-closed runtime 接线；精确 model identity 注入 SearchStorage/EmbeddingClient；显式 profile/deploy gate、internal-only worker、非 root/只读/2 CPU/6 GiB/128 PID | **TASK 6 CANDIDATE**；140/140 worker、18/18 deploy contract GREEN，待 ECS Compose/模型验收 |
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 4/12 done，Tasks 4、6 ready | 工具自管；Task 1–3、5 已关闭，进度 33% |
...
| `docs/specs/2026-08-26-hermes-research-intelligence-platform-design.md` | Claim-first RO、可定位 Evidence、身份静默路由、CPU parser/OCR、BGE-M3 hybrid retrieval、数据库分离、富媒体与 Hermes 受控权限的统一产品/技术合同 | **CURRENT / REQUIREMENTS APPROVED / TASKS 1–3、5 DEPLOYED**；application/release `f965966`，Taskmaster 4/12 |
...
| `docs/plans/2026-08-27-hermes-semantic-retrieval-bge-m3-plan.md` | Taskmaster Task 6 的 ECS-first TDD 实施计划：BGE-M3 source-lock/CPU bake-off、独立 search migration、locator-safe chunk、BM25+dense RRF、降级、独立备份恢复与生产验收 | **CURRENT / TASKS 1–4 GREEN**；Task 4 exact-SHA `7d489c5` ECS full build + migration round-trip + PostgreSQL `5/5`；模型、migration 2 与生产 route 均未安装或启用 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence Tasks 1–3、5、Task 6 Tasks 1–4、ECS 验收、固定约束与 version tuple | **CURRENT active-memory**；branch/candidate implementation `codex/hermes-wanko-live2d` / `7d489c5`，application/release/rollback `f965966` / `f965966` / `ef043eb` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.30–5.34 是 Task 3/5、Docling 与 Secret-safe drill 证据，§5.35 是 exact-SHA lexical ECS drill | **CURRENT**；production application/release/rollback `f965966` / `f965966` / `ef043eb`；candidate 未部署 |
| `docs/runbooks/hermes-capability-registry.md` | Hermes Skill/MCP/model/parser/provider 能力台账：注入状态、授权/成本、项目级安装目录、72h PDF TTL、10min signed URL、评测/回滚/退役规则 | **CURRENT capability registry / MACHINE-GATED**；历史 13-case baseline、当前 schema-v2 16-case/7-PDF layout gate、LiteParse ECS 实测与 Docling 2.123.0 source-lock 已记录；Docling ECS 质量/RSS 尚待评测，凭据值不入文档 |
...
| `infra/scripts/backup.sh` | core/search PostgreSQL `0700/0600`、`flock`、staging→directory rename 原子集合 + SHA-256/manifest；对象快照亦原子发布 | **CANDIDATE**；生产仍运行旧脚本，待最终 SHA ECS 失败注入与双库恢复演练后切换 |
...
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` / `scripts/verify-embedding-runtime.mjs` | ECS-only immutable release 部署；core/search 隔离、双账本、按需 BGE profile、逐 release capability/rollback image、strict identity + embedding canary、原子 backup 脚本刷新 | **CURRENT CANDIDATE / PRODUCTION `f965966` UNCHANGED**；rollback `ef043eb`，合同 `18/18` GREEN |
