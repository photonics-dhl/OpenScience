...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` | 明示 flag 才开放的 Wanko 32-action / `336/176` / performance-bubble / layer-isolation harness 与真实像素、比例、mobile-safe、exact-static、90 秒陪伴、renderer cadence、资源及引导门禁；default production harness 仍 404 | **CURRENT LOCAL GATE GREEN / ECS product E2E GREEN**；本轮 31-action motion gate GREEN，截图在 ignored visual out；ECS 公网仍 21 个适用场景通过 |
