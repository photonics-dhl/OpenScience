...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% DOM 几何、WebGL2-only OGL runtime、generation-owned CSS-pixel layout/atlas/RAF transaction、OGL Text/BMFont derivative-MSDF mask/color/straight-alpha composite、first-complete-frame 与 exact resource lifecycle | 高保真 Task 4 review round 1；当前 marker `task-4-msdf-glyph-v1` 仅含真实字形，WebGL1/mobile/reduced/init/atlas/shader/FBO failure 诚实 DOM；Task 5 材质须显式切换 `task-5-resting-material-v1`，生产 `/` 继续隔离 |
...
| `apps/web/assets/optical-lab/fonts/` / `apps/web/public/optical-lab/atlas/` / `apps/web/scripts/generate-optical-atlas.mjs` | 高保真 Lab 的 Archivo 900 与 Bodoni Moda Italic 700（opsz 96）确定性实例、原样 OFL、受限 charset、MSDF JSON/PNG 与受 pin generator | Task 2；OGL/MSDF 仅为隔离 Lab 准备，生产 `/` 不导入 |
| `apps/web/assets/optical-prototype/{fonts/evolves-editorial-400.ttf,geometry-manifest.json}` / `apps/web/public/optical-prototype/title-{geometry.json,outline.svg}` / `apps/web/scripts/generate-optical-title-geometry.mjs` | Three.js 中央粒子隔离原型的 Node-only opentype 字体权威合同：Archivo 900 + 独立 Bodoni opsz96/wght400/skewX(-6deg)、1672×935、57.3% 词缝、baseline 542、批准可见边界 36.8→958.056→1600、word/glyph affine metrics、4,619 个 5px 确定性规则点、有限 SVG/JSON 同源轮廓、精确 provenance/SHA/OFL | Native prototype Task 2 post-Task4 geometry repair；真实 SVG parser/双次生成门禁，浏览器不解析字体，生产 Hero/路由未接入 |
| `apps/web/app/%5Fvisual/central-particle/page.tsx` / `apps/web/components/optical-prototype/*` / `apps/web/lib/optical-prototype/{runtime-policy,lifecycle,renderer,particle-grid,touch-texture,shaders/*}.ts` | no-index Three.js WebGL2 隔离原型：唯一 SSR h1 + 同源 SVG、route-only dynamic import、4,619 实例规则标题点阵、clean-room TouchTexture、双轴 contain/pointer 同构、abort/timeout/init/context/resize/visibility/cleanup ownership | Native prototype Task 4 + seam-anchor repair GREEN；Task 5 已批准改为连续 GPU 字形主体、seam-local 点阵与独立 curtain，全字点阵降为 debug-only；尚未加入 field/curtain/postprocessing，生产 Hero/ECS 未修改 |
| `apps/web/test/optical-lab-{model,contract,atlas}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：58% DOM layout、首个完整 mask/color frame、真实 wrong-glyph shape/density、straight-alpha AA chroma、线性 atlas upload、init/post-frame resize generation、context restore、真实 shader/FBO failure 与 exact partial cleanup | 高保真 Task 4 review round 1 focused 18/18、Web 全量 199/199、production 11-case glyph matrix GREEN；`runTask4MsdfGlyphAssertions` 绑定 `task-4-msdf-glyph-v1`，旧完整材质 probes 仅留待 `task-5-resting-material-v1`，未知 phase fail closed；输出位于已忽略 `test/visual/out/optical-lab/` |
...
| `apps/web/test/{landing-page,optical-field,landing-motion-policy,central-particle-dependencies,central-particle-geometry,central-particle-runtime,central-particle-field}.test.ts{,x}` | Landing 基线/路由/单一标题/朱红记忆点、glyph safe zone、指针时间插值、对称狭缝衍射波前、Open RO 六节点与 reduced-motion 回归门禁；中央粒子原型的依赖/import、字体 geometry、规则 grid/TouchTexture、双轴 viewport、asset/init/WebGL2 policy/resize/context/cleanup 合同 | 活文档；Native prototype Task 4 focused 38/38，production real-instancing + 800×1000/1672×800 同构 browser gate GREEN |
...
| `apps/web/test/visual/shots.mjs` | Landing 视觉截图脚本：可配置 `VISUAL_BASE_URL`，输出目录固定相对脚本解析（不依赖 cwd），等待字体就绪；1440/1920/390、Open RO、reduced-motion，并捕获 fixed-axis pointer-left/slit/right 60/150/300ms active frames | 第三轮本地 production smoke 通过（2026-08-11），待线上同矩阵 |
...
| `docs/decisions/ADR-009-optical-runtime-and-fonts.md` | 生产 Optical Field 保持 Canvas 2D；2026-08-11 增补仅限 no-index Lab 的 `ogl@1.0.11` WebGL2 exception、MSDF/OFL/font provenance、浏览器 GPU/ECS 隔离与 route budget；2026-08-12 增补 no-index native Three experiment 的精确 pins、MIT/Zlib/Codrops clean-room attribution、production-import prohibition，以及 GSAP/ControlKit/glslify/额外 React wrapper/hosted runtime 的明确禁止 | Accepted（Task 8 Step 6 前不得替换生产） |
...
| `docs/specs/2026-08-12-optical-native-particle-prototype-design.md` | 与生产 Hero/OGL Lab 隔离的 Three.js 技术样片：opentype 单一字体几何、连续 GPU 字形主体、seam-local 规则粒子、TouchTexture、57.3%/50% 弧形收束、独立纵向幕、selective Bloom/局部色散与固定 1672×935 证据 | 用户已批准路线与 Task 5 连续字形纠偏；生产 Hero 未授权接入 |
...
| `docs/superpowers/plans/2026-08-12-optical-native-particle-prototype-plan.md` | 隔离 Three.js 原型：依赖/许可、opentype 单一几何、连续 title + seam-local 规则粒子/TouchTexture、中心弧形场/纵向幕、有限 postprocessing、固定证据与 ECS 发布 | Task 1–4 与 seam-anchor repair 已完成；Task 5 连续字形纠偏已获用户确认，待书面复核后执行 |
