...
| `apps/web/components/brand/{OpticalHeadline,OpticalField}.tsx` / `apps/web/lib/optical-field/*.ts` | Optical Editorial Landing 品牌媒介：保留原标题布局；单一可访问 DOM 标题 + glyph safe zone；窄垂直点阵狭缝、指针局部径向位移、无同心圆的右向衍射波前、收窄 SVG 局部折射与 reduced-motion 降级 | 2026-08-11 第二轮形变纠偏；`b45e002` 已部署，待人工审美复核 |
