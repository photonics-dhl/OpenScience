...
| `infra/scripts/disk-cache-maintenance.sh` / `infra/systemd/openscience-disk-cache-maintenance.{service,timer}` | 每日回收**可再生产**缓存：docker build cache、dangling 镜像、journal 超限部分；并只读报告历史 release 数量与体积。**刻意不回收历史 release**——`production-release-retention.mjs` 按要求绑定发布事务、不作独立清理入口，历史清理仍为"用户授权 + 留收据"（流程见 `docs/runbooks/deployment.md`） | 已部署云上（`/usr/local/sbin/openscience-disk-cache-maintenance`，timer enabled+active，2026-09-17） |
