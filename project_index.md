...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,model,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% DOM 几何、WebGL2-only OGL runtime、generation-owned glyph/GPGPU/HDR composite、96×54 ping-pong flowmap、4px bounded refraction、三档稳定质量策略与 exact resource lifecycle | Task 7 GREEN：full/reduced-particles/reduced-bloom 保持布局拓扑，650ms exact rest；生产 `/` 隔离不变且未部署 |
| `apps/web/public/optical-lab/{target-reference,current-production,accepted-resting}.png` / `apps/web/test/visual/promote-optical-lab-resting.mjs` | 用户授权参考/current 与 Task 5 批准的 1672×941 decorative fallback；promotion 仅接受批准输出并记录 SHA-256 | Task 7 fallback matrix GREEN；验收图 589,510 B，不进入生产首页 |
...
| `apps/web/test/optical-lab-{model,contract,atlas,runtime-budget}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：58% DOM layout、BMFont/CSS ink parity、静止五区材质、96×54 flowmap、响应包络、fallback load/error、三档 FPS 自适应、资产/route budget、context restore 与真实 cleanup | Task 7 full Web `221/221`、typecheck/build/root lint 与 production-start matrix GREEN；输出位于已忽略 `test/visual/out/optical-lab/` |
...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 原型高保真重构：OGL WebGL2 多通道、全尺寸排版合同、MSDF/flowmap/GPGPU/HDR 五层静态光场、4px 轻交互与静态降级 | 用户已批准 Task 5 静止材质；Task 1–7 完成，Task 8 最终验收待执行；未替换生产 `/`、未部署 |
