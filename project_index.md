...
| `apps/agent-worker/src/ingestion-parser.ts` / `apps/agent-worker/src/index.ts` | Artifact→Blob→Hermes 桥接：Markdown/TeX 确定性解码；PDF/Office/图片显式进入 `needs_review`，待部署受控二进制解析器 | 执行中 |
