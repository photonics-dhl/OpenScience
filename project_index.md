...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% DOM 几何、WebGL2-only OGL runtime、generation-owned glyph-mask/GPGPU/HDR composite transaction、atlas-size/padding → browser-measured CSS glyph-ink 坐标契约、straight-alpha final frame 与 exact resource lifecycle | Task 5 fresh parity repair 后 isolated bounds/shape/density `.941251/.907246/1.047571`、native 五区/forbidden 与完整 lifecycle matrix 全 GREEN；等待用户原尺寸视觉批准，未提交，生产 `/` 隔离不变 |
...
| `apps/web/test/optical-lab-{model,contract,atlas}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：58% DOM layout、目标相对 suffix bounds/density/slant、BMFont pen/padding landmarks、isolated DOM/GPU shape parity、首个完整 mask/color/material frame、straight-alpha AA chroma、五区域 native metrics、staircase 在内的 forbidden topology、context restore 与真实 shader/FBO cleanup | Task 5 focused `51/51`、full Web `212/212`、typecheck/build/root lint GREEN；isolated evolves shape `.907246` 与 native material/完整 production matrix GREEN，输出位于已忽略 `test/visual/out/optical-lab/`；等待视觉批准 |
...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 原型高保真重构：OGL WebGL2 多通道、全尺寸排版合同、MSDF/flowmap/GPGPU/HDR 五层静态光场、4px 轻交互与静态降级 | 用户已书面确认；Task 1–4 完成，Task 5 native material、isolated parity 与完整工程矩阵技术 GREEN，等待原尺寸视觉批准且尚未提交/部署；Task 6+ 未开始 |
...
| `docs/superpowers/plans/2026-08-11-optical-lab-high-fidelity-reconstruction-plan.md` / `.superpowers/sdd/2026-08-11-optical-lab-high-fidelity-reconstruction-plan/task-{1,2,3,4,5}-{brief,report}.md` | 用户已确认的高保真 OGL 重构计划及逐 Task brief/report 证据：全尺寸字体样张 → 固定版本 MSDF 资产 → WebGL2 多 pass 静止光场 → 4px 内 flowmap → 静态降级/预算/真机验收 | Task 1–4 完成；Task 5 report 已 append fresh atlas/CSS ink mapping RED→GREEN、material/parity/完整 matrix 与全门禁证据；等待用户视觉批准，无 commit，Task 6+ 未开始，未部署 |
