...
| `apps/web/components/hermes/{HermesVisualAdapter,HermesRiggedPortrait,HermesWorkspaceStage,HermesPerformanceBubble,HermesGuideBubble,HermesDockAnchor,HermesAnchor,HermesDraftDiff,hermes-state}.ts(x)` / `apps/web/lib/hermes/{action-catalog,behavior-director,performance-beat,stage-sizing,anchor-registry,travel-path,dock-preferences,motion-preference,hermes-runtime-status}.ts` | Workspace-wide Hermes foundation：本地候选 32-action movable work assistant；桌面/mobile/expanded 为 `176/120/336`，全角色 drag、viewport clamp、resize recovery、位置持久化、象限气泡、安全航点、字段引导、reviewable diff 与有界 context-loss recovery；ECS 仍为 27-action 基线 | **CURRENT LOCAL CANDIDATE** `be45f91`（docs commit 在其后，未部署）；release `c97926a` / rollback `06072c1` |
| `apps/web/public/hermes/live2d/{NOTICE.md,live2dcubismcore.min.js,wanko/**}` / `apps/web/test/hermes-live2d-asset-contract.test.ts` / external `C:/Users/Mac/AppData/Local/OpenScience/Live2D/wanko-genie-v1/runtime-v09-export-20260823/` | v09 SDK 4.0 runtime：两张 texture zero-alpha RGB clean、12 motion 参数兼容、manifest 闭合；active renderer 只有单一 Cubism model/canvas，旧 pet/carrier 公开 URL 为 404 | **CURRENT ECS development deployment** `c97926a`；母版 SHA `BA111D4E...1121ADD` 未改，operator exception 与版权 notice 已部署，rollback `06072c1` |
| `apps/web/lib/hermes/{live2d-core-loader,wanko-action-director,wanko-renderer-controller,wanko-live2d-renderer,wanko-model-presentation,wanko-runtime-ownership}.ts` / `apps/web/test/{hermes-wanko-action-director,hermes-wanko-renderer-contract,hermes-wanko-runtime-ownership}.test.ts` | 浏览器单 Wanko canvas/model/RAF owner；本地 32-action profile 复用 v09 12 motions，旧 OGL pet renderer、三张 pet PNG、carrier decoder/asset/exporter/CSS 已按授权永久移出当前 tree | **CURRENT v09 local runtime candidate**；公开 Hermes 资产只保留闭合 Live2D bundle 与 notice |
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；覆盖 Wanko 32-action、`176/120/336`、气泡/图层/像素/比例/exact-static/90 秒陪伴，以及真实创建提交、拖动夹紧、刷新恢复、resize recovery 与主操作不遮挡；default production harness 仍 404 | **CURRENT LOCAL GATES GREEN / ECS product E2E GREEN** `be45f91`；截图在 ignored visual out；ECS 公网仍 21 个适用场景通过 |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko 第一阶段验证角色；§10 原生灯/烟雾托座；§11 统一 semantic performance beat、`176/120/336px` movable work-assistant 层级与双语 optical-editorial 气泡系统 | **CURRENT Hermes visual/guide design**；Task 20 本地候选 `be45f91`，Phase 2 Cubism 新动作待真实画面判断 |
...
| `docs/plans/2026-08-19-hermes-wanko-live2d-plan.md` | Tasks 1–18 历史模型/灯/烟雾/学士帽迭代与 v09 部署；Task 19 动作/语言 foundation；Task 20 把过重预览校正为 `176/120/336px` movable work assistant，并以真实 RO-create Playwright 门禁验收 | **CURRENT LOCAL TASK 20** `be45f91`；Phase 2 Cubism 新动作与 ECS 部署均未授权 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：v09 repo/runtime、旧资产永久清理、本地/ECS gates、branch/HEAD/release/rollback 与 development exception | **CURRENT / compact active-memory**；production `c97926a`，rollback `06072c1` |
