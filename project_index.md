...
| `scripts/design/minimax-client.mjs` / `minimax-client.test.mjs` | MiniMax `image-01` 设计资产客户端与无网络 Node 测试；key1→key2 仅在官方余额不足码 1008 回退，运行时 `MINIMAX_API_KEY` 仅作 key1 兼容 | 活文档 |
| `scripts/design/generate-minimax-image.mjs` | 显式 `node --env-file` 调用的设计资产生成 CLI；只接受 `docs/design-assets/generated` 内的新输出并写 provenance sidecar，不读取或记录 `.env` | 活文档 |
