...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% DOM 几何、WebGL2-only OGL shell、frozen context acquisition contract、字体 ready 后 CSS-pixel layout authority、per-context exact resource/renderer ownership、fresh-canvas restore 与诚实 DOM/static fallback | 高保真 Task 3；`task-3-runtime-shell-v1` 不宣称完整 frame，Task 4+ 才显式推进 phase 并实现 MSDF/particle/composite；生产 `/` 继续隔离 |
...
| `apps/web/test/optical-lab-{model,contract,atlas}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：三值 WebGL2 policy、首个 context 精确 attributes、58% DOM layout/parity、异步 unavailable ownership cleanup、WebGL1/no-context/reduced/mobile static、init failure、fresh WebGL2 restore、route-unmount RAF/resource cleanup 与 MSDF hash/charset integrity | 高保真 Task 3 focused 26/26 + production lifecycle matrix GREEN；旧 Candidate B visual probes 完整保留在显式 Task 4 phase branch，输出位于已忽略 `test/visual/out/optical-lab/` |
...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 原型高保真重构：OGL WebGL2 多通道、全尺寸排版合同、MSDF/flowmap/GPGPU/HDR 五层静态光场、4px 轻交互与静态降级 | 用户已书面确认；Task 1–3 完成，未部署，Task 4+ 视觉通道未实现 |
...
| `docs/superpowers/plans/2026-08-11-optical-lab-high-fidelity-reconstruction-plan.md` | 用户已确认的高保真 OGL 重构计划：全尺寸字体样张 → 固定版本 MSDF 资产 → WebGL2 多 pass 静止光场 → 4px 内 flowmap → 静态降级/预算/真机验收 | Subagent-Driven Task 1–3 完成；Task 4+ 待执行，未部署 |
