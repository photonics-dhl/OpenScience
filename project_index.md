...
| `apps/web/lib/optical-lab/asset-manifest.mjs` / `apps/web/test/optical-cache-contract.test.ts` | 两张大型 Landing 光学 PNG 的 canonical path、完整 SHA-256、内容寻址 URL 真源；合同计算真实文件 digest，并锁定 Next 精确 rewrite、immutable header、SSR/WebGL 共享 URL 与动态内容非缓存边界 | 本地 GREEN，待部署后验证 Cloudflare `HIT + Age` |
...
| `docs/specs/2026-08-16-edge-cache-asset-versioning-design.md` | Landing 大型光学资产内容寻址与 Cloudflare 长缓存设计；动态 HTML/API 不进入 immutable 规则 | 本地实现完成，待部署 |
| `docs/plans/2026-08-16-edge-cache-asset-versioning-plan.md` | 两张大型光学 PNG 的共享 manifest、精确 rewrite、缓存合同与生产验证计划 | 本地门禁中，待部署 |
...
| `docs/handoff/2026-08-16-edge-cache-asset-versioning-handoff.md` | Landing 两张大型 PNG 内容寻址缓存的本地证据、动态内容边界与部署下一步 | 当前缓存优化交接 |
