...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、deploy.sh/DNS-01 实证）；§5.3–5.6 登记 Task 19 overlay、Task 20/21 idle 与 Task 22 unified water/clean target release 流程 | 当前线上 `744c631`；rollback `8edf6fa`，Task 22 公网五轮 GREEN |
...
| `docs/superpowers/specs/2026-08-14-optical-water-flow-refinement-design.md` | Task 22 成熟商业水流规律落地：统一 idle/pointer 材质、曲率微光、标题-only target 与静态/实时污染清理 | 已实现、复审并随 `744c631` 部署 |
| `docs/superpowers/plans/2026-08-14-optical-water-flow-refinement-plan.md` | Task 22 单任务 TDD 计划：污染 RED、连续 phase 水流、完整 native/release 与授权部署 | Step 1–5 完成；release `744c631` |
