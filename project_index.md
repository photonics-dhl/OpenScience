...
> **CURRENT source/deployment anchor（2026-08-25 22:55 +08）：** working branch `codex/hermes-wanko-live2d`；deployed application / immutable release 为 `5323ba8` / `bf54eaa`；local main / origin main `c60ffdd` / `7eb2f5b`；ECS release / rollback 为 `bf54eaa` / `6b804f7`。只有本锚点与主题唯一 CURRENT handoff 可决定当前状态。
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与 Hermes 入口/菜单 viewport、上游 reflow、键盘 focus、action-first motion、phrase variation、input/approval interruption、研究导航/反馈几何的 66 案 production browser 门禁 | **CURRENT local `66/66` GREEN**；critical repeat `10/10`，public Hermes `9/9` |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：§13.8 detached 右侧停靠与 protected fallback、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；production application/release/rollback `9aef5c4` / `8395b4d` / `bf54eaa` |
| `docs/plans/2026-08-26-landing-motion-navigation-continuity-plan.md` | 冻结 Hermes 形象、将 Landing 最终合成可见水波纳入 canonical release gate，并补齐真实产品页一级/RO 二级入口 | **LOCAL CANDIDATE / deployment pending**；base `460bd878`，ECS 仍为 `8395b4d` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows Git Bash 专用密钥调用与版本化资产）；§5.24 登记 Hermes detached composition correction | CURRENT；application/release/rollback `9aef5c4` / `8395b4d` / `bf54eaa`，server/public gates GREEN |
