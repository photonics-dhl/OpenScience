...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；覆盖 Wanko 32-action、`360/200`、气泡/图层/像素/比例/exact-static、真实创建提交、drag/reload/resize、guide geometry、atomic speech 与主操作不遮挡；default production harness 仍 404 | **CURRENT LOCAL + PUBLIC UI GATES GREEN** `5f4e73c`：Web `390+5`、canonical product E2E `40/40`、公网三视口无写入 UI gate；真实账号纵向 gate 未重跑 |
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与三条 Hermes 右键/键盘/长按/反馈几何的 60 案 production browser 发布门禁；覆盖 landmark/focus/overflow/error/LCP/transfer/DOM 与 actor/menu/反馈不遮挡 | **CURRENT Research Folio release gate `60/60` GREEN**；部署候选待发布 |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko 第一阶段角色与 runtime；§11 movable `360/200px`；§12 daylight warm-paper Research Session Folio、情境菜单、locale-aware 阅读字体与匿名公网视觉验收边界 | **CURRENT Hermes visual/guide design**；§12 review release `bba5f14` deployed |
| `docs/specs/2026-08-24-research-folio-product-system-design.md` | 登录到公开验证的全部真实非 Landing 产品视觉与信息架构；暖纸阅读、graphite 证据工具、18 表面、预留 Hermes 研究页边与无覆盖合同 | **CURRENT non-Landing product UI spec / APPROVED CANDIDATE**；Landing unchanged |
...
| `docs/plans/2026-08-24-research-folio-product-system-plan.md` | Research Folio 的合同、共享 shell、Hermes 锚定几何、identity/Dashboard/create/RO/public/admin 迁移、60-case production browser 门禁与 immutable ECS 发布链 | **CURRENT execution candidate**；implementation/review complete, deploy pending |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：已部署 v09 movable assistant、Research Session Folio review release、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；production `bba5f14`，rollback `68d8be7` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows Git Bash 专用密钥调用与版本化资产）；§5.3–5.16 登记 Optical 与 Hermes 生产发布 | CURRENT；最新 ECS application release/rollback `bba5f14` / `68d8be7`，公网 no-write E2E `7/7` |
