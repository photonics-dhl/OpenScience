...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 共 12 项，Task 1 Foundation 实现完成待全仓门禁，后续 11 项未开始 | 工具自管；CURRENT tag 已切换 |
...
| `docs/plans/2026-08-26-hermes-research-intelligence-foundation-plan.md` | Taskmaster Task 1 的可执行 TDD 计划：能力台账门禁、自有 corpus、现状 parser 基准、零污染输出与全仓验收；后续三计划按实测结果展开 | **CURRENT execution plan / TASKS 1–4 COMPLETE / FINAL GATE PENDING** |
...
| `docs/runbooks/hermes-capability-registry.md` | Hermes Skill/MCP/model/parser/provider 能力台账：注入状态、授权/成本、项目级安装目录、72h PDF TTL、10min signed URL、评测/回滚/退役规则 | **CURRENT capability registry / MACHINE-GATED**；21 行、6-case `BASELINE_ONLY` 已记录；凭据值不入文档，尚未安装或部署新能力 |
