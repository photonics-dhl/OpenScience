...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；覆盖 Wanko 32-action、`336/176`、气泡/图层/像素/比例/exact-static/90 秒陪伴，以及真实创建提交、拖动夹紧、刷新恢复、resize recovery 与主操作不遮挡；default production harness 仍 404 | **CURRENT LOCAL GATES GREEN / ECS product E2E GREEN** `c123a5a`；截图在 ignored visual out；ECS 公网仍 21 个适用场景通过 |
