...
| `apps/web/app/%5Fvisual/central-particle/page.tsx` / `apps/web/components/optical-prototype/*` / `apps/web/lib/optical-prototype/{runtime-policy,lifecycle,renderer}.ts` | no-index Three.js WebGL2 隔离运行壳：唯一 SSR h1 + 同源 SVG、route-only dynamic import、desktop/reduced/mobile/init fallback、DPR cap、正交相机、generation resize、visibility/context/cleanup ownership | Native prototype Task 3；当前为空场景且未加入 particle/TouchTexture/field/curtain/postprocessing，生产 Hero 与 ECS 未修改 |
...
| `apps/web/test/{landing-page,optical-field,landing-motion-policy,central-particle-dependencies,central-particle-geometry,central-particle-runtime}.test.ts{,x}` | Landing 基线/路由/单一标题/朱红记忆点、glyph safe zone、指针时间插值、对称狭缝衍射波前、Open RO 六节点与 reduced-motion 回归门禁；中央粒子原型的依赖/import 隔离、字体 geometry，以及 WebGL2 policy/resize/context/cleanup 合同 | 活文档；Native prototype Task 3 focused 27/27，复审动态 policy/hidden restore/dispose-loss/stale-owner RED 已覆盖 |
