...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` | 明示 flag 才开放的 Wanko 27-action/layer-isolation harness 与真实像素/比例/exact-static/90 秒陪伴/renderer cadence/资源/引导 release 门禁；default production 仍 404 | **CURRENT LOCAL GATE GREEN**；完整 `test:hermes-release` exit 0，ECS 未运行 |
