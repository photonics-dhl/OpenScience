...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；CURRENT 聚合覆盖 Wanko runtime/performance/静态锚定 geometry、19 个仍适用的 runtime/guide E2E 与 5 个菜单/输入抑制/真实研究导航产品场景；新增 12 项 action/motion/zh/en 锁与 desktop/mobile actor-menu disjoint、mouth-tail、mobile no-jump/no-dead-gap 合同；default production harness 仍 404 | **CURRENT DEPLOYED gates GREEN** application `1b3bada`：Hermes `19+5`、Web `408+5`、public `5/5`；ECS `8ed2f3c` |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko renderer、`360/200px`、Research Folio；§13.3 把 12 项动作改为与 Hermes 分带的 carried tool sheet，并锁定动作/Live2D/双语短句 | **CURRENT Hermes visual/guide design / §13.3 DEPLOYED**；application `1b3bada`，release `8ed2f3c` |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：mouth-anchored speech、Research Folio release、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；application `e4a19d4`；production `7165e9b`，rollback `3010903` |
