...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% DOM 几何、WebGL2-only OGL shell、字体 ready 后 CSS-pixel layout authority、per-context exact resource ledger、fresh-canvas restore 与诚实 DOM/static fallback | 高保真 Task 3；shell 不宣称完整 frame，Task 4+ 才实现 MSDF/particle/composite；生产 `/` 继续隔离 |
...
| `apps/web/test/optical-lab-{model,contract,atlas}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：三值 WebGL2 policy、58% DOM layout/parity、OGL ownership cleanup、WebGL1/no-context/reduced/mobile static、init failure、fresh WebGL2 restore、route-unmount RAF/resource cleanup与 MSDF hash/charset integrity | 高保真 Task 3 focused 24/24 + production lifecycle matrix GREEN；旧 Candidate B visual probes 保留为 Task 4+ 重建参考，输出位于已忽略 `test/visual/out/optical-lab/` |
