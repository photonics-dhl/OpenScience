...
| `packages/domain/src/{workspace/helpers,research-object/research-objects,agent/agent,retrieval/literature-acquisition}.ts` / `apps/api/{src/routes/literature.ts,test/literature-acquisition.integration.test.ts}` | Task 5 统一文献 acquisition：active membership、system RO、Session、Task/credit/audit 共用 transaction primitives；单笔三次有界 Serializable 事务，exact replay 先于余额，Redis commit 后投递与 pending recovery | **LOCAL CANDIDATE `ff5568f`**；Domain/API/Agent 本地门禁 green，真实 PostgreSQL suite 已加入但按本地 no-Docker 规则未执行；ECS pending |
...
| `infra/migrations/` | Prisma 迁移 1–31（31 = 账号 Evidence 阅读偏好；30 = Research Identity signals / AgentTask InterestContext；29 = `agent_tasks.execution_attempt` worker fencing），各附 rollback.sql | **REPOSITORY / PRODUCTION 31/31**；search 2/2，历史迁移账本保留 |
| `infra/schema.prisma` | core Prisma schema：既有平台模型 + ResearchIdentityProfile、ReadingPreference、ClaimNode、EvidenceRecord、PresentationAsset 与 scoped relations | **CURRENT PRODUCTION**；migration 31 deployed |
