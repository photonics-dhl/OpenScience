...
| `apps/web/test/optical-lab-{model,contract,atlas}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：58% DOM layout、BMFont/CSS ink parity、静止五区材质、straight alpha、96×54 flowmap、1–2px/4px/650ms 响应包络、left/slit/right/recovered 原尺寸帧、fallback/context restore 与真实 shader/FBO cleanup | Task 6 full Web `214/214`、typecheck/build/root lint 与 production-start matrix GREEN；输出位于已忽略 `test/visual/out/optical-lab/` |
...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 原型高保真重构：OGL WebGL2 多通道、全尺寸排版合同、MSDF/flowmap/GPGPU/HDR 五层静态光场、4px 轻交互与静态降级 | 用户已批准 Task 5 静止材质；Task 1–6 完成，Task 7 静态 fallback/预算待执行；未替换生产 `/`、未部署 |
