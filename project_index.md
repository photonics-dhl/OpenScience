...
| `apps/web/components/dashboard/LiteratureAcquisition.tsx` / `apps/web/lib/{api,literature-acquisition-state}.ts` / `apps/web/app/dashboard/page.tsx` / `apps/web/test/literature-acquisition-state.test.ts` / `apps/web/test/e2e/auth-dashboard.spec.ts` / `packages/domain/src/retrieval/browser-result.ts` / `packages/domain/src/agent/agent.ts` / `apps/agent-worker/src/retrieval/handler.ts` | ScanSci plan Task 7 Personal Space acquisition：browser-safe identifier/result DTO、持久 source identifier/原始 expiry、user+target unresolved key、同 task retry/credit、active-first reload recovery、1.2–15s polling 与临时下载 | **LOCAL TASK 7 REVIEW CANDIDATE**；3 条真实 component journey + Domain/API/Worker/Web gates green；ECS 375px/one-use download acceptance pending |
