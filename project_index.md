...
| `apps/web/components/hermes/{HermesVisualAdapter,HermesRiggedPortrait,HermesWorkspaceStage,HermesDockAnchor,HermesAnchor,HermesGuideBubble,HermesDraftDiff,hermes-state}.ts(x)` / `apps/web/lib/hermes/{action-catalog,behavior-director,anchor-registry,travel-path,dock-preferences,motion-preference,hermes-runtime-status}.ts` | Workspace-wide Hermes 功能 foundation：27-action 意图、安全航点、语义字段引导、reviewable diff、draw heartbeat 与有界 context-loss recovery；2026-08-23 视觉 owner 已接 Wanko v09 local candidate | **CURRENT function foundation**；production `06072c1` / rollback `8ecf96c`，technical candidate `codex/hermes-wanko-live2d@8929ccf` |
...
| `apps/web/public/hermes/live2d/{NOTICE.md,live2dcubismcore.min.js,wanko/**}` / `apps/web/test/hermes-live2d-asset-contract.test.ts` / external `C:/Users/Mac/AppData/Local/OpenScience/Live2D/wanko-genie-v1/runtime-v09-export-20260823/` | v09 SDK 4.0 runtime 已进入仓库：两张 texture zero-alpha RGB clean、12 motion 参数兼容、manifest 闭合；active renderer 已删除造成碎片的 Pixi procedural navigator，只保留单一 Cubism model/canvas | **CURRENT local release candidate / NOT DEPLOYED**；technical candidate `8929ccf`，母版 SHA `BA111D4E...1121ADD` 未改，fresh clean-checkout aggregate/Web gates GREEN，production 仍 `06072c1`，仅待 operator 发布许可记录 |
| `apps/web/assets/hermes/carrier/**` / `apps/web/public/hermes/carrier/**` / `apps/web/lib/hermes/wanko-carrier-assets.ts` / `apps/web/lib/hermes/wanko-carrier-layout.ts` / `apps/web/scripts/export-hermes-carrier-assets.mjs` / `apps/web/scripts/export-hermes-carrier-posters.mjs` | 被用户否决的 raster carrier/poster 实验与历史合同；active zero-image runtime 不引用这些资产 | **HISTORICAL / REJECTED**；不得恢复为 CURRENT 或 fallback，未获删除授权 |
| `apps/web/lib/hermes/{live2d-core-loader,wanko-action-director,wanko-renderer-controller,wanko-live2d-renderer,wanko-model-presentation,wanko-runtime-ownership}.ts` / `apps/web/test/{hermes-wanko-action-director,hermes-wanko-renderer-contract,hermes-wanko-runtime-ownership}.test.ts` | 浏览器单 Wanko canvas/model/RAF owner 与 27-action/lifecycle foundation；当前展示仍隐藏原 bowl/effect，程序烟尾视觉已被用户否决 | **CURRENT runtime foundation only**；等待 Cubism source-variant round-trip，不是视觉候选 |
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` | 明示 flag 才开放的 Wanko 27-action/layer-isolation harness 与真实像素/比例/exact-static/90 秒陪伴/renderer cadence/资源/引导 release 门禁；aggregate 同时运行 Dashboard 与 Field Guide production E2E，default production harness 仍 404 | **CURRENT LOCAL GATE GREEN**；22 个 product E2E 通过，ECS 未运行 |
...
| `apps/web/test/workspace-readability.test.ts` / `apps/web/test/e2e/workspace-readability.spec.ts` / `apps/web/test/visual/workspace-readability-gate.mjs` | 三视觉系统阅读与控件合同：14–17 px 语义角色、浏览器无关控件、真实颜色对比、水平/垂直裁切、可访问名与 focus 可见性；覆盖桌面/移动代表路由 | **CURRENT production** `06072c1`；production-browser 18 route/view GREEN |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：v09 repo runtime、C 盘永久清理、本地 release gates、branch/HEAD/release/rollback 与许可硬门 | **CURRENT / compact active-memory**；production remains `06072c1` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离与版本化资产）；§5.3–5.13 登记 Optical 与 Hermes 生产发布 | CURRENT；最新 ECS application release/rollback `06072c1` / `8ecf96c` |
