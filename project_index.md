...
| `apps/web/assets/optical-prototype/geometry-manifest.json` / `apps/web/public/optical-prototype/title-{geometry.json,outline.svg}` / `apps/web/scripts/generate-optical-title-geometry.mjs` | Three.js 中央粒子隔离原型的 Node-only opentype 字体权威合同：1672×935、57.3%/50% 中心、单基线 word/glyph metrics、5px 确定性规则点阵、有限 SVG/JSON 同源轮廓、精确 Node provenance、输入/输出 SHA-256 与 OFL 路径 | Native prototype Task 2；真实 SVG path parser 边界门禁，浏览器不解析字体，生产 Hero/路由未接入 |
