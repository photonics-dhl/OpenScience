...
> **CURRENT source/deployment anchor（2026-08-29）：** branch `codex/hermes-wanko-live2d`；application source/production release `6cabe422a8459dfa358786c9f5aae84558949f6b`；rollback `28a3d5ca681b7744fae521dfa9154100a24e8845`；无迁移。Exact CI `33240457443` / job `99068791412`、schema 3 `hermes-parser-14-2-v1` 14/2/0/0、gateway 14/0/0、26 locators/3 table-cell、runtime/core 29/search 2/BGE/backups 7/markers 全绿；source review READY 0/0/0。Taskmaster 保持 6/12，Task 7 next、Task 10 dependency-ready。后续 docs-only closeout HEAD 不改变 production 身份。
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 6/12，Tasks 1–6 done | 工具自管；Tasks 7 和 10 dependency-ready，产品顺序先 Task 7 |
...
| `docs/plans/2026-08-29-production-release-retention-plan.md` | active+rollback 机器身份、durable pending intent 与精确 release retention 实施计划 | **IMPLEMENTED CANDIDATE / EXACT UBUNTU CI GREEN**；rollback marker 已 ECS bootstrap；禁止 broad prune，自动范围仅旧 release/capability/精确 tag |
| `docs/plans/2026-08-29-hermes-research-identity-routing-plan.md` | Taskmaster Task 7：注册身份、可纠正兴趣、服务端 InterestContext、Hermes 静默路由与生产真实旅程 | **CURRENT / CANDIDATE READY FOR EXACT CI**；复用既有 profile 模型，不引入模式切换 |
| `packages/domain/src/research-intelligence/{identity-profile-service,interest-context}.ts` / `apps/api/src/routes/research-identity.ts` / `apps/web/components/auth/ResearchProfileFields.tsx` | 注册身份、版本化可纠正兴趣、服务端确定性 InterestContext 与设置页；拒绝客户端伪造上下文、敏感字段和站外历史，Hermes 按上下文静默适配 | **TASK 7 CANDIDATE / LOCAL GATES + SECURITY REVIEW GREEN**；待 exact CI、ECS migration 30、真实产品旅程后才可 done |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、CPU parser 14/2 验收、磁盘分类、固定约束与 version tuple | **CURRENT active-memory**；branch/application/production `codex/hermes-wanko-live2d` / `6cabe422`，rollback `28a3d5c`；docs HEAD separate |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.38 acceptance/FD9 transaction，§5.39 parser recovery，§5.40 inventory，§5.41 cleanup/retention | **CURRENT**；application/release `6cabe422`，rollback `28a3d5c`，30.38GB cleanup 与 rollback marker bootstrap 后 production GREEN |
| `docs/runbooks/hermes-capability-registry.md` | Hermes Skill/MCP/model/parser/provider 能力台账：注入状态、授权/成本、项目级安装目录、72h PDF TTL、10min signed URL、评测/回滚/退役规则 | **CURRENT / MACHINE-GATED**；CPU parser/Tesseract、BGE-M3 与 PostgreSQL lexical search 为 `PRODUCTION`，其他 parser 候选与凭据边界继续记录 |
...
| `infra/migrations/` | Prisma 迁移 1–30（30 = Research Identity signals / AgentTask InterestContext；29 = `agent_tasks.execution_attempt` worker fencing；28 = Research Intelligence core metadata），各附 rollback.sql | **REPOSITORY 30 / CURRENT PRODUCTION 29/29**；active ledger 30 含需保留的历史 `20260809010000_ro_create_idempotency` |
