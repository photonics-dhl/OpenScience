...
| `docs/plans/2026-08-18-readable-workspace-hermes-guidance-plan.md` | 可读性基础→代表页面→完整 Hermes footprint 避让→edit-before-accept/缺失证据→语义动作→公网 blank RO gate→ECS checkpoint 的 TDD 实施链 | **CURRENT execution plan**；Tasks 1–5 完成于 `757de5b`，Task 6 next |
...
| `apps/web/test/visual/hermes-articulation-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-guidance-geometry-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` | 确定呼吸/字段语义关节帧、90 秒真实动作、内部 pointer articulation、逐 RAF 航行、Diff 安全与聚合门禁；内部动作不得拖动 enclosing silhouette，整图 affine mutation 必须失败 | Task 5 `757de5b` articulation + companion GREEN；公网 blank-RO release gate 仍待 Task 6 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Readable Workspace / Hermes 唯一 CURRENT 交接：当前入口、branch/HEAD/release/rollback、约束与单一 next action | **CURRENT / compact active-memory**；Tasks 1–5 完成，Task 6 next |
