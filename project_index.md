...
| `docs/specs/2026-08-26-hermes-research-intelligence-platform-design.md` | Claim-first RO、可定位 Evidence、身份静默路由、CPU parser/OCR、BGE-M3 hybrid retrieval、数据库分离、富媒体与 Hermes 受控权限的统一产品/技术合同 | **CURRENT / TASKS 1–6 DEPLOYED**；application/release `c581712`，Taskmaster 6/12 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、CPU parser cascade 验收、固定约束与 version tuple | **CURRENT active-memory**；branch/implementation/production `codex/hermes-wanko-live2d` / `c581712`，rollback `e2c0eaf` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.38 acceptance/FD9 transaction，§5.39 parser rollout/recovery/hygiene | **CURRENT**；application/release `c581712`，rollback `e2c0eaf`，production acceptance GREEN |
