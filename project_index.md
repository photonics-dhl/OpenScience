...
| `apps/web/lib/hermes/{live2d-core-loader,wanko-action-director,wanko-renderer-controller,wanko-live2d-renderer,wanko-model-presentation,wanko-runtime-ownership}.ts` / `apps/web/test/{hermes-wanko-action-director,hermes-wanko-renderer-contract,hermes-wanko-runtime-ownership}.test.ts` | 浏览器单 Wanko canvas/model/RAF owner；32-action profile 复用 byte-identical v09 12 motions，旧 OGL pet renderer、三张 pet PNG、carrier decoder/asset/exporter/CSS 已按授权永久移出当前 tree | **CURRENT v09 local runtime candidate** `2b9cef1`；公开 Hermes 资产只保留闭合 Live2D bundle 与 notice |
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；覆盖 Wanko 32-action、`360/200`、气泡/图层/像素/比例/exact-static、真实创建提交、drag/reload/resize、guide geometry、atomic speech 与主操作不遮挡；default production harness 仍 404 | **CURRENT LOCAL GATES GREEN** `2b9cef1`：Web `390+5`、canonical product E2E `40/40`、三视口 screenshots/metrics ignored；ECS `c97926a` 尚未更新 |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko 第一阶段验证角色；§10 原生灯/烟雾托座；§11 以 ECS `288/160px` 为基线精确放大至 `360/200px`，保留右栏停靠、拖动脱离、统一 semantic performance beat 与双语 optical-editorial 气泡系统 | **CURRENT Hermes visual/guide design — written design approved**；现有 Task 20 代码候选仍为 `c123a5a`，尚未实施新比例或部署 |
