...
| `apps/web/components/dashboard/LiteratureAcquisition.tsx` / `apps/web/lib/{api,literature-acquisition-state}.ts` / `apps/web/app/dashboard/page.tsx` / `apps/web/test/literature-acquisition-state.test.ts` / `apps/web/test/e2e/auth-dashboard.spec.ts` / `packages/domain/src/retrieval/browser-result.ts` / `packages/domain/src/agent/agent.ts` / `apps/agent-worker/src/retrieval/handler.ts` | ScanSci plan Task 7 Personal Space acquisition：persisted browser DTO/hash-only unresolved key、同 task/credit retry、服务端全历史 recovery priority、eligible-only Retry、permanent poll recovery、44px/inert source link 与临时下载 | **LOCAL TASK 7 REVIEW CANDIDATE `86e037e`**；6 条 component journey + Domain/API/Web gates green；ECS 375px/one-use download acceptance pending |
