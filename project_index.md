...
| `scripts/verify-release-source.mjs` / `scripts/release-sync-command.mjs` / `scripts/cloud-sync.mjs` / `scripts/evaluation-source-sync*.mjs` | 发布源守卫与云同步：生产 release 要求 clean `HEAD == release ref` 并写入 write-once `/opt/openscience-releases/<sha>`；评测同步仅允许 exact `git archive <sha>` 写入 `/opt/openscience-evals/document-parser/<sha>/source`，拒绝 active/release/mismatched target | **CURRENT release/evaluation boundaries**；完整输入、精确路径与 failed-stage cleanup 合同 GREEN |
