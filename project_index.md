...
| `apps/web/test/optical-lab-{model,contract,atlas,runtime-budget,asset-interaction}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics,asset-interaction-gate}.*` / `apps/web/test/visual/{capture-optical-lab-asset.mjs,fixtures/optical-lab-asset-accepted-1672x941.png}` | Optical Lab TDD/production-browser 门禁：production-alpha overlay-mask centroid `<=.04`；final composite 四相位五位置 centroid `<=.08`/locality `>=.80`；四象限 ambient、700ms local zero/<=900ms renderer PNG、real skip-draw/fixed-centre/follow/cap/halo mutations、touch 与全 lifecycle cleanup | Task 19 本地完整矩阵 GREEN；输出在忽略目录，fixture 入库 |
...
| `apps/web/test/visual/shots.mjs` | Task 15 Landing production browser gate：原生 1672×941 与 390×844 normal/reduced、唯一 main/h1/shared surface、导航/CTA/Latest Research、overflow/errors/focus、bounded pointer response，并对 desktop reduced accepted fixture 做 exact pixel identity | 本地 release candidate GREEN；线上矩阵 pending |
...
| `docs/handoff/2026-08-13-optical-lab-asset-interaction-handoff.md` | Optical asset 唯一 current 交接：Task 19 独立 overlay 本地 release candidate GREEN；记录双层 centroid 合同、700ms recovery、手机门禁豁免与部署边界；旧 Task 17/18 阻塞降为历史 | CURRENT；独立审查/提交/ECS 部署 pending |
...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 高保真生产共享规范：Task 19 复用 flow texture 的 geometry/visibility 通道并新增一个透明 overlay draw；70ms 响应、700ms local zero、2px ambient、5px follow、10px combined、.18 gain、8s idle cycle、纵向 .20/横向 .14；overlay `<=.04` / final `<=.08` | 物理手机门禁已豁免；本地 TDD/native/release GREEN，部署 pending |
...
| `docs/superpowers/plans/2026-08-11-optical-lab-high-fidelity-reconstruction-plan.md` / `.superpowers/sdd/2026-08-11-optical-lab-high-fidelity-reconstruction/task-8-report.md` / `.superpowers/sdd/2026-08-11-optical-lab-high-fidelity-reconstruction-plan/task-{13,15,17,18,19}-report.md` / `.superpowers/sdd/2026-08-11-optical-lab-high-fidelity-reconstruction-plan/final-fix-report.md` | 高保真 OGL 重构计划及逐 Task/final-fix 证据：Tasks 1–16 基线；Task 17 参数增强；Task 18 同一 composite 失败并撤回；Task 19 独立 transparent overlay 以真实 mask/disable mutation/full matrix 闭合 | Task 19 Steps 1–4 GREEN；Step 5 审查/提交/部署 pending |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、deploy.sh/DNS-01 实证）；§5.3 登记 Task 19 shared optical overlay、real `+5px` follow/10px cap、RTX desktop evidence、手机风险豁免与上线前后硬门禁 | 未部署；release/rollback/checkup/backup/public evidence pending |
...
| `docs/runbooks/visual-release.md` | Optical Editorial production-build 视觉/无障碍/性能发布门禁：27-case 矩阵、accepted shared surface/reduced exact/pointer focused gate、实测 route/static 预算、physical desktop/mobile cadence contract 与人工审美拒绝项；Task 19 含一次性用户风险豁免 | 活 runbook；本地代码/RTX desktop GREEN，Task 19 physical mobile 已豁免，ECS repeat pending |
