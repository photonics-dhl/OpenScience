...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/{OpticalLabClientMount,OpticalLabPage}.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,model,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页的历史 non-asset procedural 路径：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% authored DOM 几何、WebGL2-only OGL glyph/GPGPU/HDR composite 与三档质量策略；当前 asset 交互只以紧邻下一行的 full-surface contract 为准 | HISTORICAL non-asset Task 7：其 650ms rest 不适用于当前 `?candidate=asset`；生产 `/` 隔离不变且未部署 |
| `apps/web/components/optical-lab/AssetInteractionMount.tsx` / `apps/web/lib/optical-lab/asset-interaction-model.ts` / `apps/web/lib/optical-lab/ogl/{asset-interaction-renderer,asset-flow-pass}.ts` / `apps/web/lib/optical-lab/ogl/shaders/asset-{flow,composite}.ts` | 已验收 asset candidate 的独立 lazy OGL 交互层：全 stage ambient、真实 pointer/touch x/y 的 `.14` 局部分层流体响应、2/4/.08 上限、120ms 响应、900ms exact local rest、同 RAF renderer-owned readback 诊断、reduced-motion/context loss 静态回退与 generation-owned cleanup；`.58` 仅为 authored landmark | 本地工程 GREEN，等待用户动态视觉验收；仅 `?candidate=asset`，未进入生产 `/` |
| `apps/web/public/optical-lab/{target-reference,current-production,accepted-resting,energy-plate-black-alpha-v1}.png` / `apps/web/test/visual/promote-optical-lab-resting.mjs` | 用户授权参考/current、Task 5 decorative fallback 与用户确认的 1672×941 RGBA 黑底发光分解能量层；promotion 仅接受批准输出并记录 SHA-256 | 能量层 551,388 B，仅用于 `?candidate=asset` 隔离对照，不进入生产首页 |
...
| `apps/web/test/optical-lab-{model,contract,atlas,runtime-budget,asset-interaction}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics,asset-interaction-gate}.*` / `apps/web/test/visual/{capture-optical-lab-asset.mjs,fixtures/optical-lab-asset-accepted-1672x941.png}` | Optical Lab TDD/production-browser 门禁：58% authored DOM 构图、BMFont/CSS ink parity、静止五区材质、96×54 flowmap、full-surface pointer/touch/reduced/init/runtime/context-loss/unmount、五位置 centroid/locality、四象限 ambient、<=900ms renderer-owned PNG 完成时间与 ambient/active/recovered 非零像素、native 1672×941 baseline identity、halo 形状拒绝、运行预算与 exact cleanup | focused `35/35`、稳定外部 server 五位置矩阵、typecheck/build、静态与动态 browser gates GREEN；运行输出位于已忽略 `test/visual/out/optical-lab/`，baseline fixture 入库 |
...
| `docs/handoff/2026-08-13-optical-lab-asset-interaction-handoff.md` | 已验收静止资产后的唯一 current 交接：全画面分层流体交互、稳定 server 五连证据、dirty worktree 与精确续接顺序；旧固定中心正文已明确降为历史附录 | CURRENT；Tasks 11–13 工程 GREEN，3066 production preview 待用户动态视觉验收，禁止蓝色六面对象/旧 procedural/静止视觉重做，生产 `/`/ECS 不变 |
...
| `docs/specs/2026-08-11-optical-lab-high-fidelity-design.md` | `Science evolves.` 原型高保真重构：全画面分层流体已验收；最新放大修订将位移/亮度约提高两倍、radius 扩至 `.20`，并以共享表面推广至生产 Landing Hero | Tasks 11–13 已完成；用户已确认 Tasks 14–15 的增强与生产部署，待执行 |
...
| `docs/superpowers/plans/2026-08-11-optical-lab-high-fidelity-reconstruction-plan.md` / `.superpowers/sdd/2026-08-11-optical-lab-high-fidelity-reconstruction/task-8-report.md` / `.superpowers/sdd/2026-08-11-optical-lab-high-fidelity-reconstruction-plan/task-13-report.md` | 高保真 OGL 重构计划及逐 Task 证据：Tasks 1–13 完成静态、全画面流体与稳定空间门禁；Tasks 14–15 负责放大/扩域、共享 Landing 表面与生产发布 | Tasks 14–15 已获用户批准，待 TDD 执行与部署验收 |
