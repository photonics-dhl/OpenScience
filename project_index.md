...
| `apps/web/app/{%5Fvisual,_visual}/optical-lab/page.tsx` / `apps/web/components/optical-lab/*.tsx` / `apps/web/lib/optical-lab/{layout,runtime-policy,ogl/*}.ts` | 精确 `/_visual/optical-lab` 隔离比较页：单一 SSR selectable h1、批准的 Archivo/Bodoni 58% DOM 几何、WebGL2-only OGL runtime、CSS-pixel layout authority、OGL Text/BMFont derivative-MSDF mask/color/composite glyph pass、first-complete-frame 发布与 fresh-context 精确资源生命周期 | 高保真 Task 4；当前 marker `task-4-msdf-glyph-v1` 仅含真实字形，WebGL1/mobile/reduced/init/atlas failure 诚实 DOM；Task 5 材质须显式切换 `task-5-resting-material-v1`，生产 `/` 继续隔离 |
...
| `apps/web/test/optical-lab-{model,contract,atlas}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：三值 WebGL2 policy、58% DOM layout、首个完整 mask/color frame、AA 容差双向字形 shape overlap、per-word/occupied-column continuity、线性 atlas upload、resize/context restore、fallback 与 exact resource cleanup | 高保真 Task 4 focused 17/17、Web 全量 198/198、production glyph matrix GREEN；`runTask4MsdfGlyphAssertions` 绑定 `task-4-msdf-glyph-v1`，旧完整材质 probes 仅留待 `task-5-resting-material-v1`，未知 phase fail closed；输出位于已忽略 `test/visual/out/optical-lab/` |
