...
> **CURRENT source/deployment anchor（2026-08-25 12:47 +08）：** working branch deployment candidate application source `codex/hermes-wanko-live2d@e4a19d4`；当前 deployed application / immutable release 为 `eb55820` / `3010903`；local main / origin main `c60ffdd` / `7eb2f5b`；ECS release / rollback 为 `3010903` / `33418fd`。只有本锚点与主题唯一 CURRENT handoff 可决定当前状态。
...
| `apps/web/components/ui/context-menu.tsx` / `apps/web/components/hermes/{HermesVisualAdapter,HermesPresenceControl}.tsx` / `apps/web/lib/hermes/context-menu-actions.ts` | 无主题 Radix context-menu 行为原语、12 项 Hermes 动作真源与 original/compact/quiet 控制；桌面右键、Shift+F10/Menu、移动长按，普通点击保留 assistant drawer；研究动作进入真实 Hermes/Files/Versions surfaces | **CURRENT deployment candidate `e4a19d4`**；不继承 shadcn 默认视觉 |
...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；CURRENT 聚合覆盖 Wanko runtime/performance/静态锚定 geometry、19 个仍适用的 runtime/guide E2E 与 5 个菜单/输入抑制/真实研究导航产品场景；旧 40 条中的 21 条 cross-field travel/dark prompt/旧 dock 断言由 Research Folio 锚定契约取代；default production harness 仍 404 | **CURRENT candidate gates GREEN** application `e4a19d4`：Hermes `19+5`、Web `406+5`；active ECS 仍为 `3010903` |
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与五条 Hermes 入口/输入抑制/真实研究导航/反馈几何的 62 案 production browser 发布门禁；覆盖 landmark/focus/overflow/error/LCP/transfer/DOM 与 actor/menu/口部反馈不遮挡 | **CURRENT candidate release gate `62/62` GREEN**；application `e4a19d4`，production deploy pending |
...
| `docs/plans/2026-08-25-hermes-orbit-actions-plan.md` | 已批准的 Hermes 两段式发言、12 项情境动作、桌面环绕布局、移动分组、安静场景与 ECS 验收实施链 | **CURRENT / LOCAL IMPLEMENTATION COMPLETE / DEPLOY PENDING**；application `e4a19d4` |
