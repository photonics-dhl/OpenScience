...
| `apps/web/test/optical-lab-{model,contract}.test.*` / `apps/web/test/visual/optical-lab-{gate,shots,visual-metrics}.*` | Optical Lab TDD/production-browser 门禁：纯拓扑 fixtures、reference-relative resting waist/emission/continuity/directionality/particle-curtain、honest 单帧 150ms 交互、真实像素 forbidden geometry/ghost probe、transactional fallback、instanced draw、fresh context restore 与精确 GL cleanup | Candidate B fresh 14/14 + production matrix GREEN；输出位于已忽略 `test/visual/out/optical-lab/`；显式 steady-state FPS/CPU 与直接 3,840 cap 断言仍为 deferred Minor |
...
| `docs/handoff/2026-08-11-optical-lab-task8-steps3-5-handoff.md` | Optical Lab Task 8 Steps 3–5 + Candidate B 本地验收交接：实现/review-fix/globals commits、fresh 门禁、原尺寸视觉差异、SwiftShader 与 deferred performance Minor、Step 6 用户选择门禁 | 当前 handoff；Candidate B 工程收口，未部署，待用户视觉选择 |
