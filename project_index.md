...
| `packages/config/src/search-env.ts` / `packages/search/` | 独立 `SEARCH_DATABASE_URL` 配置与 Prisma client；core domain/API 不导入 search client | **TASK 2 DEPLOYED**；search 数据边界已建立，尚未写入检索业务数据 |
...
| `infra/scripts/rotate-database-credentials.sh` / `infra/scripts/rotate-database-credentials.test.mjs` | ECS-only PostgreSQL 应用凭据原子轮换：Secret 不入 argv/log、URL/初始化变量同步、角色经 stdin 更新、生产消费者重建、健康等待与失败回滚 | **CURRENT SECURITY OPERATION**；只可显式 `--confirm`，执行后须复验 release/公网/容器健康 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence Tasks 1–3、5、ECS 验收、固定约束、Task 4 接续与 version tuple | **CURRENT active-memory**；branch/HEAD `codex/hermes-wanko-live2d` / `bab1ec2`，application/release/rollback `f965966` / `f965966` / `ef043eb` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows 显式 Git Bash 专用密钥调用与版本化资产）；§5.30/5.31 是 Task 3/5 部署证据，§5.32 记录 Docling CPU candidate build 排障 | **CURRENT**；production application/release/rollback `f965966` / `f965966` / `ef043eb`；candidate 未部署 |
