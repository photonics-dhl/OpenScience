...
| `apps/web/components/ui/context-menu.tsx` / `apps/web/components/hermes/{HermesVisualAdapter,HermesPresenceControl}.tsx` / `apps/web/lib/hermes/context-menu-actions.ts` | 无主题 Radix context-menu 行为原语、12 项 Hermes 动作真源与 original/compact/quiet 控制；桌面右键、Shift+F10/Menu、移动长按，普通点击保留 assistant drawer；研究动作进入真实 Hermes/Files/Versions surfaces | **CURRENT DEPLOYED `e4a19d4` / release `7165e9b`**；不继承 shadcn 默认视觉 |
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与五条 Hermes 入口/输入抑制/真实研究导航/反馈几何的 62 案 production browser 发布门禁；覆盖 landmark/focus/overflow/error/LCP/transfer/DOM 与 actor/menu/口部反馈不遮挡 | **CURRENT release gate `62/62` + public Hermes `5/5` GREEN**；application `e4a19d4`，ECS `7165e9b` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows Git Bash 专用密钥调用与版本化资产）；§5.3–5.19 登记 Optical、Hermes 与 Research Folio 生产发布 | CURRENT；最新 ECS release/rollback `7165e9b` / `3010903`；公网 Hermes focused `5/5` |
