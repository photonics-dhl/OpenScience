...
| `docs/specs/2026-09-18-figure-plan-reuse-binding.md` | `figurePlan.reuse` 的论文原图绑定设计提案：`paper_original_figure` 新 subtype、`(researchObjectId, versionId, figureId)` 查找、版本作用域、`paper_original_missing_<id>` 失败模式、parser 检测 vs 手动上传 | **已实现**：`packages/domain/src/assets/paper-figure.ts:registerPaperFigure` + `apps/api/src/routes/paper-figures.ts` 提供 `POST /research-objects/:id/versions/:vid/paper-figures`（workspace write scope；来源批准沿既有审阅权限）；`findPaperOriginalAssets` + `requirePaperOriginalsForReuse` 在 `submitPresentationGeneration` 内强制；illustration-planner 在 `reuse` + 绑定存在时本地构造场景（不走 LLM），handler 图像阶段直接拷贝 bytes 走 chatgpt-web 之外；`packages/ai-gateway/src/gateway.ts` chat-review retry 2 → 4；`scene-image.ts` 当时加入 `transliterateMathToAscii`；9/20 候选已移除有损转译，保留原科学符号（状态见 CURRENT） |
