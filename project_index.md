...
| `apps/agent-worker/src/parser-evaluation.ts` / `apps/agent-worker/test/{parser-evaluation,parser-evaluation-corpus-export}.test.ts` / `infra/scripts/evaluate-document-parsers.sh` / `infra/parser-candidates/liteparse/` / `infra/parser-candidates/docling/` | Task 4 ECS-only 候选 bake-off：content-free/false-ready-safe 报告、schema-v2 16-case exporter 与 7-PDF layout 子集、候选白名单、canonical owned 输出根、无网络/只读/非 root/CPU-RSS-PID-time-64KiB-output 硬限制、显式 kill/remove 与 staging 原子发布；LiteParse/Docling 均禁用 OCR，Docling 在 corpus 前输出包与模型 aggregate lock | **CURRENT**；LiteParse ECS gate 完成，Docling source-locked local gate GREEN/ECS pending；均为 `APPROVED_PILOT`，不修改 production Compose |
