...
| `apps/web/components/hermes/{HermesVisualAdapter,HermesRiggedPortrait,HermesWorkspaceStage,HermesDockAnchor,HermesAnchor,HermesGuideBubble,HermesDraftDiff,hermes-state}.ts(x)` / `apps/web/lib/hermes/{action-catalog,behavior-director,motion-mixer,anchor-registry,travel-path,dock-preferences,motion-preference,pet-motion,pet-mesh-renderer}.ts` | Workspace-wide Hermes CURRENT 实现：单一 OGL mesh owner、27-action 分层动作、shuffle-cycle、安全航点、用户停靠优先、语义字段引导、reviewable diff 与显式 full/system/reduced 动效偏好 | **CURRENT implementation**；production release GREEN，用户环境需以 `?hermes-motion=full` 复验 |
...
| `docs/specs/2026-08-17-hermes-workspace-companion-motion-design.md` | Hermes Workspace Companion CURRENT 设计：四层 mesh 动作混合、25+ 行为、C 安全航道、语义字段引导、用户停靠、reviewable diff、移动端/reduced 与真实像素验收 | **CURRENT Hermes visual/guide design**；核心候选已实现，待最终视觉验收 |
...
| `docs/plans/2026-08-17-hermes-workspace-companion-motion-plan.md` | Hermes Workspace Companion 八任务实施计划：事实源、行为导演、四层 mixer、锚点/路径/停靠、全局 stage、字段引导、diff 与真实 release gate | **CURRENT execution plan**；核心候选/release gate GREEN，高级偏好 UI 与两信号提示仍待后续，提交/部署等待视觉验收 |
...
| `apps/web/test/{hermes-behavior-director,hermes-motion-mixer,hermes-travel-path,hermes-dock-preferences,hermes-anchor-registry,hermes-draft-diff}.test.ts` / `apps/web/test/e2e/{hermes-workspace-stage,hermes-field-guide,hermes-draft-diff}.spec.ts` | Workspace Companion 行为、动作混合、4×5 路径、停靠/锚点、single-flight Diff 与真实路由/表单交互测试 | CURRENT acceptance evidence |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Workspace Companion CURRENT 交接：单一 stage、27-action、航行/字段/Diff、真实 release 证据与明确未完成边界 | **CURRENT** |
