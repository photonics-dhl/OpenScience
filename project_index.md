...
> **CURRENT source/deployment anchor（2026-08-30）：** repository main and application source/production release `689331845574612130f223d08c92e61721c16586`；rollback `c435c4c8b2800bb20998fd9a9a93f2db96328661`；production core/search `32/32` / `2/2`。Task 10 已部署、真实生产验收并合并；Taskmaster 10/12，Task 11 dependency-ready。后续 docs-only HEAD 与 ECS application source 仍不得混写。
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 10/12，Tasks 1–10 done | 工具自管；Task 11 dependency-ready |
...
| `docs/plans/2026-08-30-hermes-external-retrieval-lifecycle-plan.md` | Taskmaster Task 10：Semantic Scholar/Tavily/ScanSci legal-only adapter、rights provenance、72h 私有缓存、10min 认证签名链接、租约 GC 与 ECS canary | **COMPLETE / PRODUCTION ACCEPTED `6893318`**；ScanSci legal-only 默认 disabled，Tavily 当前额度耗尽 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、Task 10 验收、磁盘卫生、固定约束与下一动作 | **CURRENT active-memory**；main/production `6893318`，rollback `c435c4c`；Task 11 next |
