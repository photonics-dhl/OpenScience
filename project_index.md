...
| `apps/web/components/hermes/{HermesVisualAdapter,HermesRiggedPortrait,HermesWorkspaceStage,HermesDockAnchor,HermesAnchor,HermesGuideBubble,HermesDraftDiff,hermes-state}.ts(x)` / `apps/web/lib/hermes/{action-catalog,behavior-director,motion-mixer,anchor-registry,travel-path,dock-preferences,motion-preference,pet-motion,pet-mesh-renderer,part-rig,part-rig-shaders,hermes-runtime-status}.ts` | Workspace-wide Hermes CURRENT 实现：单一 OGL mesh owner、27-action 分层动作、安全航点、语义字段引导、reviewable diff、bind-UV 语义部件刚体位移、draw heartbeat 与有界 context-loss recovery；透明舞台 pointer-through，仅真实 rig/控制/bubble 命中 | **CURRENT production** `39c752b`；公网真实队列/heartbeat/idle/pointer GREEN，rollback `1b76b46` |
| `apps/web/public/hermes/pet/` / `apps/web/scripts/hermes/inspect-pet-assets.mjs` / `apps/web/test/hermes-pet-asset-contract.test.ts` | 三张原创 824px RGBA 纹理、来源说明与资产合同；旧整图/CSS-signal presentation 已删除 | 纹理/fallback 资产继续复用；不得单独作为生命感证据 |
...
| `apps/web/test/workspace-readability.test.ts` / `apps/web/test/e2e/workspace-readability.spec.ts` / `apps/web/test/visual/workspace-readability-gate.mjs` | 三视觉系统阅读与控件合同：14–17 px 语义角色、浏览器无关控件、必要文本对比/裁切/可访问名，以及桌面/移动代表路由 | **CURRENT local candidate** `3cb00c1`；本地 GREEN，尚未部署 |
...
| `infra/scripts/deploy.sh` / `infra/scripts/deploy.test.mjs` | ECS dry-run/confirmed release 部署；所有 production Compose 路径强制同一 `.env.prod`；自包含 Parser 镜像先行 healthy、全栈 `--wait`，随后重启 bind-mounted `api/web/agent-worker` 并硬校验应用健康与公网状态 | CURRENT；数据服务与 Parser 不进入源码重启集合 |
...
| `docs/specs/2026-08-17-hermes-workspace-companion-motion-design.md` | Hermes Workspace Companion CURRENT 视觉/guide renderer 合同：四层 mesh 动作混合、25+ 行为、C 安全航道、语义字段引导、用户停靠、reviewable diff、移动端/reduced 与真实像素验收 | **CURRENT Hermes visual/guide design**；2026-08-18 readable-workspace design 仅接管本轮增量实施范围 |
| `docs/specs/2026-08-18-readable-workspace-hermes-guidance-design.md` | B 平衡学者工作台、三视觉系统阅读基线、浏览器无关控件样式、Hermes 不遮挡引导编舞与公网 blank RO→六字段→diff→commit 真实验收 | **CURRENT implementation design**；Tasks 1–6 本地候选完成，公网验收待 Task 7 |
| `docs/plans/2026-08-18-readable-workspace-hermes-guidance-plan.md` | 可读性基础→代表页面→完整 Hermes footprint 避让→edit-before-accept/缺失证据→语义动作→公网 blank RO gate→ECS checkpoint 的 TDD 实施链 | **CURRENT execution plan**；Tasks 1–6 完成于 `80cd6e9`，Task 7 next |
...
| `packages/domain/src/agent/agent.ts` / `apps/agent-worker/src/index.ts` / `apps/agent-worker/test/queue-recovery.test.ts` | Hermes guide 的 Serializable credit 预留、三次 `P2034` 上界、未派发 DB outbox 协调与单消费者 processing 启动恢复；ingestion 两阶段任务保持隔离 | CURRENT；崩溃恢复合同 GREEN |
| `scripts/docs/hermes-renderer-index.mjs` / `scripts/docs/hermes-renderer-index.test.mjs` | Hermes 事实源门禁：唯一 CURRENT 是 2026-08-17 Workspace Companion，旧 articulated/2.5D spec 保持 DEPRECATED/NO-GO | CURRENT；4/4 合同 GREEN |
...
| `apps/web/test/e2e/hermes-blank-ro-flow.spec.ts` / `apps/web/test/visual/hermes-blank-ro-production-gate.mjs` | blank RO 字段级 accept/edit-accept/reject/missing/save/reload/commit 合同；公网脚本要求真实 secure session、MiniMax/真实 API、零网络拦截、唯一 task 与 idle/travel/working/review | `80cd6e9` 本地 production-build mock `1/1` GREEN；真实 ECS leg 待 Task 7 部署后执行，输出仅写 ignored evidence |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Readable Workspace / Hermes 唯一 CURRENT 交接：当前入口、branch/HEAD/release/rollback、约束与单一 next action | **CURRENT / compact active-memory**；Tasks 1–6 本地候选完成，Task 7 next |
...
| `docs/decisions/ADR-010-hermes-visual-runtime-and-live2d-license-gate.md` | Hermes 原创 articulated renderer、静态 fallback、真实任务/六态/单实例契约；Wanko/Live2D 仅在运营主体记录资格与协议接受后启用 | Accepted |
...
| `infra/nginx/openscience.test.mjs` | 生产 Nginx 合同：`/api` rewrite、Auth 页面/API 分流、Curator 保护、Tunnel 真实 IP 与部署同步 | 5/5 GREEN（2026-08-12） |
