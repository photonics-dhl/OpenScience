...
> **CURRENT source/deployment anchor（2026-08-25 01:52 +08）：** working branch application source `codex/hermes-wanko-live2d@eb55820` + candidate metadata；deployed application remains `33418fd`；local main / origin main `c60ffdd` / `7eb2f5b`；ECS release / rollback 为 `33418fd` / `2abfe42`。只有本锚点与主题唯一 CURRENT handoff 可决定当前状态。
...
| `apps/web/components/ui/context-menu.tsx` / `apps/web/components/hermes/HermesVisualAdapter.tsx` | 无主题 Radix context-menu 行为原语与真实 Hermes 产品菜单；桌面右键、Shift+F10/Menu、移动长按，普通点击保留 assistant drawer；视觉由 Research Folio 定义 | CURRENT product interaction；不继承 shadcn 默认视觉 |
...
| `apps/web/components/shell/ResearchFolioPrimitives.tsx` / `apps/web/app/{layout.tsx,tokens.css,globals.css}` | Research Folio 非 Landing 产品地基：全局 Source Serif 4/Noto Serif SC 阅读、暖纸层级、graphite 工具例外、workflow/evidence/decision 原语与旧暗色类兼容桥 | CURRENT deployment candidate；Landing selectors unchanged |
| `apps/web/components/hermes/{HermesVisualAdapter,HermesRiggedPortrait,HermesWorkspaceStage,HermesPerformanceBubble,HermesGuideBubble,HermesDockAnchor,HermesAnchor,HermesDraftDiff,hermes-state}.ts(x)` / `apps/web/lib/hermes/{action-catalog,behavior-director,performance-beat,performance-director,companion-placement,stage-sizing,anchor-registry,travel-path,dock-preferences,motion-preference,hermes-runtime-status}.ts` | Workspace-wide Hermes foundation：精确 desktop/mobile `360/200`；锚定态保留显式 guide/menu/drawer，companion feedback 为口部锚定单句暖纸气泡；连续 ledger 菜单、224px mobile、reduced-motion 与 detached runtime foundation 保持 | **CURRENT CANDIDATE** `eb55820`；ECS 仍为 `33418fd`，rollback `2abfe42` |
...
| `apps/web/app/_visual/hermes-live2d/page.tsx` / `apps/web/app/%5Fvisual/hermes-live2d/page.tsx` / `apps/web/test/visual/hermes-live2d-motion-gate.mjs` / `apps/web/test/visual/hermes-work-assistant-experience-gate.mjs` / `apps/web/test/visual/hermes-companion-motion-gate.mjs` / `apps/web/test/visual/hermes-performance-gate.mjs` / `apps/web/test/visual/hermes-release-gate.mjs` / `apps/web/test/e2e/hermes-dashboard.spec.ts` / `apps/web/test/e2e/hermes-field-guide.spec.ts` | 明示 flag 才开放的真实 RO-create fixture + 折叠诊断 tray；CURRENT 聚合覆盖 Wanko runtime/performance/静态锚定 geometry、19 个仍适用的 runtime/guide E2E 与 3 个右键/键盘/长按/口部反馈场景；旧 40 条中的 21 条 cross-field travel/dark prompt/旧 dock 断言由 Research Folio 锚定契约取代，文件仅保留 historical evidence；default production harness 仍 404 | **CURRENT candidate gates GREEN** `eb55820`：Hermes `19+3`、Web `403+5`；真实账号纵向 gate 未重跑 |
...
| `apps/web/test/visual/product-release-manifest.mjs` / `apps/web/test/e2e/product-release.spec.ts` / `apps/web/playwright.release.config.ts` | 18 个真实产品表面 × 三视口、Landing reduced-motion 与三条 Hermes 右键/键盘/长按/反馈几何的 60 案 production browser 发布门禁；覆盖 landmark/focus/overflow/error/LCP/transfer/DOM 与 actor/menu/口部反馈不遮挡 | **CURRENT Research Folio release gate `60/60` GREEN**；candidate `eb55820` fresh rerun |
...
| `docs/specs/2026-08-19-hermes-wanko-live2d-design.md` | Wanko 第一阶段角色与 runtime；§11 movable `360/200px`；§12 Research Session Folio；§13.1 approved mouth-anchored single-sentence speech 与 carried-tool ledger | **CURRENT Hermes visual/guide design**；§13.1 candidate `eb55820` pending deployment |
...
| `docs/plans/2026-08-24-research-folio-product-system-plan.md` | Research Folio 的合同、共享 shell、Hermes 锚定几何、identity/Dashboard/create/RO/public/admin 迁移、60-case production browser 门禁与 immutable ECS 发布链；Tasks 9–10 为口部 speech/menu refinement | **Tasks 1–9 COMPLETE；Task 10 predeploy COMPLETE**；candidate source `eb55820`，ECS `33418fd` / `2abfe42` |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | Hermes Wanko Live2D 唯一 CURRENT 交接：mouth-anchored speech candidate、Research Folio release、本地/远端/ECS version tuple、约束与验收入口 | **CURRENT / compact active-memory**；candidate `eb55820`；production `33418fd`，rollback `2abfe42` |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser 隔离、Windows Git Bash 专用密钥调用与版本化资产）；§5.3–5.17 登记 Optical、Hermes 与 Research Folio 生产发布 | CURRENT；最新 ECS application release/rollback `33418fd` / `2abfe42`；公网 focused `4/4` |
