...
| `apps/web/components/optical-lab/AssetInteractionMount.tsx` / `apps/web/lib/optical-lab/asset-interaction-model.ts` / `apps/web/lib/optical-lab/ogl/{asset-interaction-renderer,asset-flow-pass}.ts` / `apps/web/lib/optical-lab/ogl/shaders/asset-{flow,composite,overlay}.ts` | accepted shared surface 的 OGL 层；Task 22 idle `.05/6px`、三尺度流与不回绕 10s shader clock；Task 23 本地 Landing-only presentation path 以 `uPresentationAlpha` 增加完整合成透明度、`2.2px` 非线性漂移、中央曲率呼吸和字缘高光，全部在 local input 下让位且 Lab 为零；交互仍为 70ms/5px/10px/.18、700ms local zero | 生产仍为 Task 22 `744c631`；Task 23 发布授权齐备，待执行部署 |
...
| `docs/superpowers/specs/2026-08-14-optical-water-flow-refinement-design.md` | Task 22 成熟商业水流规律落地；Task 23 amendment 将验收改为最终合成可见性，并以 Landing-only presentation alpha 纠正“原始 canvas 有变化但肉眼静止” | Task 22 `744c631` 已部署；Task 23 用户已批准，等待手机豁免后部署 |
| `docs/superpowers/plans/2026-08-14-optical-water-flow-refinement-plan.md` | Task 22 水流/污染计划与 Task 23 最终合成显著性纠偏计划 | Task 22 完成；Task 23 Step 1–2 完成、Step 3 计时门禁未闭合、未部署 |
