...
| `apps/web/app/%5Fvisual/central-particle/page.tsx` / `apps/web/components/optical-prototype/*` / `apps/web/lib/optical-prototype/{runtime-policy,lifecycle,renderer,particle-grid,continuous-title,field-model,curtain-grid,touch-texture,shaders/*}.ts` | no-index Three.js WebGL2 隔离原型：唯一 SSR h1 + 同源 SVG、route-only dynamic import、连续 GPU outline title、508 个 seam-local glyph points + 1,305 个独立 curtain points、57.3%/50% 径向/切向/右向固定场、clean-room TouchTexture、双轴 contain/pointer 同构及完整 ownership | Native prototype Task 5 工程门禁 GREEN，1,813 instances；全字 4,619 点阵仅 debug，移动/reduced 静态 SVG，生产 Hero/ECS 未修改；Bloom/局部色散留待 Task 6 与用户视觉裁决 |
...
| `apps/web/test/{landing-page,optical-field,landing-motion-policy,central-particle-dependencies,central-particle-geometry,central-particle-runtime,central-particle-field,central-particle-visual-metrics}.test.*` / `apps/web/test/visual/central-particle-visual-metrics.mjs` / `apps/web/test/visual/central-particle-frame-metrics.mjs` | Landing 与中央粒子原型回归门禁：依赖/import、字体 geometry、连续字形、seam/default-debug 边界、规则 field/curtain/TouchTexture、双轴 viewport、asset/init/WebGL2 policy/resize/context/cleanup，以及拒绝点阵字、机械柱、对称扇、环、随机散点的图像指标 | Native prototype Task 5 focused 56/56；production fixed-time 1672×935 + resize/context/static/invalid-shader browser matrix GREEN，截图位于 ignored `apps/web/test/visual/out/central-particle/` |
