...
> **CURRENT source/deployment anchor（2026-08-26 12:46 +08）：** working branch/HEAD `codex/hermes-wanko-live2d@2b0c011`；local candidate / deployed application / immutable release 为 `2b0c011` / `47c8aa9` / `73677d5`；local main / origin main `c60ffdd` / `7eb2f5b`；ECS release / rollback 为 `73677d5` / `263c783`。只有本锚点与主题唯一 CURRENT handoff 可决定当前状态。
...
| `apps/web/components/brand/{OpticalHeadline,OpticalField}.tsx` / `apps/web/lib/optical-field/*.ts` | 保留的 Canvas/glyph-particle 媒介与模型；OGL 可用时不挂载，仅在 normal-motion + WebGL unavailable 时以 accepted plate 提供同源标题水纹 fallback | **Task 24 continuity candidate `2b0c011`**；reduced-motion 不挂载，勿误删 |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Hermes 冻结、Landing Task 24 与本地/ECS version tuple、约束及验收入口 | **CURRENT active-memory**；local candidate / deployed application / release / rollback `2b0c011` / `47c8aa9` / `73677d5` / `263c783` |
...
| `docs/superpowers/specs/2026-08-14-optical-water-flow-refinement-design.md` | 唯一 CURRENT Landing water spec；Task 24 恢复系统 cursor、真实慢/快轨迹与连贯 OGL 水流，并规定 WebGL unavailable 时的 Canvas continuity fallback | **TASK 24 continuity candidate `2b0c011`**；待生产部署 |
| `docs/superpowers/plans/2026-08-14-optical-water-flow-refinement-plan.md` | Task 22/23 历史实施；Task 24 已恢复 OGL 水流并补 WebGL-disabled Canvas fallback、滚出/滚回与跨发布回归保护 | **Step 7 local GREEN `2b0c011`**；待公网 no-WebGL 复测 |
