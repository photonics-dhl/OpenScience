...
| `apps/agent-worker/src/parser-evaluation.ts` / `apps/agent-worker/test/{parser-evaluation,parser-evaluation-corpus-export}.test.ts` / `infra/scripts/evaluate-document-parsers.sh` / `infra/parser-candidates/liteparse/` | Task 4 ECS-only 候选 bake-off：content-free/false-ready-safe 报告、schema-v2 16-case exporter 与 7-PDF layout 子集、候选白名单、canonical owned 输出根、无网络/只读/非 root/CPU-RSS-PID-time-64KiB-output 硬限制、显式 kill/remove 与 staging 原子发布；LiteParse 2.14.0 独立 lock 与无 OCR 入口 | **CURRENT / LOCAL GATES GREEN / ECS EVALUATION PENDING**；不修改 production Compose，LiteParse 保持 `APPROVED_PILOT` |
