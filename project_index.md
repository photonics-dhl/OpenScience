...
| `packages/ai-gateway/src/{ocr,gateway,provider,ocr-contract-self-test}.ts` / `packages/ai-gateway/test/{ocr-gateway,minimax-vision-provider,provider-boundary}.test.ts` | provider-neutral LLM OCR candidate contract；Gateway-owned prompt、page/magic/dimension/byte bounds、external-processing policy、per-provider kill switch、MiniMax Coding Plan VLM adapter、hash/versioned-cost/latency audit | **TASK 5 IN PROGRESS / DEFAULT DISABLED**；35/35 Gateway tests + compiled self-test GREEN；credential rotation and paid canary not performed |
...
| `docs/plans/2026-08-27-hermes-ai-gateway-llm-ocr-routing-plan.md` | Taskmaster Task 5 的可执行 TDD 计划：provider-neutral OCR、MiniMax Coding Plan VLM adapter、逐页 fallback、kill switch、hash/cost/latency 审计与 ECS-only 验收 | **CURRENT / IN PROGRESS**；默认不启用付费 vision route |
